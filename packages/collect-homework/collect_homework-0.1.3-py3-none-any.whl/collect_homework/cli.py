...
@cli.command('diff', help="do a git diff on the repos")
...
function summary() {
    local quiet="$1"; shift
    local slug="$1"; shift
    [[ -d $slug ]] || { if [[ "$quiet" == False ]]; then printf '%27s ' $slug; echo MISSING; fi; return; }
    git -C $slug log >& /dev/null || { if [[ "$quiet" == False ]]; then printf '%27s ' $slug; echo EMPTY; fi; return; }
    printf '%24s ' $slug
