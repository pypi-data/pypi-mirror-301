import sys
import os
import requests
from typing import List, Dict, Any, Optional
from pydantic import BaseModel
import time

root_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
sys.path.insert(0, root_dir)

from get_embeddings import get_embeddings

QDRANT_SERVER_URL = "http://localhost:8100"

class SearchQuery(BaseModel):
    search_phrase: str = ''
    filter: Optional[Dict[str, Any]] = None
    top_k: int = 20

def get_user_memory(
    search_string: str,
    folders: List[str] = [],
    start_time_unix: int = 0,
    end_time_unix: int = int(time.time()),
    top_k: int = 5
) -> List[Dict[str, Any]]:
    search_query = SearchQuery(
        search_phrase=search_string,
        filter={
            "must": [
                {
                    "key": "timestamp_unix",
                    "range": {
                        "gte": start_time_unix,
                        "lte": end_time_unix
                    }
                }
            ]
        },
        top_k=top_k
    )

    result = []
    
    for folder in folders:
        try:
            endpoint = f"{QDRANT_SERVER_URL}/search_{folder}"
            response = requests.post(endpoint, json=search_query.dict())
            response.raise_for_status()
            search_result = response.json()
            print(f"{folder.upper()} CONTEXT: ", search_result)
            if search_result:
                result.extend(search_result)
        except requests.RequestException as e:
            print(f"Error querying {folder} from Qdrant server: {e}")

    return result

def upsert_memory(
    collection: str,
    content: str,
    timestamp_unix: Optional[int] = None
) -> Dict[str, str]:
    if timestamp_unix is None:
        timestamp_unix = int(time.time())

    vector = get_embeddings(content)
    point = {
        "vector": vector,
        "payload": {
            "content": content,
            "timestamp_unix": timestamp_unix
        }
    }

    try:
        endpoint = f"{QDRANT_SERVER_URL}/upsert_{collection}"
        response = requests.post(endpoint, json=point)
        response.raise_for_status()
        return response.json()
    except requests.RequestException as e:
        print(f"Error upserting to {collection} in Qdrant server: {e}")
        return {"error": str(e)}

def delete_collection(collection_name: str) -> Dict[str, str]:
    try:
        endpoint = f"{QDRANT_SERVER_URL}/delete_vectors"
        response = requests.post(endpoint, json={"collection_name": collection_name})
        response.raise_for_status()
        return response.json()
    except requests.RequestException as e:
        print(f"Error deleting collection {collection_name} from Qdrant server: {e}")
        return {"error": str(e)}

