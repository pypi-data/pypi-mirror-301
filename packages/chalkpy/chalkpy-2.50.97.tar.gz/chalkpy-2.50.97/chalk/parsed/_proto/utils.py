...
_max_timedelta_seconds = timedelta.max.total_seconds()


def seconds_to_proto_duration(seconds: float) -> duration_pb2.Duration:
    return timedelta_to_proto_duration(
        timedelta.max if seconds >= _max_timedelta_seconds else timedelta(seconds=seconds)
    )


def seconds_int_to_proto_duration(seconds: int) -> duration_pb2.Duration:
    seconds = min(MAX_DURATION_SECONDS, seconds)
