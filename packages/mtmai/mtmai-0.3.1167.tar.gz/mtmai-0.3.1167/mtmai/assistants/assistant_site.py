...
from mtmai.models.chat import AssisantConfig, AssisantStart, AssisantWorkBrench
...
class SiteAssistant:
    async def get_config(self):
        return AssisantConfig(
            chatProfile="site",
            markdown_description_tpl="""
            <h1>站点助手</h1>
            <p>站点助手是站点的智能助手，可以协助用户完成站点的各种操作和配置。</p>
            """,
            starts=[
            ...
            workbenchs=[
                AssisantWorkBrench(label="站点", description="站点设置"),
                AssisantWorkBrench(label="文章", description="文章管理"),
                AssisantWorkBrench(label="任务", description="任务管理"),
            ]
        )

    def get_workbench_prompt(self):
        config = self.get_config()
        return f"""<workbench>{json.dumps(config.workbenchs)}</workbench>"""
    ...
    async def stream_messages(self, messages: list[BaseMessage], user_id: str):
        ...
        assistant_prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    """你是Bolt,是专业有用的助手，会进行工作流程的思考,协助用户完成 site 项目的操作和配置

<background_story>
  用户能跟你聊天必定已经选定了 siteId 参数，这个参数表示用户选定的站点。
  ...
  <example>
    ...
    <assistant_response>
      ...
      已经启动任务帮你生成文章，请留意任务进度，任务结束后你可以进行进一步的编辑。
            ...
            tag_names=tag_names,
            MODIFICATIONS_TAG_NAME=MODIFICATIONS_TAG_NAME, cwd="",
            workbench_prompt=self.get_workbench_prompt(),
        )
