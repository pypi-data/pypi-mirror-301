...
async def chat_config(request: ChatConfigRequest, user: OptionalUserDep, db: SessionDep):
...
async def chat_completions(
    ...
    chat_profile = request.headers.get("X-Chat-Profile")

    assistant_agent = await get_assistant_agent(chat_profile)
    return StreamingResponse(
        assistant_agent.stream_messages(messages=messages, user_id=str(user.id)),
        media_type="text/event-stream",
        headers={
...
我将为你创建一个最简单的配置。我将使用 Vite 来快速搭建一个简单的 HTML 和 JavaScript 项目。这是一个入门级的开发环境，非常适合简单的项目。

<boltArtifact id="simple-vite-project" title="创建最简单的 Vite 项目">
...
我已经为你创建了一个最简单的项目。这个项目包含以下内容：

1. 一个基本的 HTML 文件（index.html）
2. 一个简单的 JavaScript 文件（main.js）
3. 一个 package.json 文件，用于配置项目依赖和脚本

项目已经设置完成，开发服务器正在运行。你可以在浏览器中看到一个简单的网页，显示 "你好，世界！" 和一条简单的消息。

这个项目为你提供了一个良好的起点。你可以根据需要修改 HTML 和 JavaScript 文件来扩展项目功能。如果你想添加样式，可以创建一个 CSS 文件并将其链接到 HTML 中。

如果你有任何问题或者想要进一步开发这个项目，请随时告诉我！
"""
    for token in example_stream_response.split(" "):
        yield aisdk.text(token + " ")
