...
from mtmai.agents.assisant import Assistant
...
async def chat_completions(
    ...
    # return aisdk.AiSDKStreamResponse(
    #     fake_video_streamer(),
    # )

    # assistant_config = mtmai_context.graph_config.assistants[0]
    # primary_assistant = AssistantNode(assistant_config)
    # async for a in await primary_assistant.run_graph(messages=messages):
    #     print(a)

    assistant_agent = Assistant()
    return StreamingResponse(
        # fake_video_streamer(),
        assistant_agent.stream_messages(messages=messages),
        media_type="text/event-stream",
        headers={
