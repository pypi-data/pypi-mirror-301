...
# Export abstract base classes and other useful items from this module.
...
Scalar = time.Scalar
LatinHypercube = sampler.LatinHypercube
...
def simulate_from_model(
    instance, particles=1, observations=1, common_prng_seed=False
):
    """
...
def forecast(ctx, times, filename):
    ...
    invalid_fs = [
        ctx.component['time'].to_unicode(t)
        for t in times
        if t < start or t >= end
    ]
    if invalid_fs:
    ...
    if start < est_end:
        ...
        adaptive = ctx.get_setting(
            ['filter', 'adaptive_fit', 'enabled'], False
        )
        if adaptive:
        ...
        logger.info(
            '  {}  No estimation pass needed for {}'.format(
    ...
    for start_time in forecast_times:
        ...
        do_resample = ctx.get_setting(
            ['filter', 'resample', 'before_forecasting'], False
        )
        if do_resample and update['history'] is not None:
            logger.debug(
                '  {:%H:%M:%S}  Resampling before forecast'.format(
        ...
        if 'max_forecast_ahead' in ctx.settings['time']:
            max_end = ctx.component['time'].add_scalar(
                start_time, ctx.settings['time']['max_forecast_ahead']
            )
            if max_end < fs_end:
                fs_end = max_end

        logger.info(
            '  {}  Forecasting from {} to {}'.format(
                datetime.datetime.now().strftime('%H:%M:%S'),
        ...
        fstate = pfilter.run(
            ctx, start_time, fs_end, {}, history=update['history']
        )
