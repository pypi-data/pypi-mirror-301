...
libasyncevent = os.path.join(lib_dir, "libSqlAsyncEventHandler.so")
...
class ConnectionAsyncLinux:
    ...
    def __init__(self, conn_str):
        ...
        self.async_lib = ctypes.CDLL(libasyncevent)
    ...
    def _initialize(self):
        self._create_async_event_manager()
    ...
    def _create_async_event_manager(self):
        # self.event_mgr = self.async_lib.AsyncSQLEventManager_new(self.odbc.SQLDriverConnectW, self.odbc.SQLExecDirectW)
        self.event_mgr = AsyncSQLEventManager(self.odbc.SQLDriverConnectW, self.odbc.SQLExecDirectW)
    ...
    def _create_event(self, handle, handle_type):
        event_obj = self.async_lib.Event_new_with_arg(handle, handle_type)
        return event_obj

    def wait_for_query_completion(self):
        if self.query_ret == const.SQL_STILL_EXECUTING:
            # self.async_lib.Wait(self.query_event)
            self.query_event.wait()
        
    def _connect_to_server(self):
        # conn_event = self._create_event(self.hdbc, const.SQL_HANDLE_DBC)
        conn_event = Event(handle=self.hdbc, event_type=const.SQL_HANDLE_DBC)

        # self.async_lib.addConnHandle(self.event_mgr,conn_event)
        self.event_mgr.add_conn_handle(conn_event)
    ...
    def execute(self, query):
        # stmt_event = self._create_event(self.hstmt, const.SQL_HANDLE_STMT)
        stmt_event = Event(handle=self.hstmt, event_type=const.SQL_HANDLE_STMT)

        # self.async_lib.addStmtHandle(self.event_mgr, stmt_event)
        self.event_mgr.add_stmt_handle(stmt_event)
