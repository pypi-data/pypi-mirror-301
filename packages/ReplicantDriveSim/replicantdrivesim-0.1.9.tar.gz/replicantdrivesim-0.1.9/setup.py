...
class CMakeBuild(build_ext):
    ...
    def build_extension(self, ext):
        ...
        print(glob.glob(os.path.join(build_temp, "*")))
        lib_paths = glob.glob(
            os.path.join(
                build_temp, 
                #"lib",
                #f"libReplicantDriveSim*{self.file_extension}"
                #"libReplicantDriveSim.1.0.0.dylib"
                #"replicantdrivesim.so"
                "replicantdrivesim_pypi.so"
                #"replicantdrivesim.cpython-38-darwin.so"
