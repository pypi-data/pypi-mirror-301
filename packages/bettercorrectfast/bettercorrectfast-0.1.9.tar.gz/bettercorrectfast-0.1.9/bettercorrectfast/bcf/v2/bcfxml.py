...
from ..agnostic import extensions as ag_ex
from . import model as mdl
from .model import version as ve
from .model import project as pr
from .model import markup as ma
from .model import extensions as mdl_extensions
from ..inmemory_zipfile import InMemoryZipFile, ZipFileInterface
...
class BcfXml:
    ...
    def __init__(
        ...
        self._version: Optional[ve.Version] = None
        self._project_info: Optional[pr.ProjectExtension] = None
    ...
    @property
    def version(self) -> ve.Version:
        """Bcf Version."""
        if not self._version:
            self._version = (
                self._xml_handler.parse(self._zip_file.read("bcf.version"), ve.Version)
                if self._zip_file
                else ve.Version(version_id="2.1")
            )
        return self._version

    @version.setter
    def version(self, value: ve.Version) -> None:
        self._version = value

    @property
    def project_info(self) -> Optional[pr.ProjectExtension]:
        """BCF project information."""
        if not self._project_info and self._zip_file and zipfile.Path(self._zip_file, "project.bcfp").exists():
            self._project_info = self._xml_handler.parse(self._zip_file.read("project.bcfp"), pr.ProjectExtension)
        return self._project_info

    @project_info.setter
    def project_info(self, value: Optional[pr.ProjectExtension]) -> None:
        self._project_info = value

    @property
    def project(self) -> Optional[pr.Project]:
        """BCF project."""
    ...
    def extensions(self) -> Optional[mdl_extensions.Extensions]:
        ...
        if not self._extensions and self.extension_schema:
            ...
            attrs = ag_ex.get_extensions_attributes(extensions) # Slightly altered
    ...
    def create_new(
        ...
        instance.project_info = pr.ProjectExtension(
            project=pr.Project(name=project_name, project_id=str(uuid.uuid4())), extension_schema=""
        )
        return instance
    ...
    def get_project(self, _filepath: Optional[str] = None) -> Optional[pr.Project]:
        """Deprecated method."""
    ...
    def get_header(self, guid: str) -> Optional[ma.Header]:
    ...
    def add_comment(self, _topic: ma.Topic, _comment: Optional[ma.Comment] = None) -> None:
        """Deprecated method."""
