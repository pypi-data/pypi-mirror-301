...
class TraverseDatasetItems:
    ...
    def item_processor(**kwargs) -> Optional[Any]:
        ...
        默认打印 file_abs_path 和 raw_item_data，并直接返回 raw_item_data。
    ...
    def traverse_dataset(self, traverse_dir: str, item_file_type: str) -> List[tuple]:
        """
        ...
        for root, dirs, files in os.walk(traverse_dir):
            for file in files:
                ...
                raw_item_data = self.item_reader(file_abs_path)

                processed_item_data = self.item_processor(traverse_dir=traverse_dir,
                                                          file_abs_path=file_abs_path,
                                                          raw_item_data=raw_item_data)

                if self.item_filter(traverse_dir=traverse_dir, file_abs_path=file_abs_path,
