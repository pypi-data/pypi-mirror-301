...
class DatasetTimeframeAggregator:
    ...
    def _aggregate_candles(self, end_time):
        ...
        aggregated_time = datetime.utcfromtimestamp(end_time.timestamp()).strftime('%Y-%m-%d %H:%M:%S')
    ...
    def aggregate(self, new_candle):
        ...
        while candle_time >= self.start_time + timedelta(seconds=self.aggregation_window_sec):
