    ...
    def load_database(self):
        ...
        if isinstance(self.dataset_name, str):
            ...
            for id, recording_name in enumerate(tqdm(self.recNames, desc = self.dataset_name)):
                ...
                if ana.iloc[0,1] != 0:
                    print(f'Warning. {recording} - Analysis starts at {ana.iloc[0,1]} instead of 0. Check again.')
                if ana.iloc[-1,1] != len(recording)//100:
                    print(f'Warning. {recording} - Analysis ends at {ana.iloc[-1,1]} instead of 0. Check again.')
