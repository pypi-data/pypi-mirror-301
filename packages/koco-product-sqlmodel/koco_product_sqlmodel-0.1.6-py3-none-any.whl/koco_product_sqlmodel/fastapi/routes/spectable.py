...
class SpecTableRoute(rgen.ParentRoute):
    ...
    async def post_object(self, object: sqlm.CSpecTablePost, request: Request) -> sqlm.SQLModel:
    ...
    async def patch_object(self, id: int, obj: sqlm.CSpecTablePost, request: Request) -> sqlm.SQLModel:
...
class SpecTableItemRoute(SpecTableRoute):
    ...
    async def post_object(self, object: sqlm.CSpecTableItemPost, request: Request) -> sqlm.SQLModel:
        object.user_id = await rsec.get_user_id_from_request(request=request)
        new_obj = mdb_gen.post_object(db_obj=self.sqlmodel_db(**object.model_dump()))
        return self.sqlmodel_get(**new_obj.model_dump())

    async def patch_object(self, id: int, obj: sqlm.CSpecTableItemPost, request: Request) -> sqlm.SQLModel:
        obj.user_id = await rsec.get_user_id_from_request(request=request)
        updated_object = mdb_gen.patch_object(
            id=id, db_obj=obj, db_obj_type=self.sqlmodel_db
        )
