...
class NacelleSensors(BladedModel):
    r"""
    ...
    """
    _relative_schema_path: str = PrivateAttr('Components/DrivetrainAndNacelle/NacelleSensors/NacelleSensors.json')
    ...
    def items(self) -> list[tuple[str, NacelleSensor]]:
        """
    ...
    def values(self) -> list[NacelleSensor]:
        """
    ...
    def __setitem__(self, key, value):
        ...
        if key in self.__fields__:
            raise KeyError(f"Item accessors can only be used for custom entries. '{key}' is a pre-defined model property.")
        if not isinstance(value, NacelleSensor):
            raise TypeError(f"Entries must be of type 'NacelleSensor'; received '{type(value).__name__}'")
    ...
    def __setattr__(self, name, value):
        if not name in self.__fields__ and not isinstance(value, NacelleSensor):
            raise TypeError(f"Entries must be of type 'NacelleSensor'; received '{type(value).__name__}'")
        super().__setattr__(name, value)

    class __CustomEntries__(BaseModel):
        entries: Dict[str, NacelleSensor] = Field(default={})
    ...
    def _factory(cls, data):
        if isinstance(data, NacelleSensors):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define NacelleSensors models')
        return custom_entries_parser(cls, data, dict, NacelleSensors.__CustomEntries__)

    @classmethod
    def parse_obj(cls: Type['Model'], obj: Any) -> 'Model':
        return custom_entries_parser(cls, obj, dict, NacelleSensors.__CustomEntries__)
    ...
    def from_file(cls: Type['Model'], path: Union[str, Path]) -> 'Model':
        ...
        Examples
        --------
        >>> model = NacelleSensors.from_file('/path/to/file')
        """
    ...
    def from_json(cls: Type['Model'], b: StrBytes) -> 'Model':
        ...
        Examples
        --------
        >>> model = NacelleSensors.from_json('{ ... }')
        """
...
NacelleSensors.update_forward_refs()
NacelleSensors.__CustomEntries__.update_forward_refs() 
