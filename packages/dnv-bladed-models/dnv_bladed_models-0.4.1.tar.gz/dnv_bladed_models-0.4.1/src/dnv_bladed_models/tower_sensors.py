...
class TowerSensors(BladedModel):
    r"""
    ...
    """
    _relative_schema_path: str = PrivateAttr('Components/Tower/TowerSensors/TowerSensors.json')
    ...
    def items(self) -> list[tuple[str, TowerSensor]]:
        """
    ...
    def values(self) -> list[TowerSensor]:
        """
    ...
    def __setitem__(self, key, value):
        ...
        if key in self.__fields__:
            raise KeyError(f"Item accessors can only be used for custom entries. '{key}' is a pre-defined model property.")
        if not isinstance(value, TowerSensor):
            raise TypeError(f"Entries must be of type 'TowerSensor'; received '{type(value).__name__}'")
    ...
    def __setattr__(self, name, value):
        if not name in self.__fields__ and not isinstance(value, TowerSensor):
            raise TypeError(f"Entries must be of type 'TowerSensor'; received '{type(value).__name__}'")
        super().__setattr__(name, value)

    class __CustomEntries__(BaseModel):
        entries: Dict[str, TowerSensor] = Field(default={})
    ...
    def _factory(cls, data):
        if isinstance(data, TowerSensors):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define TowerSensors models')
        return custom_entries_parser(cls, data, dict, TowerSensors.__CustomEntries__)

    @classmethod
    def parse_obj(cls: Type['Model'], obj: Any) -> 'Model':
        return custom_entries_parser(cls, obj, dict, TowerSensors.__CustomEntries__)
    ...
    def from_file(cls: Type['Model'], path: Union[str, Path]) -> 'Model':
        ...
        Examples
        --------
        >>> model = TowerSensors.from_file('/path/to/file')
        """
...
TowerSensors.update_forward_refs()
TowerSensors.__CustomEntries__.update_forward_refs() 
