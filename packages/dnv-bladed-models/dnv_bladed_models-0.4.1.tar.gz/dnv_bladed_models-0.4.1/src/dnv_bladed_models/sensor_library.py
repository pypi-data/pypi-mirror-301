...
class SensorLibrary(BladedModel):
    r"""
    ...
    """
    _relative_schema_path: str = PrivateAttr('Turbine/BladedControl/SensorsUsed/SensorLibrary/SensorLibrary.json')
    ...
    def items(self) -> list[tuple[str, SensorReference]]:
        """
    ...
    def values(self) -> list[SensorReference]:
        """
    ...
    def __setitem__(self, key, value):
        ...
        if key in self.__fields__:
            raise KeyError(f"Item accessors can only be used for custom entries. '{key}' is a pre-defined model property.")
        if not isinstance(value, SensorReference):
            raise TypeError(f"Entries must be of type 'SensorReference'; received '{type(value).__name__}'")
    ...
    def __setattr__(self, name, value):
        if not name in self.__fields__ and not isinstance(value, SensorReference):
            raise TypeError(f"Entries must be of type 'SensorReference'; received '{type(value).__name__}'")
        super().__setattr__(name, value)

    class __CustomEntries__(BaseModel):
        entries: Dict[str, SensorReference] = Field(default={})
    ...
    def _factory(cls, data):
        if isinstance(data, SensorLibrary):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define SensorLibrary models')
        return custom_entries_parser(cls, data, dict, SensorLibrary.__CustomEntries__)

    @classmethod
    def parse_obj(cls: Type['Model'], obj: Any) -> 'Model':
        return custom_entries_parser(cls, obj, dict, SensorLibrary.__CustomEntries__)
    ...
    def from_file(cls: Type['Model'], path: Union[str, Path]) -> 'Model':
        ...
        Examples
        --------
        >>> model = SensorLibrary.from_file('/path/to/file')
        """
...
SensorLibrary.update_forward_refs()
SensorLibrary.__CustomEntries__.update_forward_refs() 
