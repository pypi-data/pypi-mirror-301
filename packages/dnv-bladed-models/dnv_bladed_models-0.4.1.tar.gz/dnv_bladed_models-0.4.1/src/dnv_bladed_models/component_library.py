...
class ComponentLibrary(BladedModel):
    ...
    -----
    """
    _relative_schema_path: str = PrivateAttr('ComponentLibrary/ComponentLibrary.json')
    ...
    def __setitem__(self, key, value):
        ...
        if not isinstance(value, Component):
            raise TypeError(f"Entries must be of type 'Component'; received '{type(value).__name__}'")
    ...
    def __setattr__(self, name, value):
        if not name in self.__fields__ and not isinstance(value, Component):
            raise TypeError(f"Entries must be of type 'Component'; received '{type(value).__name__}'")
        super().__setattr__(name, value)

    class __CustomEntries__(BaseModel):
        entries: Dict[str, Component] = Field(default={})
    ...
    def _factory(cls, data):
        ...
        return custom_entries_parser(cls, data, dict, ComponentLibrary.__CustomEntries__)

    @classmethod
    def parse_obj(cls: Type['Model'], obj: Any) -> 'Model':
        return custom_entries_parser(cls, obj, dict, ComponentLibrary.__CustomEntries__)
