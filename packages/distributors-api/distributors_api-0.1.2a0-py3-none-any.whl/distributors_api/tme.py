...
def decode_product_dictionary(product):
    ...
    response = {'Links': links, 'Description': description, 'Symbol': symbol, 'OrderInfo': order_info,
                ...
                'Distributor Order Number': symbol['SymbolTME']}
    return response
...
class TME(DistributorConnectorBase):
    ...
    def get_component_impl(self, distributor_order_number):
        ...
        """
        assert len(distributor_order_number) <= self.max_part_count_per_request
        results = self.get_products(distributor_order_number)
        response = []
        for result in results:
            response.append({'Distributor Order Number': result['Symbol']['SymbolTME'],
                             'DistributorComponentWebPage': result['Links']['ProductInformationPage'],
                             "Manufacturer": result['Parameters']['Manufacturer'],
                             'Manufacturer Part Number': result['Parameters']['Manufacturer Part Number']})
        return response

    def get_stock_and_prices(self, distributor_order_number_set):
        response = self.tme.get_prices_and_stocks(distributor_order_number_set)
        if response:
            stock_and_price = []
            for product in response['ProductList']:
                ...
                for price in product['PriceList']:
                    price_list.append({'amount': price['Amount'],
                                       'price': price['PriceValue'],
                                       'currency': payment_info['Currency'],
                                       'vatRate': payment_info['VatRate'],
                                       'priceType': payment_info['PriceType']})
    ...
    def get_components_don_from_category(self, category):
        if category == "Resistors":
            ...
            tme_symbol_list = self.tme.get_symbols(resistors_category_id)
            return tme_symbol_list
    ...
    def search(self, components):
        response = self.tme.search(components, on_stock=self.require_on_stock)
        if response and len(response) != 0:
            products = []
