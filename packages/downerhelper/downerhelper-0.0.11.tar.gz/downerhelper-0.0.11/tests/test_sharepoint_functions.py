...
def test_get_sharepoint_list(site_address, queue, keyvault_url):
    sp_list = get_sharepoint_list(site_address, list_name_1, 0, keyvault_url, queue)
    expected = [
        {
            "Title": "test0",
            "Date": "2024-09-13",
    ...
    with pytest.raises(Exception):
        get_sharepoint_list(site_address + "1", list_name_1, 0, keyvault_url, queue)

    with pytest.raises(Exception):
        get_sharepoint_list(site_address, list_name_1 + "1", 0, keyvault_url, queue)
...
def test_form_sharepoint_item(timestamp_data, queue):
    ...
    item = form_sharepoint_list_item(attributes, title, pairs, queue)
    ...
    item = form_sharepoint_list_item(attributes, None, pairs, queue)
    ...
    with pytest.raises(Exception):
        form_sharepoint_list_item(attributes, title, None, queue)
...
def test_create_sharepoint_list_item(queue, site_address, keyvault_url):
    sp_list_old = get_sharepoint_list(site_address, list_name_2, 0, keyvault_url, queue)
    ...
    item = form_sharepoint_list_item(attributes, title, pairs, queue)
    item_id = create_sharepoint_list_item(item, site_address, list_name_2, key_url, queue)
    assert int(item_id) == len(sp_list_old) + 1

    sp_list_new = get_sharepoint_list(site_address, list_name_2, 0, keyvault_url, queue)
    ...
    with pytest.raises(Exception):
        create_sharepoint_list_item(item, site_address + "null", list_name_2, key_url, queue)

    item['nonenone'] = 'bad'
    with pytest.raises(Exception):
        create_sharepoint_list_item(item, site_address, list_name_2, key_url, queue)

def test_create_sharepoint_list_attachment(queue, site_address, qgis_creds, feature_service, keyvault_url):
    ...
    item_id = create_sharepoint_list_item(item, site_address, list_name_2, item_key_url, queue)
    assert item_id != None
    ...
    attach_key_url = get_key_url('create-sharepoint-list-item-attachment', keyvault_url, queue)
    ...
    for group in attachment_data['attachmentGroups']:
        for info in group['attachmentInfos']:
            ...
            assert create_sharepoint_list_attachment(item_id, attachment, info['name'], site_address, list_name_2, attach_key_url, queue)
            expected.append({
                'name': info['name'],
                'content': base64.b64encode(attachment).decode('utf-8')
            })

    sp_attachments = get_sharepoint_list_attachments(item_id, site_address, list_name_2, retrieve_key_url, queue)
