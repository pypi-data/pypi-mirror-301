...
from btpy.core.azure_utility import get_blob_container_client, get_blob_client, to_blob_index_format
...
async def find_migrations(from_version: str, to_version: str):
    service_version_changes = _merge_service_versions(await get_service_versions(from_version),
                                                      await get_service_versions(to_version))
    migrations = []
    migration_ids = set()
    ...
    for service, versions in service_version_changes.items():
        ...
        # be `None`.
        service_migrations = await find_service_migrations(all_migrations, service, versions[0], versions[1])
...
async def find_service_migrations(all_migrations: [dict], service: str, from_version: str, to_version: str):
    if from_version is None or to_version is None:
        return []
    if from_version > to_version:
        raise Exception(f"from_version must not be higher than to_version ({from_version}, {to_version})")
    if from_version == to_version:
        return []
...
async def _load_all_migrations():
    ...
    for migration_blob_name in migration_blob_name_it:
        if "/" not in migration_blob_name:
            migrations.append(await get_migration(_without_file_ext(migration_blob_name)))

    return migrations


def _matches_service_version(migration: dict, service: str, min_version: str, max_version: str):
...
def _merge_service_versions(from_versions: dict, to_versions: dict):
    ...
    for service_name, version in to_versions.items():
        ...
        service_version_changes[service_name] = (service_version_changes[service_name][0], version)
