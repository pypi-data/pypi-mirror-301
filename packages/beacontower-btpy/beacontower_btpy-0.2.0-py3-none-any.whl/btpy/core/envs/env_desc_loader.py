...
from ..azure_utility import get_blob_container_client, get_blob_client
...
async def get_service_versions(version: str):
    version_blob = get_blob_client(VERSION_CONTAINER_NAME, f"{SERVICE_VERSIONS_FOLDER_NAME}/{version}.yaml")
...
def _load_desc_template(version: str):
    with get_blob_container_client(VERSION_CONTAINER_NAME) as container_client:
        blob_client = container_client.get_blob_client(f"{DESC_TEMPLATES_FOLDER_NAME}/{version}.yaml")
...
def _extract_version(env_blob_data: dict):
    if BT_VERSION_KEY not in env_blob_data:
        raise Exception(f"'bt_version' not found in environment data")
