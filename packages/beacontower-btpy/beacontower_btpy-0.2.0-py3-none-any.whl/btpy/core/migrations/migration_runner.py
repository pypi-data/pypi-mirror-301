...
async def _download_migration_files(migration: dict, temp_dir: str):
    ...
    try:
        migration_file_blob_it = get_blob_container_client(MIGRATIONS_CONTAINER_NAME).list_blobs(
            name_starts_with=f"{migration['id']}/")
    
        for migration_file_blob_entry in migration_file_blob_it:
            migration_file_blob = get_blob_client(MIGRATIONS_CONTAINER_NAME, migration_file_blob_entry.name)
            migration_file_content = migration_file_blob.download_blob().readall()
            migration_file_path = os.path.join(temp_dir, os.path.basename(migration_file_blob_entry.name))
...
def _execute_migration_python_files(temp_dir: str):
    ...
    result = subprocess.run(["python", entry_point_file], capture_output=True, text=True)
