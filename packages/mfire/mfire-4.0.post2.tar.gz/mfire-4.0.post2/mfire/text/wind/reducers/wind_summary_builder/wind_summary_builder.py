...
from mfire.text.wind.reducers.utils import add_previous_time_in_dataset
from mfire.text.wind.reducers.wind_summary_builder.helpers import (
    WindSummaryBuilderMixin,
)
from mfire.text.wind.reducers.wind_summary_builder.utils import (
    get_valid_time_of_wind_type,
)
from mfire.text.wind.reducers.wind_summary_builder.wind_intensity import WindIntensity
...
# Logging
LOGGER = get_logger(name=__name__, bind="wind_summary_builder")
...
class WindSummaryBuilder(WindSummaryBuilderMixin, BaseParamSummaryBuilder):
    ...
    def _export_dataset_attrs_with_extra_info(self) -> str:
        """Export dataset attributes and extra info like case nbr as string.
        ...
        output: OrderedDict = self._get_sorted_dataset_attrs()
        return ", ".join(list(f"{k}: {v}" for k, v in output.items()))
    ...
    def compute(self, reference_datetime: Datetime) -> dict:
        ...
        self.dataset.attrs[self.SELECTOR_KEY] = self.case
    ...
    def _generate_summary(self, reference_datetime: Datetime) -> None:
        ...
        # Add extra content: useful for understand output
        add_extra_content_to_summary(
            self._summary, self._export_dataset_attrs_with_extra_info()
        )
