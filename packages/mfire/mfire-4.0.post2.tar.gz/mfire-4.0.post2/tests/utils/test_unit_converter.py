...
class TestUnitConverterFunctions:
    ...
    @pytest.mark.parametrize(
        "input,output_units,context,expected",
        [
            ((1.0, "m"), "cm", None, 100.0),
    ...
    def test_unit_conversion(self, input, output_units, context, expected):
        result = unit_conversion(input, output_units, context=context)
        assert_identically_close(result, expected)
