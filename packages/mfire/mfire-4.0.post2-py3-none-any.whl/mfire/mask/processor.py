...
class GridProcessor(BaseModel):
    ...
    def _compute_axis_altitude(self, data_array: xr.DataArray) -> xr.DataArray:
        ...
        This function identifies "Axis" type areas in `self.areas` and calculates
        minimum, maximum altitude, and the number of data points within those areas.
        It then creates new DataArrays representing regions below and above specified
        altitude thresholds (`self.MIN_ALT` and `self.MAX_ALT`) meeting a data point
        percentage criteria (`self.PERCENT`). These new DataArrays are concatenated
        along the "id" dimension with the original `data_array`.
        ...
        for area_id, area_info in self.areas.items():
            ...
            area_da = data_array.sel(id=area_id)
            combined_data = area_da * altitude_da

            # Calculate minimum, maximum altitude, and total data points
            min_alt = combined_data.min().values
            max_alt = combined_data.max().values
            nbr_pts = int(combined_data.sum())
            ...
            for altitude in [alt for alt in self.MIN_ALT if min_alt <= alt <= max_alt]:
                ...
                data_below = ((combined_data < altitude) * area_da).mask.bool
                nbr_pts_below = data_below.count().item()

                # Check if data point percentage meets the criteria
                if (
                    1 - self.PERCENT / 100
                    > nbr_pts_below / nbr_pts
                    > self.PERCENT / 100
                ):
                    # Create a new DataArray for the region below the threshold
                    data_below = data_below.expand_dims(dim="id").assign_coords(
                        id=[f"{area_id}_alt__inf_{altitude}"]
                    )
                    data_below["areaName"] = data_below["altAreaName"] = (
                        ["id"],
                        [_("en dessous de {altitude} m").format(altitude=altitude)],
                    )
                    data_below["areaType"] = (["id"], ["Altitude"])
                    temp_da.append(data_below)

            # Similar loop for maximum altitude thresholds (above)
            for altitude in [alt for alt in self.MAX_ALT if min_alt <= alt <= max_alt]:
                data_above = ((combined_data > altitude) * area_da).mask.bool
                nbr_pts_above = data_above.count().item()
                if (
                    1 - self.PERCENT / 100
                    > nbr_pts_above / nbr_pts
                    > self.PERCENT / 100
                ):
                    data_above = data_above.expand_dims(dim="id").assign_coords(
                        id=[f"{area_id}_alt__sup_{altitude}"]
                    )
                    data_above["areaName"] = data_above["altAreaName"] = (
                        ["id"],
                        [_("au-dessus de {altitude} m").format(altitude=altitude)],
                    )
                    data_above["areaType"] = (["id"], ["Altitude"])
                    temp_da.append(data_above)
