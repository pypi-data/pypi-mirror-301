                ...
                if self.has_ignored_values:
                    if len(brackets) == 1:
                        if rounded_min == rounded_max:
                            overall_term = self.CLOSE_TO
                        else:
                            overall_term = self.AROUND_TERM
                    else:
                        overall_term = self.OVERALL_TERM
                else:
                    # "de 2 à 4 °C sur xxx, jusqu'à 8 à 10 dans le YYY"
                    # de 1 à YYY, jusqu'à 12 sur le XXX"
                    # "de 5 à 8 °C dans le ZZZ"
                    if len(brackets) == 2 or rounded_min != rounded_max:
                        overall_term = self.FROM_TERM
                    else:
                        # only one interval and only one value in it :
                        # "1°C dans le ZZZ"
                        overall_term = self.NO_TERM
