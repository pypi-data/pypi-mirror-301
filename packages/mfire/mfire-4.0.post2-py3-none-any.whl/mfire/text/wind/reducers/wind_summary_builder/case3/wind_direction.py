...
def set_and_check_class_nbr(shift: float) -> float:
    if 360.0 % shift != 0.0:
...
class PcdFinder:
    ...
    WIND_DIRECTION_CLASS: WindDirection = WindDirection120
        ...
        for class_nbr in range(cls.CLASSES_NBR):
            ...
            if class_cur[1] < cls.CLASS_SIZE:
                ...
                cnt = np.count_nonzero(
                    (value >= class_cur[0]) & (value <= class_cur[1])
                )
    ...
    def _post_process_found_periods(self) -> None:
        ...
        if pcd_len == 0:
            ...
            # If the PCD is < 3h, then keep not it
            if not self._pcd_filtered and not self._check_pcd_duration(self._pcd[0]):
                self._reset_pcd()
            # If the PCD does not covered enough the monitoring period, then keep not it
            elif self._check_pcd_coverage() is False:
                self._reset_pcd()
