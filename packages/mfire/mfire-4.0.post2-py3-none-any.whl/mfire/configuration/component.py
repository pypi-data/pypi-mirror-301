...
class AbstractComponent(BaseModel):
    ...
    def box(self) -> Tuple:
        ...
        mesh_size = 0.26  # TODO: Retrieve mesh size dynamically based on configuration
        ...
        for best_files in files_groups:
            ...
            for file_id, file_start, file_stop in best_files:
                ...
                for param in self.all_parameters:
                    if (file_id, param) not in self.data_config:
                        # Preprocess data if it doesn't exist
                        self.data_config[
                            (file_id, param)
                        ] = self.rules.resource_handler(file_id, None, param)
            ...
            for param in self.all_parameters:
                ...
                new_risk["params"][param] = FieldComposite(
                    ...
                    name=param,
                    selection=self.selection((slice_start, slice_stop)),
                )

            # Create and append RiskComponentComposite object
            result.append(RiskComponentComposite(**new_risk))
        return result
    ...
    def process_weather(self, files_groups: dict, weather: dict) -> SynthesisComposite:
        ...
        for best_files in files_groups.keys():
            for file_id, file_start, file_stop in best_files:
                ...
                # Preprocess relative humidity data for each weather parameter
                for param in self.all_parameters:
