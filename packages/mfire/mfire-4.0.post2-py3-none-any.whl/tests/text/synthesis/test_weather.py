...
from tests.text.synthesis.factories import WeatherBuilderFactory
...
class TestWeatherReducer:
    ...
    def _compute(
        ...
        **kwargs,
        ...
        self.reducer = WeatherReducer(geo_id="id_axis", composite=composite)
        self.reducer.densities["DT"]["required"] = defaultdict(lambda: requiredDT)
        self.reducer.densities["DHmax"]["required"] = defaultdict(lambda: requiredDHmax)
        return self.reducer.compute()
    ...
    def test_process_lpn(self, code, lpn, has_snow_risk, assert_equals_result):
        np.random.seed(0)
        assert_equals_result(
            {
                language: self._compute(
...
class TestWeatherBuilder:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ([{"key": "1xTS", "lab": "Label", "loc": "Loc"}], "Label Loc."),
            (
                [{"key": "1xTS_temp", "lab": "Label", "temp": "Temp", "loc": "Loc"}],
                "Temp, Label Loc.",
            ),
        ],
    )
    ...
    def _compute(
        ...
        **kwargs,
        ...
        builder = WeatherBuilder(geo_id="id_axis", composite=composite)
