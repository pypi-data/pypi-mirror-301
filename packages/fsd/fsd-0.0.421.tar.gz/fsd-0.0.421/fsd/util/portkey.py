...
class BedrockModel(BaseModel):
    ...
    def generate_image(self, prompt: str, size: str = "1024x1024"):
        logger.info("Using Stable Diffusion for image generation")
...
class LlamaModel(BaseModel):
    async def prompt(self, messages: List[Dict[str, str]], max_tokens: int, temperature: float, top_p: float) -> Dict:
        logger.info("Using LlamaModel for prompt")
        common_params = {
            "messages": messages,
            "temperature": 0.2,
            "top_p": 0.1,
    ...
    async def stream_prompt(self, messages: List[Dict[str, str]], max_tokens: int, temperature: float, top_p: float):
        logger.info("Using LlamaModel for stream_prompt")
        common_params = {
            "messages": messages,
            "max_tokens": 2048,
            "temperature": 0.2,
...
class CodingLlamaModel(BaseModel):
    async def prompt(self, messages: List[Dict[str, str]], max_tokens: int, temperature: float, top_p: float) -> Dict:
        logger.info("Using CodingLlamaModel for prompt")
        common_params = {
            "messages": messages,
            "temperature": 0.2,
            "top_p": 0.1,
    ...
    async def stream_prompt(self, messages: List[Dict[str, str]], max_tokens: int, temperature: float, top_p: float):
        logger.info("Using CodingLlamaModel for stream_prompt")
        common_params = {
            "messages": messages,
            "max_tokens": 2048,
            "temperature": 0.2,
...
class AIGateway:
    ...
    async def coding_prompt(self, 
        ...
        logger.error("All coding models failed to respond")
        raise Exception("All coding models failed to respond")
    ...
    def generate_image(self, prompt: str, size: str = "1024x1024"):
        ...
        model_type = self._select_model(self.IMAGE_MODEL_WEIGHTS)
        ...
        try:
            if model_type == "dalle3":
                ...
                image = self.bedrock_model.generate_image(prompt, size)
            logger.info(f"Successfully generated image with {model_type} model")
            return image
        except Exception as e:
