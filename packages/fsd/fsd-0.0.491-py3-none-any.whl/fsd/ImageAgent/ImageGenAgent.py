...
class ImageGenAgent:
    ...
    async def process_image_generation(self, steps: List[Dict]):
        """
        Processes image generation steps concurrently, with a maximum of 3 concurrent generations.
        """
        semaphore = asyncio.Semaphore(3)  # Limit to 3 concurrent tasks

        async def generate_with_semaphore(step):
        ...
        tasks = [generate_with_semaphore(step) for step in steps]
