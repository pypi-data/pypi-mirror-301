...
class ControllerAgent:
    ...
    async def get_pro_coding_requests(self, instructions, context, file_lists, context_files, role, crawl_logs, original_prompt_language):
        ...
        async def process_task(task, coding_agent):
            ...
            if self.is_coding_file(file_name):
                ...
                if main_path:
                    ...
                    try:
                        result = await coding_agent.get_coding_request(file_name, techStack, prompt)
        ...
        for group in plan.get('groups', []):
            ...
            coding_agents = [CodingAgent(self.repo) for _ in tasks]
            
            # Initialize all agents in the group
            for agent, task in zip(coding_agents, tasks):
                agent.initial_setup(context_files, instructions, context, crawl_logs)

            # Execute all tasks in the group concurrently
            await asyncio.gather(*[process_task(task, agent) for task, agent in zip(tasks, coding_agents)], return_exceptions=True)

            # Clean up the agents after the group is done
