...
class ControllerAgent:
    ...
    async def get_pro_coding_requests(
        ...
        logger.info("-------------------------------------------------")
        logger.info("#### The `Coding Agent Pro` is starting the coding phase in supercharged mode.")
        ...
        for group in plan.get('groups', []):
            ...
            for agent in coding_agents:
                ...
                # Each agent gets a deep copy of the shared conversation history
                agent.conversation_history = copy.deepcopy(shared_conversation_history)
            ...
            async def process_task(task, coding_agent):
                ...
                if self.is_coding_file(full_file):
                    ...
                    if main_path:
                        ...
                        logger.info(
                            f"#### The `{techStack.upper()} Agent` is processing file: `{os.path.basename(full_file)}`."
                        )

                        try:
                            # Call get_coding_request, which handles building the user prompt and updating the conversation history
                            ...
                            logger.info(
                                f"\n #### `{techStack.upper()} Agent` finished successfully: `{os.path.basename(full_file)}`."
                            )
            ...
            # Collect conversation histories from agents
            agent_histories = [agent.conversation_history for agent in coding_agents]

            # Combine conversation histories
            shared_conversation_history = self.combine_conversation_histories(shared_conversation_history, agent_histories)

            # Optionally, prune or summarize the conversation history here if necessary
    ...
    # Helper functions
    def combine_conversation_histories(self, shared_history, agent_histories):
        # Combine shared history with new histories from agents
        combined_history = copy.deepcopy(shared_history)
        for history in agent_histories:
            # Skip initial setup messages to avoid duplication
            agent_new_messages = history[len(shared_history):]
            combined_history.extend(agent_new_messages)
        # Remove duplicates if necessary
        combined_history = self.remove_duplicate_messages(combined_history)
        return combined_history

    def remove_duplicate_messages(self, conversation_history):
        seen = set()
        unique_history = []
        for message in conversation_history:
            message_tuple = (message['role'], message['content'])
            if message_tuple not in seen:
                seen.add(message_tuple)
