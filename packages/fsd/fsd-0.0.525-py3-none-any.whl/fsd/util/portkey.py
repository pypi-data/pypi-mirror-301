...
class BedrockModel(BaseModel):
    ...
    async def generate_image(self, prompt: str, size: str = "1024x1024", model: str = "stability.stable-diffusion-xl-v1"):
        logger.debug(f"Using {model} for image generation")
        return await asyncio.to_thread(self.portkey.images.generate,
            prompt=prompt,
            model=model
        )
...
class DalleModel(BaseModel):
    async def generate_image(self, prompt: str, size: str = "1024x1024"):
        ...
        return await asyncio.to_thread(self.portkey.images.generate, prompt=prompt, size=size)
...
class AIGateway:
    ...
    async def coding_prompt(self, 
        ...
        logger.debug("All coding models failed to respond")
