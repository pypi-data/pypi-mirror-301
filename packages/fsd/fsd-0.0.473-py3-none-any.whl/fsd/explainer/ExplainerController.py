...
class ExplainerController:
    ...
    async def get_normal_answer(self, user_prompt, language, role,file_attachments, focused_files, crawl_logs):
        """Generate idea plans based on user prompt and available files."""
        return await self.normalExplainer.get_normal_answer_plans(self.conversation_history, user_prompt, language, role, file_attachments, focused_files, crawl_logs)

    async def get_file_answer(self, user_prompt, language, files, role, file_attachments, focused_files, crawl_logs):
        """Generate idea plans based on user prompt and available files."""
        return await self.mainExplainer.get_answer_plans(self.conversation_history, user_prompt, language, files, role, file_attachments, focused_files, crawl_logs)
    ...
    async def get_started(self, user_prompt, file_attachments, focused_files):
        ...
        if pipeline == "1" or pipeline == 1:
            ...
            self.conversation_history = await self.get_file_answer(finalPrompt, language, working_files, role, file_attachments, focused_files, crawl_logs)
            ...
            self.conversation_history = await self.get_normal_answer(finalPrompt, language, role, file_attachments, focused_files, crawl_logs)
