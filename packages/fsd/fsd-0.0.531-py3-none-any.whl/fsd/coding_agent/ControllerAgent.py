        ...
        async def process_task(task):
            ...
            if self.is_coding_file(file_name):
                ...
                if main_path:
                    ...
                    try:
                        # Run synchronous functions in a thread to prevent blocking the event loop
                        result = await asyncio.to_thread(self.coder.get_coding_requests, file_name, techStack, prompt)
                        await asyncio.to_thread(self.code_manager.handle_coding_agent_response, main_path, result)
