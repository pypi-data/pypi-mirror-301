...
class BedrockModel(BaseModel):
    ...
    def generate_image(self, prompt: str, size: str = "1024x1024"):
        logger.debug("Using Stable Diffusion for image generation")
...
class CodingModel(BaseModel):
    async def prompt(self, messages: List[Dict[str, str]], max_tokens: int, temperature: float, top_p: float) -> Dict:
        logger.debug("Using CodingModel for prompt")
        common_params = {
            "messages": messages,
            "max_tokens": 4096,
            "temperature": 0.2,
    ...
    async def stream_prompt(self, messages: List[Dict[str, str]], max_tokens: int, temperature: float, top_p: float):
        logger.debug("Using CodingModel for stream_prompt")
        common_params = {
            "messages": messages,
            "max_tokens": 4096,
            "temperature": 0.2,
...
class AIGateway:
    ...
    def __new__(cls):
        if cls._instance is None:
            ...
            cls._instance.coding_model = CodingModel(cls.API_KEY, cls.VIRTUAL_KEYS["bedrock"], cls.CONFIG_ID)
