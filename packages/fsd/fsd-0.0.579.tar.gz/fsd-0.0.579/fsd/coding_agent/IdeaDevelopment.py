    ...
    def initial_setup(self, role, crawl_logs, context):
        ...
        if context:
            ...
            if all_working_files_contents:
                self.conversation_history.append({"role": "user", "content": f"This is data for potential existing files we may need to modify or update. Use it accordingly if appropriate, otherwise you can ignore it:\n{all_working_files_contents}"})
            ...
            if all_context_files_contents:
                self.conversation_history.append({"role": "user", "content": f"This is context for potential existing files we need to read to provide context to support the task. Do not modify these files:\n{all_context_files_contents}"})
                self.conversation_history.append({"role": "assistant", "content": "Understood."})
