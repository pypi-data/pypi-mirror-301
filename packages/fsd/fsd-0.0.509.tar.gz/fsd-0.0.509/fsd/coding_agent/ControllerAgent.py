...
class ControllerAgent:
    ...
    async def get_pro_coding_requests(self, instructions, context, file_lists, context_files, role, crawl_logs, original_prompt_language):
        ...
        async def process_task(task):
            ...
            if self.is_coding_file(file_name):
                ...
                if main_path:
                    ...
                    try:
                        # Since get_coding_request is async, we await it directly
                        result = await self.coder.get_coding_request(file_name, techStack, prompt)

                        # Check if handle_coding_agent_response is async or not
                        # If it's async, await it directly
                        # If it's synchronous and blocking, consider wrapping it with asyncio.to_thread

                        # Assuming handle_coding_agent_response is async
        ...
        # Process groups sequentially, but all tasks within each group concurrently
        for group in plan.get('groups', []):
            ...
            await asyncio.gather(*[process_task(task) for task in tasks], return_exceptions=True)
