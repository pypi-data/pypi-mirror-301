...
async def login(login_data: LoginData):
    ...
    print(user_data)
    if not user_data:
        return {"status": "failed", "permission": 1}
    if login_data.token:
    ...
    return {"status": "failed", "permission": 1, "token": ""}

async def run_server():
    config = uvicorn.Config(app, host="0.0.0.0", port=9000, loop="asyncio")
...
def start_server():
    ...
    server_process = subprocess.Popen(
        ["python", "-c", 
         "from dxr_cli.cli import app; import uvicorn; uvicorn.run(app, host='0.0.0.0', port=9000, log_config={'version': 1, 'formatters': {'default': {'format': '%(asctime)s - %(levelname)s - %(message)s', 'datefmt': '%Y-%m-%d %H:%M:%S'}}, 'handlers': {'default': {'class': 'logging.FileHandler', 'filename': '/tmp/fastapi_server.log', 'formatter': 'default'}}, 'root': {'level': 'INFO', 'handlers': ['default']}})"
        ],
...
dxr.add_command(stop_server)  # 添加新命令
