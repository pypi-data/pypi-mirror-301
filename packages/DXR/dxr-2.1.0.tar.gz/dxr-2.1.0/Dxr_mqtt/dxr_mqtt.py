...
def setServerUrl(url='127.0.0.1', port=1883, clientID=str(int(time.time() * 1000)), user=None, password=None, token=None):
    global server_url, server_port, client_id, can_publish
    server_url = url
    ...
    if user is not None and password is not None:
        # 使用用户名和密码请求登录接口
        login_url = f"http://{url}:9000/login"
...
class Dxr_Publisher:
    ...
    def publish(self, msg):
        global mqttc, is_use_crypto, can_publish
    ...
    def await_publish(self, msg, timeout=None, topic=None):
        global callback_dit, mqttc, is_use_crypto, can_publish
        if not can_publish:
            return
        if mqttc is None:
            mqttc = Mqtt()
    ...
    def stop_awit_publish(self, topic=None):
        global callback_dit, mqttc, can_publish
        if not can_publish:
            return
        if topic is None:
            await_topic = self.topic + '_response'
