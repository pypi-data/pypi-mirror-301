...
class Final:
    """
    \\@since 0.3.26b3 (experimental; to 0.3.27b3 `FinalClass`)
    ...
    Usage:
    ```py \\
    class FinalClass(Final, final = True): ...
    ...
    def __init_subclass__(cls, /, *args, **kwds):
        try:
            ...
            _b = False
        if (
            # "predicate_sentinel" not in kwds and # removed 0.3.27b3
            "_final_sentinel" not in kwds and
            # 0.3.27b3
            "final" not in kwds
        ) or (
            _b is True
...
del os, ma, mmap, array, copy, tp, tpe, ty, ct, io, tk, wa, fn, abc, abc2, aio, sb, enum, ins, zi, url, hl, dis, ast, ErrorHandler # Not for export
