...
def decode_tile_batch(tile_batch):
    ...
    for tile_data_tuple in tile_batch:
        ...
        # Iterate over the layers in the decoded tile
        for layer_name, layer_data in decoded_tile.items():
            if layer_name not in layers:
                layers[layer_name] = extract_layer_fields(layer_data)
...
def merge_layer_dicts(layers_accumulated, new_layers):
    ...
    for name, layer in new_layers.items():
        if name not in layers_accumulated:
            ...
            layers_accumulated[name].update(layer)
    ...
    with tqdm(total=total_tiles, desc="Processing tiles") as pbar:
        with ProcessPoolExecutor(max_workers=workers) as executor:
            ...
            while offset < total_tiles:
                ...
                # Submit a batch for parallel decoding
                futures.append(executor.submit(decode_tile_batch, tile_batch))
