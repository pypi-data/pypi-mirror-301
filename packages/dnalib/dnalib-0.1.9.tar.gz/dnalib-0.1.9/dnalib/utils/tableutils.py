...
class TableUtils:
    ...
    @staticmethod
    def clone_table(source_catalog, layer, table_name, target_catalog, target_layer, target_table_name):
        """ 
            Método que executa um clone "safe" entre duas tabelas.

            Args:
                source_catalog (str): nome do catálogo de origem.
                layer (str): nome da camada de origem.
                table_name (str): nome da tabela de origem.
                target_catalog (str): nome do catálogo de destino.
                target_layer (str): nome da camada de destino.
                target_table_name (str): nome da tabela de destino.
        """
        if TableUtils.table_exists(f"{source_catalog}.{layer}", table_name):        
            Utils.spark_instance().sql(f"""
                CREATE OR REPLACE TABLE {target_catalog}.{target_layer}.{target_table_name}
                CLONE {source_catalog}.{layer}.{table_name} 
              """)
            df_comment = Utils.spark_instance().sql(f"DESCRIBE EXTENDED {source_catalog}.{layer}.{table_name}").filter("col_name == 'Comment'").select('data_type')
            if df_comment.count() > 0:                
                table_comment = df_comment.collect()[0][0]
                Utils.spark_instance().sql(f"COMMENT ON TABLE {target_catalog}.{target_layer}.{target_table_name} IS '{table_comment}'")
        else:
            log(__name__).warning(f"Table {source_catalog}.{layer}.{table_name} does not exists in lakehouse")

    @staticmethod
    def grant_select(source_catalog, layer, table_name, users):
        """ 
            Método que executa grant select para uma tabela e uma lista de usuários.

            Args:
                source_catalog (str): nome do catálogo.
                layer (str): nome da camada.
        ...
        if TableUtils.table_exists(f"{source_catalog}.{layer}", table_name):        
            for user in users:                
                try:
                    Utils.spark_instance().sql(f"""GRANT SELECT ON TABLE {source_catalog}.{layer}.{table_name} TO `{user}`""")           
                except:
                    log(__name__).warning(f"Could not grant SELECT in table {source_catalog}.{layer}.{table_name} to user {end_user}.")
        else:
            log(__name__).warning(f"Table {source_catalog}.{layer}.{table_name} does not exists in lakehouse.")
