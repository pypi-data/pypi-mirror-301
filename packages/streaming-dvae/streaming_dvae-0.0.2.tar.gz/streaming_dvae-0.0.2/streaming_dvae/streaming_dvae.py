...
class StreamingDVAE:
    ...
    def decode_caches(self, to_audio: bool = False):
        ...
        if not to_audio:
            return mel
        return self.vocos.streaming_decode(mel, True)
    ...
    def streaming_decode(
        ...
        for idx, code in enumerate(torch.unbind(codes, dim=2)):
            ...
            if not to_audio:
                ...
                return self.vocos.streaming_decode(mel, is_last)
    ...
    def test_streaming_decode(self, wav_path: str):
        ...
        for code in torch.unbind(codes, dim=2):
            mel_hat += self.streaming_decode(code[:, :, None])
