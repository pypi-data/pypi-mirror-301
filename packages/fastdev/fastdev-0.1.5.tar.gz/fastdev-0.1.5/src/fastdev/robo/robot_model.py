...
class RobotModel:
    ...
    def apply_mimic_joints(
        ...
        if joint_values.shape[-1] != self.num_dofs:
            raise ValueError(
                f"Expected joint_values to have shape (batch_size, {self.num_dofs}), got {joint_values.shape}"
            )
        if not self.has_mimic_joints:
            ...
            mimic_joint_values = torch.index_select(
                joint_values,
                dim=-1,
                index=self._mimic_joint_indices.to(joint_values.device),  # type: ignore
            )
            mimic_joint_values = mimic_joint_values * self._mimic_multipliers.to(  # type: ignore
                joint_values.device
            ) + self._mimic_offsets.to(joint_values.device)  # type: ignore
