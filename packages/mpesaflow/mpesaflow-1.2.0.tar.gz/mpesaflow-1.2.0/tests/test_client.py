...
class TestMpesaFlow:
    ...
    def test_base_url_env(self) -> None:
        with update_env(MPESA_FLOW_BASE_URL="http://localhost:5000/from/env"):
        ...
        with update_env(MPESA_FLOW_BASE_URL="http://localhost:5000/from/env"):
            with pytest.raises(ValueError, match=r"you must pass base_url=None"):
                MpesaFlow(bearer_token=bearer_token, _strict_response_validation=True, environment="sandbox")

            client = MpesaFlow(
                base_url=None, bearer_token=bearer_token, _strict_response_validation=True, environment="sandbox"
            )
            assert str(client.base_url).startswith("https://sandbox-api.mpesaflow.com/v1")
...
class TestAsyncMpesaFlow:
    ...
    def test_base_url_env(self) -> None:
        ...
        with update_env(MPESA_FLOW_BASE_URL="http://localhost:5000/from/env"):
            with pytest.raises(ValueError, match=r"you must pass base_url=None"):
                AsyncMpesaFlow(bearer_token=bearer_token, _strict_response_validation=True, environment="sandbox")
