...
class TestTextSnip:
    ...
    def test_text_bounds(self, textbounds: list[dict], monkeypatch):
        ...
        """
        monkeypatch.setattr("snip.snippets.text.get_font", get_font)
        ...
        for tb in textbounds:
            ...
            snip_bounds = snip.text_size

            # Check the width and height within a tolerance
            tol = 1  # 1px
            assert abs(snip_bounds[0] - bounds[0]) < tol
            assert abs(snip_bounds[1] - bounds[1]) < tol

            # 180deg rotation
            snip.rotation = 180
            bounds = snip.bounds

            assert abs(snip_bounds[0] - bounds[2]) < tol
            assert abs(snip_bounds[1] - bounds[3]) < tol

            # 90deg rotation
            snip.rotation = 90
            bounds = snip.bounds

            assert abs(snip_bounds[0] - (bounds[3] - bounds[1])) < tol
            assert abs(snip_bounds[1] - (bounds[2] - bounds[0])) < tol
