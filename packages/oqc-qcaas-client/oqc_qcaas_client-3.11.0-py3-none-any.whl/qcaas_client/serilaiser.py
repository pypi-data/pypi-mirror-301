...
client_config_string = "qcaas_client.compiler_config"
scc_config_string = "scc.compiler.config"
qat_config_string = "qat.purr.compiler.config"
...
def json_dumps(*args, serializable_types=None, **kwargs):
    ...
    json_string = json.dumps(*args, serializable_types=serializable_types, **kwargs)
    json_string = json_string.replace(client_config_string, qat_config_string)
    return json_string
...
class CustomJsonDecoder(JSONDecoder):
    ...
    def default(self, obj):
        ...
        obj_type = obj.get(type_label)
        if obj_type is None:
            return obj

        if obj_type == "<class 'tuple'>":
            return tuple(obj[data_label])

        if self.serializable_types is not None:
            old_paths = [scc_config_string, qat_config_string]
            for old_path in old_paths:
                if old_path in obj_type:
                    obj_type = obj_type.replace(old_path, client_config_string)
                    break
        ...
        if data_label in obj:
            data = obj[data_label]
            if obj.get(dataclass_label, False):
...
class CustomJSONEncoder(JSONEncoder):
    ...
    def default(self, obj):
        ...
        if (
            self.serializable_types is not None
            and typ_str not in self.serializable_types
        ...
        try:
            ...
            if is_dataclass(obj):
                return {
                    type_label: typ_str,
                    dataclass_label: True,
                    data_label: self.default(asdict(obj)),
                }
            elif isinstance(obj, Enum):
                return {type_label: typ_str, "$value": obj.value}
            elif isinstance(obj, complex):
                return {type_label: typ_str, data_label: str(obj)}
            elif isinstance(obj, tuple):
                return {
                    type_label: typ_str,
                    data_label: tuple(self.default(val) for val in obj),
                }
