...
from .includes.ha_info import HomeAssistantInfo
from .includes.tools import gethostname
...
def main():
    ...
    if mqtti.broker:
        ...
        mqtti.make_sensors()
        sensor_status = mqtti.get_sensor_by_name(SENSOR_NAME_STATUS)
        sensor_lastupdate = mqtti.get_sensor_by_name(SENSOR_NAME_LASTUPDATE)
    ...
    while loopcount > 0:
        ...
        if interval_diff.total_seconds() >= 3600:
        ...
        if mqtti.broker:
            ...
            if mqtti.connected:
                ...
                if args.printresults or args.printtime:
                    infolog(f"{t:16s} : {sensor_lastupdate.get_timestamp()}")
