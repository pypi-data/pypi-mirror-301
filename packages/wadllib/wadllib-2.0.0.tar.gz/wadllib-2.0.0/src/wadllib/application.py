...
class Resource(WADLResolvableDefinition):
        ...
        if representation is not None:
            if media_type == 'application/json':
                if representation_needs_processing:
                    if hasattr(representation, "decode"):
                        self.representation = json.loads(
                            representation.decode())
                    else:
                        self.representation = json.loads(str(representation))
                else:
    ...
    def _method_tag_iter(self):
        ...
        yield from definition.findall(wadl_xpath('method'))
...
class RepresentationDefinition(WADLResolvableDefinition, HasParametersMixin):
    ...
    def params(self, resource):
        return super().params(['query', 'plain'], resource)
    ...
    def _generate_multipart_form(self, parts):
        ...
        for is_binary, name, value in parts:
            ...
            if is_binary:
                ...
                # in our case, but without it both cgi.FieldStorage and
                # multipart.parse_form_data try to decode the file contents
                # as text.
...
class Link(WADLResolvableDefinition):
    ...
    def __init__(self, parameter, link_tag):
        ...
        """
        super().__init__(parameter.application)
        self.parameter = parameter
        self.tag = link_tag
