...
class SelectRenderer(PromptRenderer):
    """
    Renderer for select prompts.
    ...
    def create_layout(self) -> Layout:
        """
        Create and return the layout for the select prompt.

        Returns
        -------
        Layout
            The layout for the select prompt.
        """
    ...
    def get_formatted_text(self) -> FormattedText:
        """
        Get the formatted text for the select prompt options.

        Returns
        -------
        FormattedText
            The formatted text for the select prompt options.
        """
        result = []
        for idx, choice in enumerate(self.prompt.choices):
            ...
            style = "class:selected" if is_active else "class:unselected"
            result.append((style, f"{pointer} {choice}\n"))
        return FormattedText(result)

    def get_instructions(self) -> str:
        """
        Get the instructions for using the select prompt.

        Returns
        -------
        str
            The instruction text for the select prompt.
        """
        return (
