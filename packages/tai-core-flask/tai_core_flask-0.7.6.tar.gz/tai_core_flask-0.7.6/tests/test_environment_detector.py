...
from tai_core_flask.environments.azure_nb_environment import AzureNBEnvironment
from tai_core_flask.environments.credentialed_vm_environment import \
    CredentialedVMEnvironment
from tai_core_flask.environments.databricks_environment import (
    DATABRICKS_ENV_VAR, DatabricksEnvironment)
from tai_core_flask.environments.local_ipython_environment import \
    LocalIPythonEnvironment
from tai_core_flask.environments.public_vm_environment import \
    PublicVMEnvironment
...
class TestEnvironmentDetector(object):
    ...
    def test_public_vm(self, mocker):
        mocker.patch('tai_core_flask.FlaskHelper._is_local_port_available',
                     return_value=True)
    ...
    def test_public_vm_fail_on_port(self, mocker):
        mocker.patch('tai_core_flask.FlaskHelper._is_local_port_available',
                     return_value=False)
        with pytest.taises(RuntimeError) as exception:
    ...
    def test_azure_nb(self, mocker):
        ...
        mocker.patch(
            'tai_core_flask.environments.azure_nb_environment.'
