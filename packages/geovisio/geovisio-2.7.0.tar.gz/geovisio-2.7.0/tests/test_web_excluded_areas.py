...
from tests.test_web_upload_set import _create_upload_set, _add_files_to_upload_set
import pytest
...
def test_upload_set_excluded_pictures(datafiles, client, defaultAccountToken):
    ...
    usid = _create_upload_set(client, defaultAccountToken())
    response = _add_files_to_upload_set(client, usid, datafiles, "1.jpg", defaultAccountToken(), raw_response=True)
    assert response.status_code == 400, response.text
    rjson = response.json
...
def test_upload_set_user_excluded_pictures(datafiles, client, camilleAccountToken, defaultAccountToken):
    ...
    usid = _create_upload_set(client, camilleAccountToken())
    response = _add_files_to_upload_set(client, usid, datafiles, "1.jpg", camilleAccountToken(), raw_response=True)
    assert response.status_code == 400, response.text
    rjson = response.json
    assert rjson["message"] == "Picture has invalid metadata"
