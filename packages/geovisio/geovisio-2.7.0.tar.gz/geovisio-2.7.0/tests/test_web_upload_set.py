...
from tests.conftest import SEQ_IMGS, create_test_app, start_background_worker, waitForUploadSetState, waitForUploadSetStateReady
import pytest
import os
...
def test_upload_set_sort_by_filename(datafiles, app_client_with_auth, bobAccountToken, bobAccountID, sort_method, expected_names):
    ...
    upload_set_id = _create_upload_set(client, title="some title", sort_method=sort_method, estimated_nb_files=5, token=bobAccountToken())
    ...
    for name in ["some_names_010.jpg", "some_names_011.jpg", "some_names_312.jpg", "some_names_001.jpg", "some_names_002.jpg"]:
        _add_files_to_upload_set(app_client_with_auth, upload_set_id, datafiles, name, token=bobAccountToken())
...
def test_add_item_to_upload_set_auth_mandatory_ok(datafiles, app_client_with_auth, bobAccountToken, bobAccountID):
    ...
    upload_set_id = _create_upload_set(client, token=bobAccountToken(), metadata={"some": "metadata", "an_array": ["pouet"]})

    r = _add_files_to_upload_set(client, upload_set_id, datafiles, "1.jpg", token=bobAccountToken())
    ...
    response = _add_files_to_upload_set(client, upload_set_id, datafiles, "1.jpg", token=bobAccountToken(), raw_response=True)
...
def test_add_item_to_upload_set_already_blurred(datafiles, app_client_with_auth, bobAccountToken, bobAccountID):
    client = app_client_with_auth
    upload_set_id = _create_upload_set(client, token=bobAccountToken(), estimated_nb_files=1)

    r = _add_files_to_upload_set(client, upload_set_id, datafiles, "1.jpg", token=bobAccountToken(), additional_data={"isBlurred": "true"})
...
def test_add_item_to_upload_set_external_lat_lon_missing(datafiles, app_client_with_auth, bobAccountToken, bobAccountID):
    client = app_client_with_auth
    upload_set_id = _create_upload_set(client, token=bobAccountToken())
...
def test_add_item_to_upload_no_auth(datafiles, app_client_without_auth, defaultAccountID):
    ...
    upload_set_id = _create_upload_set(app_client_without_auth, token=None, estimated_nb_files=1)

    r = _add_files_to_upload_set(app_client_without_auth, upload_set_id, datafiles, "1.jpg", token=None)
...
def test_item_already_added_in_another_upload_set(datafiles, app_client_with_auth, bobAccountToken):
    ...
    upload_set_1_id = _create_upload_set(app_client_with_auth, token=bobAccountToken(), estimated_nb_files=2)
    upload_set_2_id = _create_upload_set(app_client_with_auth, token=bobAccountToken(), title="some other title", estimated_nb_files=2)

    r = _add_files_to_upload_set(app_client_with_auth, upload_set_1_id, datafiles, "1.jpg", token=bobAccountToken())
    # id must be valid uuid
    ...
    response = _add_files_to_upload_set(app_client_with_auth, upload_set_2_id, datafiles, "2.jpg", token=bobAccountToken())

    # if we add the same file into the other upload_set, we get a conflict
    response = _add_files_to_upload_set(
        app_client_with_auth, upload_set_2_id, datafiles, "1.jpg", token=bobAccountToken(), raw_response=True
    )
    assert response.status_code == 409, response.text
    assert response.json == {
...
def test_duplicate_authorized(datafiles, app_client_with_auth_and_duplicates, bobAccountToken):
    ...
    upload_set_1_id = _create_upload_set(client, token=bobAccountToken())
    upload_set_2_id = _create_upload_set(client, token=bobAccountToken(), title="some other title")

    _add_files_to_upload_set(client, upload_set_1_id, datafiles, "1.jpg", token=bobAccountToken())
    r2 = _add_files_to_upload_set(client, upload_set_2_id, datafiles, "1.jpg", token=bobAccountToken())
    # we can also add it on the same uploadset
    r = _add_files_to_upload_set(client, upload_set_2_id, datafiles, "1.jpg", token=bobAccountToken(), raw_response=True)
...
def test_add_item_to_non_owned_upload_set(datafiles, app_client_with_auth, bobAccountToken, defaultAccountToken):
    ...
    response = _add_files_to_upload_set(client, upload_set_id, datafiles, "1.jpg", token=defaultAccountToken(), raw_response=True)
...
def test_main_upload_set_workflow(datafiles, app_client_with_auth, bobAccountToken, bobAccountID):
    ...
    """
    upload_set_id = _create_upload_set(app_client_with_auth, token=bobAccountToken(), estimated_nb_files=2)

    r = _add_files_to_upload_set(app_client_with_auth, upload_set_id, datafiles, "1.jpg", token=bobAccountToken())
    ...
    r = _add_files_to_upload_set(app_client_with_auth, upload_set_id, datafiles, "2.jpg", token=bobAccountToken())
...
def test_split_workers(app_client_with_split_workers, datafiles, dburl, tmp_path, bobAccountID, bobAccountToken):
    ...
    upload_set_id = _create_upload_set(client, token=bobAccountToken(), estimated_nb_files=2)
    ...
    r = _add_files_to_upload_set(client, upload_set_id, datafiles, "2.jpg", token=bobAccountToken())
    pic_2_id = UUID(r["picture_id"])
...
def test_upload_set_with_manual_completion(datafiles, app_client_with_auth, bobAccountToken, bobAccountID):
    ...
    _add_files_to_upload_set(app_client_with_auth, upload_set_id, datafiles, "1.jpg", token=bobAccountToken())

    # if we query the upload set is it still not completed
    upload_set_r = _get_upload_set(client, upload_set_id, token=bobAccountToken())
    assert upload_set_r["completed"] is False

    _add_files_to_upload_set(app_client_with_auth, upload_set_id, datafiles, "2.jpg", token=bobAccountToken())
...
def test_split_workers_manual_completion(app_client_with_split_workers, datafiles, dburl, tmp_path, bobAccountID, bobAccountToken):
    ...
    """
    client = app_client_with_split_workers
    upload_set_id = _create_upload_set(client, token=bobAccountToken(), estimated_nb_files=3)
...
def _create_upload_set(client, token=None, title="some title", **kwargs):
...
def _add_files_to_upload_set(client, id, datafiles, file_name, token=None, raw_response=False, additional_data=None, headers=None):
...
def test_list_upload_sets(datafiles, app_client_with_auth, bobAccountToken):
    ...
    """
    u1 = _create_upload_set(app_client_with_auth, token=bobAccountToken())
    u2 = _create_upload_set(app_client_with_auth, token=bobAccountToken())
    u3 = _create_upload_set(app_client_with_auth, token=bobAccountToken())

    _add_files_to_upload_set(app_client_with_auth, u1, datafiles, "1.jpg", token=bobAccountToken())
    _add_files_to_upload_set(app_client_with_auth, u1, datafiles, "2.jpg", token=bobAccountToken())

    _add_files_to_upload_set(app_client_with_auth, u3, datafiles, "3.jpg", token=bobAccountToken())
...
def test_upload_set_deletion(datafiles, app_client_with_auth, bobAccountToken, bobAccountID):
    ...
    upload_set_id = _create_upload_set(client, token=bobAccountToken(), estimated_nb_files=8)

    for p in ["1.jpg", "2.jpg", "3.jpg", "4.jpg", "5.jpg", "e1.jpg", "e2.jpg", "e3.jpg"]:
        _add_files_to_upload_set(client, upload_set_id, datafiles, p, token=bobAccountToken())
...
def test_main_upload_set_dispatch_to_several_collections(datafiles, app_client_with_auth, bobAccountToken, bobAccountID):
    ...
    upload_set_id = _create_upload_set(app_client_with_auth, token=bobAccountToken(), estimated_nb_files=9, split_time=120)

    for p in datafiles.listdir():
        if p.ext == ".jpg" and p.basename not in ["b1.jpg", "b2.jpg", "e3.jpg"]:
            _add_files_to_upload_set(app_client_with_auth, upload_set_id, datafiles, p.basename, token=bobAccountToken())
    ...
    for p in {"b1.jpg", "b2.jpg", "e3.jpg"}:
        _add_files_to_upload_set(app_client_with_auth, upload_set_id, datafiles, p, token=bobAccountToken())
...
def test_capture_duplicates(datafiles, app_client_with_auth, bobAccountToken, fsesUrl):
    ...
    upload_set_id = _create_upload_set(client, token=bobAccountToken(), estimated_nb_files=5)
...
def test_invalid_files(datafiles, app_client_with_auth, bobAccountToken, fsesUrl):
    ...
    upload_set_id = _create_upload_set(client, token=bobAccountToken(), estimated_nb_files=6)

    r = _add_files_to_upload_set(client, upload_set_id, datafiles, "invalid_exif.jpg", token=bobAccountToken(), raw_response=True)
    assert r.status_code == 400
    assert r.json == {
    ...
    r = _add_files_to_upload_set(client, upload_set_id, datafiles, "e1_without_exif.jpg", token=bobAccountToken(), raw_response=True)
    assert r.status_code == 400
    assert r.json == {
        "message": "Impossible to parse picture metadata",
        "status_code": 400,
...
\t- No valid date in picture EXIF tags"""
    ...
    _add_files_to_upload_set(client, upload_set_id, datafiles, "e2.jpg", token=bobAccountToken())

    r = _add_files_to_upload_set(client, upload_set_id, datafiles, "not_a_jpg.txt", token=bobAccountToken(), raw_response=True)
    assert r.status_code == 400
    assert r.json == {
        "message": "Picture file is either missing or in an unsupported format (should be jpg)",
        "status_code": 400,
    }
    r = _add_files_to_upload_set(client, upload_set_id, datafiles, "invalid.jpg", token=bobAccountToken(), raw_response=True)
    assert r.status_code == 400
    assert r.json == {
    ...
    _add_files_to_upload_set(client, upload_set_id, datafiles, "e3.jpg", token=bobAccountToken())
...
def test_invalid_api_calls_not_counted(datafiles, app_client_with_auth, bobAccountToken):
    ...
    """
    client = app_client_with_auth
    u_id = _create_upload_set(client, token=bobAccountToken(), estimated_nb_files=2)
    ...
    # add good file
    _add_files_to_upload_set(client, u_id, datafiles, "1.jpg", token=bobAccountToken())
    pic_id = db.fetchone(current_app, "SELECT id FROM pictures")[0]

    # duplicate should count
    r = _add_files_to_upload_set(client, u_id, datafiles, "1.jpg", token=bobAccountToken(), raw_response=True)
...
def test_upload_set_forbidden_deletion(datafiles, app_client_with_auth, bobAccountToken, defaultAccountToken):
    ...
    upload_set_id = _create_upload_set(client, token=defaultAccountToken())
    r = client.delete(
        f"/api/upload_sets/{upload_set_id}",
        headers={"Authorization": f"Bearer {bobAccountToken()}"},
    )
...
def test_upload_set_deleted_after_all_its_pictures_deletion(
    ...
    for p in ["1.jpg", "2.jpg", "3.jpg", "4.jpg", "5.jpg"]:
        r = _add_files_to_upload_set(client, upload_set_id, datafiles, p, token=bobAccountToken())
