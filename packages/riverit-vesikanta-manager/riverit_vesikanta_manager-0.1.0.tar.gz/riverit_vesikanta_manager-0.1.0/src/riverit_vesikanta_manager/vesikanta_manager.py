...
class VesikantaManager:
    ...
    def open_lukematietojen_haku(self) -> None:
        ...
        try:
            self.vesikanta_window.find(const.LUKEMATIEDOT_WND)
        except ElementNotFound as e:
            raise ApplicationException(
    ...
    def read_lukematiedot_in(self, report_files_path: Path) -> bool:
        ...
        if not self.config.update_yearly_consumption_estimate:
            self.vesikanta_window.click(const.DONT_UPDATE_YEARLY_CONSUMPTION_CHECKBOX)

        self.vesikanta_window.click(const.LUE_LUKEMAT_SISAAN_BTN)
        ...
        lukemien_siirto_dialog = self.vesikanta_window.find_child_window(
    ...
    def save_report_csv(self, report_files_path: str) -> str:
        ...
        self.vesikanta_window = self._get_main_window()
        try:
            self.vesikanta_window.click(const.REPORT_TO_FILE_BTN)
    ...
    def set_customer_group_interval(
        self, customer_group_start: str, customer_group_end: str
    ) -> None:
        """NOTE: NOT IN USE
        Sets the customer group interval in the Vesikanta Manager.

        Args:
            `customer_group_start` (str): The starting value of the customer group.
            `customer_group_end` (str): The ending value of the customer group.
        """
        self.vesikanta_window = self._get_main_window()
        ...
        tracking_data_window.set_value(
            value=customer_group_start, locator=const.TRACKING_GRP_START_FLD
        )
        tracking_data_window.set_value(
            value=customer_group_end, locator=const.TRACKING_GRP_END_FLD
        )
    ...
    def set_revenue_report_interval(
        ...
        revenue_report_window.set_value(
            customer_group_start, locator=const.REVENUE_GRP_START_FLD
        )
        revenue_report_window.set_value(
            customer_group_end, locator=const.REVENUE_GRP_END_FLD
        )
