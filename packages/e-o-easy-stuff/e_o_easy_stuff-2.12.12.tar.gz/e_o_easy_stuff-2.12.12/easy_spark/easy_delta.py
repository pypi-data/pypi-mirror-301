...
from easy_spark_fs.easy_spark_cat_helpers import EasySparkCatHelpers
...
# TODO: Move fs to its owns library
# TODO: See if you can get redit of the static methods
# TODO: Overwrite eq and + -
# TODO: init_delta=False do this automatically
# TODO: Add collect to easy_df
# TODO: Add Method to easy return a list of coloumns
# TODO: Add Empty or null for pandas
# TODO:
...
class EasyDeltaPath:
    ...
    def __init__(self, path: str = None, spark: SparkSession = None, create_table_if_not_exists: bool = False,
        ...
        self.spark_fs_helpers = EasySparkFSHelpers(spark)
        self.spark_cat_helpers = EasySparkCatHelpers(spark)
    ...
    def to_fs(self, must_java_import=False) -> EasySparkFS:
        return EasySparkFS(EasyDeltaPath._spark, self.path, must_java_import)
    ...
    def exists(self) -> bool:
        if self.for_name:
            return self.spark_cat_helpers.table_exists_from_path(self.path)
        else:
            return self.spark_fs_helpers.file_exists_from_path(self.path, False)
    ...
    def add_from_list(self, records: list[dict], df_format="delta",
        ...
        new_easy_df = self.spark_instance.create_easy_df().clone_from_list(records, schema, ignore_order)
    ...
    def add_from_df(self, df: DataFrame, df_format="delta", merge_option: str = "overwriteSchema",
                    partition_columns: list[str] = None) -> 'EasyDeltaPath':
        new_easy_df = self.spark_instance.create_easy_df(df)
    ...
    def add_from_pd_df(self, pd_df: DataFramePandas, df_format="delta", schema: StructType = None,
        ...
        new_easy_df = self.spark_instance.create_easy_df().clone_from_pd_df(pd_df, schema)
        self.save(new_easy_df, df_format, mode="append", merge_option=merge_option, partition_columns=partition_columns)
        return self
