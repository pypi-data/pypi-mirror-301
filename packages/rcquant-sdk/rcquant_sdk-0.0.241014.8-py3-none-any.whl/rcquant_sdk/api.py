...
def fin_get_history_ohlc(exchang_id: str, instrument_id: str, range: str,
    ...
    result, msg, dfohlc = fin_read_ohlc_list(f'{exchang_id}.{instrument_id}', int(start_date), int(end_date), range)
    if result is False:
...
def fin_get_history_ohlc_convert(exchang_id: str, instrument_id: str, range: str,
    ...
    '''
    if range[-1] != 's':
        return [False, 'error range', None]

    # convert from 3s
    result, msg, dfohlc = fin_read_ohlc_list(f'{exchang_id}.{instrument_id}', int(start_date), int(end_date), '3s')
    if result is False:
        return [result, msg, dfohlc]
    # print('s2,time:%f' % (time.time()))
    if range != '3s':
        dfohlc = convert_ohlc_period(dfohlc, range)
