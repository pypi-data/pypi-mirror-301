...
def _should_include_cycle(cycle: dict, product_value: int, site: dict):
    product = find_primary_product(cycle) or {}
    return all([
        product.get('economicValueShare', 0) > 0,
        product_value > 0,
        bool(site),
...
def format_for_grouping(cycles: dict):
    def format(cycle: dict):
        ...
        } if _should_include_cycle(cycle, product_value, site) else None
