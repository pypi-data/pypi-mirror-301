...
class Store:
    ...
    def add_docs(self, file_paths: Union[str, List[str]]):
        """
        Adds documents from file paths or directories to the Qdrant collection.

        Args:
            file_paths (Union[str, List[str]]): The file paths or directories of the documents to add.
        """
        from qdrant_client.http.models import PointStruct
        ...
        points = [point for result in results if result for point in result]
        ...
        if points:
            try:
                ...
                logger.info(f"Successfully processed and added {len(points)} points from {len(all_files)} files.")
            except Exception as e:
                logger.error(f"Error upserting points to collection: {e}")
        else:
            logger.warning("No valid points to add to the collection.")
