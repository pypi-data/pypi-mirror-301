...
from ._utils import fix_hostname
...
class Root:
    ...
    def __init__(
        ...
        # Fix hostname, if account locator is used to connect to Snowflake
        #  replace _ (underscores) with - (dashes)
        self._hostname = None
        if self._connection.rest is not None and hasattr(self._connection.rest, "_host"):
            # In stored procedures connection.rest doesn't have _host
            self._hostname = fix_hostname(
                self._connection.rest._host,
