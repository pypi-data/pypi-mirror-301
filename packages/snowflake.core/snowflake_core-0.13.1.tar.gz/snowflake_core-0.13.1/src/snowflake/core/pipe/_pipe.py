...
class PipeResource(SchemaObjectReferenceMixin[PipeCollection]):
    ...
    @api_telemetry
    def drop(self, if_exist: Optional[bool] = None) -> None:
        """Drop this pipe.

        Parameters
        __________
        if_exist: bool, optional
            If ``True``, does not throw an exception if the pipe does not exist. The default is ``None``,
        ...
        """
        self._api.delete_pipe(self.database.name, self.schema.name, self.name, if_exist, async_req=False)

    @api_telemetry
    def refresh(
        self, if_exist: Optional[bool] = None, prefix: Optional[str] = None, modified_after: Optional[datetime] = None
    ) -> None:
        """Refresh this pipe.
        ...
        self._api.refresh_pipe(
            self.database.name, self.schema.name, self.name, if_exist, prefix, modified_after, async_req=False
        )
