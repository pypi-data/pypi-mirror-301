...
class AccountCollection(AccountObjectCollectionParent["AccountResource"]):
    ...
    def create(
        ...
        Parameters
        __________
        account: an instance of :class:`Account`.
...
class AccountResource(ObjectReferenceMixin[AccountCollection]):
    ...
    @api_telemetry
    def drop(self, grace_period_in_days: int, if_exists: bool = False) -> None:
        """Drop this account.
        ...
        if_exists : bool
            If ``True``, does not throw an exception if the account does not exist. The default is ``None``,
