...
def dq (o):
    return '"' + o.replace ('"', '""') + '"' if isinstance (o, str) else o

def main (asArgs):

    if not asArgs:
        print ("version prints the teradatasql version")
        print ("verbose:connection (the default) and verbose:-connection controls printing connection status")
        ...
        print ("Otherwise the SQL request is executed")
        return
    ...
    for sArg in asArgs:
        ...
        if sArg.startswith ("version"):

            print ("version =", dq (__version__))

        elif sArg == "verbose:connection":
            ...
            sArg = sArg.replace (",,", "\x01") # temporarily replace double commas (literal commas) with Ctrl+A

            mapParams = {}
            for sPair in sArg.split (","):
            ...
            if bVerboseConnection:
                ...
                print ("Status =" , dq (sStatus))
                print ("Remote =" , dq (sRemote))
                print ("Version =", dq (sVersion))
                print ("Session =", dq (sSession))
            ...
            print ("No connection available for", sArg)
                ...
                for iResult in itertools.count ():
                    for iRow, row in enumerate (cur.fetchall ()):
                        for iColumn, value in enumerate (row):
                            sName = cur.description [iColumn][0]
                            sType = cur.columntypename [iColumn]
                            print ("Result {} Row {} Column {} {} {} = {}".format (iResult + 1, iRow + 1, iColumn + 1, dq (sName), sType, dq (value)))
                    if not cur.nextset ():
                        break
