...
class LabeledDataset(RootModel[t.List[_Instance]], t.Generic[_Instance, _Label]):
    """
    ...
    ...         return item[1]
    ...
    ... dataset = SentimentDataset.model_validate(
    ...
    def cv(
        ...
        for train_index, test_index in rskf.split(self, self.labels()):
            yield cls.model_validate(self[i] for i in train_index), cls.model_validate(self[i] for i in test_index)
    ...
    def balance(self, seed: int = 0):
        ...
        return cls.model_validate(list(upsampled))
    ...
    def split(self, test_size: t.Union[float, int, None] = None, seed: int = 0, shuffle: bool = True):
        ...
        return cls.model_validate(train), cls.model_validate(test)
