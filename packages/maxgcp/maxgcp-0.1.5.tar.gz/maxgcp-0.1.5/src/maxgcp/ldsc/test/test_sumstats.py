...
import maxgcp.ldsc.parse as ps
import maxgcp.ldsc.sumstats as s
from maxgcp.ldsc.scripts.ldsc import parser
...
def t(attr):
    ...
    def inner(obj):
        return getattr(obj, attr, float("nan"))
...
def test_check_condnum():
    x = np.ones((2, 2))
    x[1, 1] += 1e-5
    args.invert_anyway = False  # type: ignore
    with pytest.raises(ValueError):
        s._check_ld_condnum(args, log, x)
    args.invert_anyway = True  # type: ignore
...
class Test_RG_Statistical(unittest.TestCase):
    ...
    def test_rg_ratio_noint(self):
        rg_ratio = [
            x.rg_ratio
            for x in self.rg_noint
            if getattr(x, "rg_ratio", None) is not None
        ]
        assert len(rg_ratio) == 0

    def test_rg_se(self):
        assert_allclose(
            np.nanmean(map(t("rg_se"), self.rg)),
            np.nanstd(map(t("rg_ratio"), self.rg)),
            atol=0.02,
        )

    def test_rg_se_noint(self):
        rg_se = [
            x.rg_se for x in self.rg_noint if getattr(x, "rg_se", None) is not None
        ]
        assert len(rg_se) == 0
        rg_ratio = [
            x.rg_ratio
    ...
    def test_gencov_tot_noint(self):
        gencov = [getattr(x, "gencov", None) for x in self.rg_noint]
        tot = [getattr(x, "tot", None) for x in gencov if x is not None]
        assert len(tot) == 0
        # assert_allclose(
    ...
    def test_gencov_tot_se(self):
        assert_allclose(
            np.nanstd(map(t("tot"), map(t("gencov"), self.rg))),
            np.nanmean(map(t("tot_se"), map(t("gencov"), self.rg))),
            atol=0.02,
        )

    def test_gencov_tot_se_noint(self):
        gencov = [getattr(x, "gencov", None) for x in self.rg_noint]
        tot_se = [getattr(x, "tot_se", None) for x in gencov if x is not None]
        assert len(tot_se) == 0
        # assert_allclose(
        #     np.nanstd(map(t("tot"), map(t("gencov"), self.rg_noint))),
        #     np.nanmean(map(t("tot_se"), map(t("gencov"), self.rg_noint))),
        #     atol=0.02,
        # )
    ...
    def test_gencov_cat_noint(self):
        gencov = [getattr(x, "gencov", None) for x in self.rg_noint]
        cat = [getattr(x, "cat", None) for x in gencov if x is not None]
        assert len(cat) == 0
        # assert_allclose(
        #     np.nanmean(map(t("cat"), map(t("gencov"), self.rg_noint))),
        #     [0, 0],
        #     atol=0.02,
        # )
    ...
    def test_gencov_cat_se(self):
        assert_allclose(
            np.nanstd(map(t("cat"), map(t("gencov"), self.rg))),
            np.nanmean(map(t("cat_se"), map(t("gencov"), self.rg))),
            atol=0.02,
        )

    def test_gencov_cat_se_noint(self):
        gencov = [getattr(x, "gencov", None) for x in self.rg_noint]
        cat_se = [getattr(x, "cat_se", None) for x in gencov if x is not None]
        assert len(cat_se) == 0
        # assert_allclose(
        #     np.nanstd(map(t("cat"), map(t("gencov"), self.rg_noint))),
        #     np.nanmean(map(t("cat_se"), map(t("gencov"), self.rg_noint))),
        #     atol=0.02,
        # )
    ...
    def test_gencov_int_se(self):
        assert_allclose(
            np.nanmean(map(t("intercept_se"), map(t("gencov"), self.rg))),
            np.nanstd(map(t("intercept"), map(t("gencov"), self.rg))),
            atol=0.1,
        )
    ...
    def test_hsq_int_se(self):
        assert_allclose(
            np.nanmean(map(t("intercept_se"), map(t("hsq2"), self.rg))),
            np.nanstd(map(t("intercept"), map(t("hsq2"), self.rg))),
            atol=0.1,
        )
...
class Test_H2_Statistical(unittest.TestCase):
    ...
    def test_tot_se(self):
        assert_allclose(
            np.nanmean(map(t("tot_se"), self.h2)),
            np.nanstd(map(t("tot"), self.h2)),
            atol=0.05,
        )

    def test_tot_se_noint(self):
        assert_allclose(
            np.nanmean(map(t("tot_se"), self.h2_noint)),
            np.nanstd(map(t("tot"), self.h2_noint)),
            atol=0.05,
        )
    ...
    def test_coef_se(self):
        for h in [self.h2, self.h2_noint]:
            assert_array_almost_equal(
                np.nanmean(map(t("coef_se"), h), axis=0),
    ...
    def test_prop_se(self):
        for h in [self.h2, self.h2_noint]:
            assert np.all(
                np.nanmean(map(t("prop_se"), h), axis=0)
                - np.nanstd(map(t("prop"), h), axis=0)
                < 0.02
            )
    ...
    def test_int_se(self):
        assert_allclose(
            np.nanstd(map(t("intercept"), self.h2)),
            np.nanmean(map(t("intercept_se"), self.h2)),
            atol=0.1,
        )
...
class Test_Estimate(unittest.TestCase):
    def test_h2_M(self):  # check --M works
        ...
        with open(DIR + "/simulate_test/ldscore/oneld_onefile.l2.M_5_50") as f:
            args.M = str(float(f.read()))
        y = s.estimate_h2(args, log)
        assert_array_almost_equal(x.tot, y.tot)
        assert_array_almost_equal(x.tot_se, y.tot_se)
        args.M = "1,2"
        with pytest.raises(ValueError):
            s.estimate_h2(args, log)
        args.M = "foo_bar"
        with pytest.raises(ValueError):
            s.estimate_h2(args, log)
    ...
    def test_h2_ref_ld(self):  # test different ways of reading ref ld
        ...
        assert x.tot == pytest.approx(y.tot)
    ...
    def test_rg_ref_ld(self):
        ...
        assert x.rg_ratio == pytest.approx(y.rg_ratio)
        assert x.rg_jknife == pytest.approx(z.rg_jknife)
        assert x.rg_se == pytest.approx(y.rg_se)
    ...
    def test_no_check_alleles(self):
        ...
        assert x.rg_jknife == pytest.approx(y.rg_jknife)
