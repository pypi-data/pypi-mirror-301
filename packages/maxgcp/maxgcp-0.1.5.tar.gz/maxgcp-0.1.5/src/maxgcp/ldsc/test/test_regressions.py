...
import maxgcp.ldsc.regressions as reg
...
class Test_h2_obs_to_liab(unittest.TestCase):
    def test_bad_data(self):
        ...
        with pytest.raises(ValueError):
            reg.h2_obs_to_liab(1, 0.5, 1)
        with pytest.raises(ValueError):
            reg.h2_obs_to_liab(1, 0, 0.5)
        with pytest.raises(ValueError):
            reg.h2_obs_to_liab(1, 0.5, 0)
...
class Test_Coef(unittest.TestCase):
    ...
    def test_intercept(self):
        assert self.hsq_int.intercept is not None
        assert self.hsq_int.ratio is not None
        assert isinstance(self.hsq_int.ratio, float)
...
class Test_RG_2D(unittest.TestCase):
    ...
    def test_rg(self):
        ...
        assert isinstance(self.rg.rg_ratio, float)
