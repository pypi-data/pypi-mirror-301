...
import maxgcp.ldsc.scripts.munge_sumstats as munge
...
class test_process_n(unittest.TestCase):
    def setUp(self):
        self.dat = pd.DataFrame(["rs1", "rs2", "rs3"], columns=["SNP"])  # type: ignore
        self.dat_filtered = pd.DataFrame(["rs2", "rs3"], columns=["SNP"])  # type: ignore
        self.dat_filtered["N"] = [1234, 1234.0]
        self.dat_filtered9999 = pd.DataFrame(["rs2", "rs3"], columns=["SNP"])  # type: ignore
...
class test_parse_flag_cnames(unittest.TestCase):
    ...
    def test_sign_error(self):
        ...
        with pytest.raises(ValueError):
            munge.parse_flag_cnames(log, self.args)
...
class test_end_to_end(unittest.TestCase):
    def setUp(self):
        self.args = munge.parser.parse_args("")
        self.args.sumstats = "src/maxgcp/ldsc/test/munge_test/sumstats"
        self.args.out = "asdf"
    ...
    def test_basic(self):
        ...
        correct = pd.read_csv(
            "src/maxgcp/ldsc/test/munge_test/correct.sumstats", sep=r"\s+", header=0
        )
        assert_frame_equal(x, correct)

    def test_merge_alleles(self):
        self.args.merge_alleles = "src/maxgcp/ldsc/test/munge_test/merge_alleles"
        x = munge.munge_sumstats(self.args, p=False)
        correct = pd.read_csv(
            "src/maxgcp/ldsc/test/munge_test/correct_merge.sumstats",
            sep=r"\s+",
            header=0,
        )
        assert_frame_equal(x, correct)
    ...
    def test_bad_merge_alleles(self):
        self.args.merge_alleles = "src/maxgcp/ldsc/test/munge_test/merge_alleles_bad"
        with pytest.raises(ValueError):
            munge.munge_sumstats(self.args, p=False)
