...
class YTAEnum(Enum):
    ...
    def get_valid_name(cls, name: str):
        ...
        """
        str_names = cls.get_all_names_as_str()

        try:
            return str_names[[enum_name.lower() for enum_name in str_names].index(name.lower())]
        except Exception:
            return None
...
def is_valid_name(name: any, enum: YTAEnum, ignore_case: bool = False):
    ...
    if not ignore_case:
        ...
        str_names = get_all_names_as_str()
        try:
            str_names[[enum_name.lower() for enum_name in str_names].index(name.lower())]
