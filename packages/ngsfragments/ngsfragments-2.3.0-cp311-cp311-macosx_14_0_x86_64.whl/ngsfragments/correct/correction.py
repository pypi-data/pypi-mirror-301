...
def binned_bias_correct_counts(values: np.ndarray,
    ...
    for column in bin_bias.columns:
        ...
        res = loess_1d.loess_1d(medians, bins[:-1])
        res = res[0]
...
def gc_mappability_bin_correct(values: np.ndarray,
    ...
    # Determine bins
    km = KMeans(n_clusters=n_bins, n_init = "auto")
    km.fit(bin_bias.df.loc[:,"gc"].values.reshape(-1,1))
    gc_labels = km.labels_
    km = KMeans(n_clusters=n_bins, n_init = "auto")
    km.fit(bin_bias.df.loc[:,"mappability"].values.reshape(-1,1))
    m_labels = km.labels_
    labels = np.array([str(gc_labels[i])+"-"+str(m_labels[i]) for i in range(len(m_labels))])

    # Calculate medians for clusters
    label_gc = pd.Series(bin_bias.df.loc[:,"gc"].values).groupby(labels).median()
    label_m = pd.Series(bin_bias.df.loc[:,"mappability"].values).groupby(labels).median()
    label_x = pd.Series(values).groupby(labels).median()

    # Run 2D loess
    res = loess_2d.loess_2d(x=label_gc.values, y=label_m.values, z=label_x.values)

    # Calculate corrected values
    for i, l in enumerate(label_x.index.values):
        new_values[labels==l] = new_values[labels==l] / res[0][i]

    # Correct others
    bin_bias = bin_bias.copy()
    bin_bias.df = bin_bias.df.drop(columns=["gc","mappability","blacklist"])
