...
def calculate_averages_for_specific_string(data, string_to_average, verbose=False):
    """
    ...
    if string_to_average in data:
        if verbose: print("per_use_case is present...")
        for metric, use_cases in data[string_to_average].items():
            ...
            if all_values:
                ...
                if verbose: print("average_key[", average_key, "], all_values[", average_value, "]")
                data[average_key] = {k: average_value for k in values.keys()}

    # Appel récursif pour traiter les sous-niveaux
    for key, value in data.items():
        if isinstance(value, dict):
            calculate_averages_for_specific_string(value, string_to_average)
...
def make_thematic(root_directory, dataset_name, test_case_number, nnvvppp_algoname, thematic_function, verbose=False,
    ...
    **thematic_kwargs : Arguments nommés supplémentaires à passer à la fonction thématique.
    ...
    most_recent_json_file = utils.get_last_json(
        root_directory,
        dataset_name,
        test_case_number,
...
def make_generic(root_directory, dataset_name, test_case_number, nnvvppp_algoname, verbose=False,
    ...
    # Trouvez le fichier JSON le plus récent dans le dossier des résultats
    most_recent_json_file = utils.get_last_json(root_directory,
                                          dataset_name,
                                          test_case_number,
                                          nnvvppp_algoname, verbose=verbose)
    if (most_recent_json_file):
...
def make_use_case_json_summary(filenames: list, output_file: str):
    """
    ...
    for filename in filenames:
        ...
        with open(filename, 'r') as f:
            ...
            for metric, metric_data in data["metrics"].items():
                # Vérifie si la métrique existe dans 'final_summary', sinon l'initialise
            ...
            for thematic, thematic_data in data["thematics"].items():
                ...
                # Parcourt les catégories dans les données thématiques
                for category, algorithms in thematic_data.items():
                    # Vérifie si 'use_case_id' existe dans 'per_use_case', sinon l'initialise
                    if use_case_id not in final_summary["thematics"][thematic]["per_use_case"]:
                        final_summary["thematics"][thematic]["per_use_case"][use_case_id] = {}

                    # Vérifie si la catégorie existe pour ce 'use_case_id', sinon l'initialise
                    if category not in final_summary["thematics"][thematic]["per_use_case"][use_case_id]:
                        final_summary["thematics"][thematic]["per_use_case"][use_case_id][category] = {}

                    # Parcourt les algorithmes et leurs valeurs pour chaque catégorie
                    for algorithm, value in algorithms.items():
                        print(
                            f"thematic = [{thematic}], category = [{category}], algorithm = [{algorithm}], value = [{value}]")

                        # Insère les valeurs dans le bon endroit du dictionnaire
                        final_summary["thematics"][thematic]["per_use_case"][use_case_id][category][algorithm] = value
...
def create_summary_json_at_satellite_type_level_from_data_folder(root_directory: str, satellite_type: str):
    ...
    make_use_case_json_summary(json_path_list, output_json_path)

    output_json_path_2 = os.path.join(data_summary_folder_path, "bis_" + make_json_data_level_name(satellite_type))

    # calculate_mean_metrics_by_per_use_case(output_json_path,output_json_path_2)
