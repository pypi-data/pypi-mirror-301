...
def get_param(request, param_name, param_type, max_length=None, required=True, data=None, LOG=None):
    ...
    if required and param is None: raise_error(400, f'Missing required param {param_name} on {request.url.path}', LOG)
    if param_type==str:
        if max_length and len(param)>max_length: raise_error(400, f'Invalid param {param_name} on {request.url.path}', LOG)
        return param.strip()
        ...
        if max_length is not None and sys.getsizeof(param)>max_length: raise_error(400, f'Invalid param {param_name} on {request.url.path}', LOG)
        return param
    elif param_type==dict:
        if max_length is not None and sys.getsizeof(param)>max_length: raise_error(400, f'Invalid param {param_name} on {request.url.path}', LOG)
        return param
...
class APIServer:
    ...
    async def req(self, request):
        ...
        req_uid = get_uid()
        task = {'req_uid':req_uid, 'created_at':now(), 'key':key, 'status':'REQUESTED', 'payload':to_json(payload)}
    ...
    async def get_upload_url(self, request):
        ...
        key = get_param(request, 'key', str, max_length=256, LOG=self.LOG)
        self.check_access(key)
        if self.fs is None: raise_error(self.LOG, err_msg='Upload links not available', status_code=400)
        bucket = 'kardio'
        fname = f'EXT-{get_uid()}'
    ...
    async def done(self, request):
        ...
        self.check_worker_access(key)
        self.db.select_and_update('api_tasks', 'req_uid', {'req_uid':req_uid, 'status':'DONE', 'result':result,
