...
def grab_task(db):
    ...
    try:
        ...
        Q = f"SELECT * FROM {tbl} WHERE status == 'REQUESTED' ORDER BY uid LIMIT 1"
        cursor.execute(Q)
        ...
        cursor.execute(f"UPDATE {tbl} SET status = ? WHERE uid = ?", ('PROCESSING', d['uid']))
        db.conn.commit()
        return d
...
class APIServer:
    ...
    def get_task(self, uid): return self.db.select_one('api_tasks', 'uid', uid)
    ...
    async def req(self, request):
        ...
        task = {'uid':get_uid(), 'created_at':now(), 'key':key, 'status':'REQUESTED', 'payload':to_json(payload)}
    ...
    async def check(self, request):
        ...
        uid = get_param(request, 'uid', int, LOG=self.LOG)
        self.check_access(key, enforce_credits=False)
        task = self.db.select_one('api_tasks', 'uid', uid)
        if task is None: raise_error(self.LOG, err_msg='Request not found', status_code=400)
    ...
    async def proc(self, request):
        ...
        self.check_access(key)
        # Call sync worker
        result = sync_request(f'{self.wurl}/proc', method='POST', payload=payload, LOG=self.LOG)
    ...
    async def grab(self, request):
        ...
        if task is not None: self.db.select_and_update('api_tasks', 'uid', task['uid'], {'status':'DISPATCHED'})
        if task is not None: self.LOG(2, 0, label='API', label2='/worker/grab', msg=f"uid={task['uid']}")
        return rest_ok(result=task)

    async def start(self, request):
        data = await request.json()
        key = get_param(request, 'key', str, max_length=256, data=data, LOG=self.LOG)
        uid = get_param(request, 'uid', int, data=data, LOG=self.LOG)
        eta = get_param(request, 'eta', int, data=data, LOG=self.LOG)
        self.check_worker_access(key)
        self.db.select_and_update('api_tasks', 'uid', {'uid':uid, 'status':'START', 'eta':eta})
        return rest_ok()
    ...
    async def done(self, request):
        ...
        self.check_worker_access(key)
        self.db.select_and_update('api_tasks', 'uid', {'uid':uid, 'status':'DONE', 'result':result,
                                                       'credits':used_credits, 'eta':None})
    ...
    async def error(self, request):
        data = await request.json()
        key = get_param(request, 'key', str, max_length=256, data=data, LOG=self.LOG)
        uid = get_param(request, 'uid', int, max_length=256, data=data, LOG=self.LOG)
        self.check_worker_access(key)
        self.db.select_and_update('api_tasks', 'uid', {'uid':uid, 'status':'ERROR', 'eta':None})
        return rest_ok()
    ...
    def run(self, port=None):
        ...
        routes = [
            ...
            # Client sync routes
            Route('/apiserver/proc', endpoint=self.req, methods=['POST']),
            # Worker routes
...
class APIClient:
    ...
    def check(self, req_id):
        return self.do_request('/apiserver/check', {'key':self.key, 'req_id':req_id})
...
class APIWorker:
    ...
    def __init__(self, service, cfg=None, api_server_url=None):
        ...
        self.LOG(2, 0, label='READY', label2='STARTUP', msg=f'API Server at {self.api_server_url}')
    ...
    def start(self, uid, eta):
        return self.request('worker/start', {'key':self.wkey, 'id':uid, 'eta':eta})

    def done(self, uid, result, used_credits=0.):
        return self.request('worker/done', {'key':self.wkey, 'id':uid, 'result':result, 'credits':used_credits})

    def error(self, uid):
        return self.request('worker/error', {'key':self.wkey, 'id':uid})
            ...
            while True:
                ...
                try:
                    ...
                    if not cb(task, self.start, self.done, self.error):
                        self.LOG(2, 0, label='APIWorker', label2='DONE', msg=f"{uid}")
                    else:
                        self.LOG(3, 0, label='APIWorker', label2='ABORTED', msg=f"{uid}")
                except Exception as exc:
                    self.error(uid)
                    self.LOG(4, 0, label='APIWorker', label2='Callback EXCEPTION', msg=f"{uid}, exc={exc}")
