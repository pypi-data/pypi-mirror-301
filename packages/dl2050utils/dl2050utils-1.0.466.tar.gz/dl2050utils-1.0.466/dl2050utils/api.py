...
class APIServer:
    ...
    async def proc(self, request):
        ...
        if self.sync_worker_url is None: raise_error(self.LOG, err_msg='Sync Worker not found', status_code=400)
        data = await request.json()
        ...
        self.check_access(key)
        # Call sync worker
        result = sync_request(f'{self.sync_worker_url}/proc', method='POST', payload=payload, LOG=self.LOG)
