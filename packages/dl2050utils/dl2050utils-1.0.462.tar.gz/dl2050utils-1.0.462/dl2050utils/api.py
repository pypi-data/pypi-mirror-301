...
class APIServer:
    ...
    async def check(self, request):
        ...
        req_uid = get_param(request, 'req_uid', int, LOG=self.LOG)
    ...
    async def grab(self, request):
        ...
        if task is not None: self.db.select_and_update('api_tasks', 'req_uid', task['req_uid'], {'status':'DISPATCHED'})
    ...
    async def start(self, request):
        data = await request.json()
        key = get_param(request, 'key', str, max_length=256, data=data, LOG=self.LOG)
        req_uid = get_param(request, 'req_uid', int, data=data, LOG=self.LOG)
    ...
    async def error(self, request):
        data = await request.json()
        key = get_param(request, 'key', str, max_length=256, data=data, LOG=self.LOG)
        req_uid = get_param(request, 'req_uid', int, max_length=256, data=data, LOG=self.LOG)
        self.check_worker_access(key)
