...
class GS:
    ...
    def upload_mem(self, bucket_name, blob_name, data, content_type='application/octet-stream'):
        """
    ...
    def download_mem(self, bucket_name, blob_name, as_string=False, encoding='utf-8'):
        """
        ...
        Args:
            ...
            blob_name (str): Name of the blob to download.
        Returns:
            str: Data from the blob as a UTF-8 decoded string.
        """
        try:
            bucket = self.gc.bucket(bucket_name)
            blob = bucket.blob(blob_name)
            data = blob.download_as_bytes()
            if as_string: return data.decode(encoding)
            return data
        except Exception as exc:
