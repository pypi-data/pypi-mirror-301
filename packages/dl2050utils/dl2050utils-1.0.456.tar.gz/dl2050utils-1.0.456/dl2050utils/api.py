            ...
            while True:
                ...
                try:
                    ...
                    if not cb(task, self.start, self.done, self.error):
                        self.LOG(2, 0, label='API_Worker', label2='DONE', msg=f"{uid}")
                    else:
                        self.LOG(3, 0, label='API_Worker', label2='ABORTED', msg=f"{uid}")
                except Exception as exc:
                    self.error(uid)
                    self.LOG(4, 0, label='API_Worker', label2='Callback EXCEPTION', msg=f"{uid}, exc={exc}")
