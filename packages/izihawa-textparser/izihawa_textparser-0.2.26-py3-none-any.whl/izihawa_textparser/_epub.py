...
from ebooklib.utils import (
    guess_type,
    get_pages_for_items, parse_html_string,
)
...
class EpubParser:
    ...
    def parse_soup(self, soup: BeautifulSoup):
        ...
        for el in list(soup.select(".Equation img")):
        ...
        for el in list(soup.select(".InlineEquation img")):
            if alt := el.attrs.get("alt"):
        ...
        for el in list(soup.select(".ListItem > .ItemContent")):
    ...
    with tempfile.NamedTemporaryFile(mode="w+b", delete=False) as t_file:
        ...
        for item in items:
            if _ := (
                ...
                if text := epub_parser.parse_soup(soup):
                    text = re.sub("\n([a-z])", r" \g<1>", text)
                    text = re.sub(
                        r"\n\s*\n\s*$",
                        "\n\n",
