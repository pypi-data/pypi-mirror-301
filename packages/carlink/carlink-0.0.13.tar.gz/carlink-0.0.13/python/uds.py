...
class UdsClient():
  def __init__(self, carlink, tx_addr: int, rx_addr: int | None = None, bus: int = 0, sub_addr: int | None = None, timeout: float = 1,
    ...
    self.timeout = timeout
    self.debug = debug
    can_send_with_timeout = partial(carlink.can_send, timeout=int(tx_timeout*1000))
    self._can_client = CanClient(can_send_with_timeout, carlink.can_recv, self.tx_addr, self.rx_addr, self.bus, self.sub_addr, debug=self.debug)
