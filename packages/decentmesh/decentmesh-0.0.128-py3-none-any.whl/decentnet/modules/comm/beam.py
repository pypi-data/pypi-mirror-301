...
class Beam:
    ...
    def initialize_incoming_transmission(self, genesis_block: Block) -> bool:
        ...
        if self.conn_bc.difficulty.compression_type != genesis_block.diff.compression_type:
            logger.error("Connecting to relay with use of different compression type is not supported")
            self.close()
            return False
