...
from decentnet.consensus.difficulty_byte_sizing import T_COST_BYTE_SIZE, M_COST_BYTE_SIZE, P_COST_BYTE_SIZE, \
    ...
    EXPRESS_POW_BYTE_SIZE, EXPRESS_POW_CHUNK_SLICE
...
class Difficulty:
    ...
    def from_bytes(cls, difficulty_bytes: bytes | memoryview):
        ...
        compression_level = int.from_bytes(
            ...
            signed=False)
        express_pow = int.from_bytes(difficulty_bytes[COMPRESSION_LEVEL_CHUNK_SLICE:EXPRESS_POW_CHUNK_SLICE],
                                     byteorder=ENDIAN_TYPE,
                                     signed=False)

        return cls(t_cost, m_cost, p_cost, n_bits, hash_len_chars, compression_level, express_pow)
    ...
    def __iter__(self):
        yield from [self.t_cost, self.m_cost, self.p_cost, self.n_bits,
                    self.hash_len_chars, self.compression_level, self.express]

    def __str__(self):
        return f"{self.t_cost}:{self.m_cost}:{self.p_cost}:{self.n_bits}:{self.hash_len_chars}:{self.compression_level}:{self.express}"
