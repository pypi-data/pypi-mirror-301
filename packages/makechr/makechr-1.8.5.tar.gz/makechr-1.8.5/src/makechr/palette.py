...
class Palette(object):
  ...
  def select(self, color_needs):
    want = set([c for c in color_needs if c != 0xff])
...
class PaletteFileReader(object):
  def read(self, filename):
    ...
    fp.close()
    if len(content) >= 0x1a and content[:0x09] == b'(VALIANT)':
      short_palette = content[0x0c:0x0f]
      if short_palette == b'\x02\x10\x01':
        length = content[0x18]
        data = content[0x19:0x19 + length]
        values = [int(n) for n in data]
        pal = Palette()
        pal.set_bg_color(values[0])
        for row in [values[i*4:i*4+4] for i in range(4)]:
          if row:
            pal.add(row)
        return pal
    return None
