...
class File:
    ...
    def from_config(cls, file_: dict, context: t.Any) -> "File":
        ...
        # set default for analysis if not present
        parents_.setdefault("analysis", None)
    ...
    def _construct_fw_path(self) -> str:
        ...
        if not self.parents:
            raise ValueError(
                "Missing parents information when constructing Flywheel path"
            )
        if not all(key in self.parents for key in FLYWHEEL_HIERARCHY):
            raise ValueError(
                f"Missing hierarchy information when constructing Flywheel path. "
                f"Expected: {FLYWHEEL_HIERARCHY}"
            )
