...
class TcpUtility(Utility):
    ...
    def __init__(self, password=None, timeout=900.0):
        ...
        self.__connection = TcpConnection(
            self.__poller,
            on_disconnected=self.__onDisconnected,
            on_message_received=self.__onMessageReceived,
            on_connected=self.__onConnected,
            timeout=timeout,
        )
        if password is not None:
            self.__connection.encryptor = get_encryptor(password)
