    ...
    **request_kwargs, 
    ...
    def gen_step():
        ...
        def process(info: dict, /) -> AttrDict:
            ...
            return attr
        if async_:
            if need_to_find is None:
                yield YieldFrom(async_map(process, it), identity=True)
            else:
                async def request():
                ...
                yield YieldFrom(request(), identity=True)
        else:
            it = cast(Iterator, it)
            if need_to_find is None:
                yield YieldFrom(map(process, it), identity=True)
            else:
                for attr in map(process, it):
                ...
                try:
                    if len(find_ids) <= len(id_to_dirnode) // page_size:
                        ...
                        while ids := ",".join(map(str, islice(ids_it, 10_000))):
                            # NOTE: 批量给目录添加星标，这样便于吧这些目录进行批量拉取
