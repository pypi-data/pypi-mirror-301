...
class ProductPlatformSyncTask(BigIntAuditBase):
    __table_args__ = (
        Index("idx_product_platform_sync_task_product_platform_id_type_status", "product_platform_id", "type", "status"),
    )

    product_platform_id: Mapped[int] = Column(BIGINT, ForeignKey("product_platform.id"), nullable=False)
    task_type: Mapped[SyncTaskType] = Column(SyncTaskType, nullable=False)
    task_meta: Mapped[dict] = Column(JSONBDict, nullable=True)
    status: Mapped[SyncTaskStatus] = Column(SyncTaskStatus, nullable=False)
