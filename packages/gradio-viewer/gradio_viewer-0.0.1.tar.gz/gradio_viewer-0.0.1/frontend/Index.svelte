...
<script lang="ts">
	...
	import { Block, BlockLabel, UploadText } from "@gradio/atoms";
	...
	async function show_pdf(file) {
		...
		viewer.innerHTML = '';
		const blinking = document.getElementById('viewer-blinker');
...
<Block {visible} {elem_id} {elem_classes} {container} {scale} {min_width} {height}>
    ...
    {#if file}
		<div class="viewer-container">
			<div id="viewer-blinker" style="display: none">
			...
			<div class="button-row">
				{#if Array.isArray(value) && value.length > 1}
					<button class="next-button" on:click={previous_file}>
						<svg width="18" height="18" viewBox="0 0 18 18" xmlns="http://www.w3.org/2000/svg">
							<polygon points="14.4,1.8 3.6,9 14.4,16.2" fill="none" stroke-width="1.5" stroke-linejoin="round" stroke-linecap="round" />
						  </svg>
					</button>
				{/if}
				...
				{#if Array.isArray(value) && value.length > 1}
					<button class="next-button" on:click={next_file}>
						<svg width="18" height="18" viewBox="0 0 18 18" xmlns="http://www.w3.org/2000/svg">
							<polygon points="3.6,1.8 14.4,9 3.6,16.2" fill="none" stroke="#F5F5F5" stroke-width="1.5" stroke-linejoin="round" stroke-linecap="round" />
						</svg>
					</button>
				{/if}
			</div>
