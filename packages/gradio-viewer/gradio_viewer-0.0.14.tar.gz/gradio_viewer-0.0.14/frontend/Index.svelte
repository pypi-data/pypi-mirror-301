...
<script lang="ts">
	...
	$: if (value && (!Array.isArray(value) || value.length > 0)) {
		...
		if (value && JSON.stringify(file_to_show(value, n)) !== JSON.stringify(old_file)) {
			...
			file = add_url(file_to_show(value, n), root);
			console.log(file);
    ...
    async function show_error(error: string) {
	...
	function add_url(file: FileData, root: string) {
			if (file) {
	...
	async function show_pdf(file: FileData) {
	...
	async function show_html(file: FileData) {
		await tick(); // Attend la fin du rendu
	...
	async function show_image(file: FileData) {
		await tick(); // Attend la fin du rendu
		viewer.innerHTML = ''; // Vide le conteneur
		blinking.style.display = 'block';
	...
	async function show_markdown(file: FileData) {
		await tick(); // Attend la fin du rendu
		viewer.innerHTML = ''; // Vide le conteneur
		blinking.style.display = 'block';
