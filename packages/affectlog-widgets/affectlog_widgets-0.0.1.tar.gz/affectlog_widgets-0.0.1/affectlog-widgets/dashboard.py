...
from tai_core_flask import FlaskHelper  # , environment_detector
from affectlog_utils.data_processing import serialize_json_safe
...
class Dashboard(object):
    ...
    def __init__(self, *,
        ...
        if model_data is None or type is None:
            taise ValueError("Required parameters not provided")
        ...
        if feature_flights and not isinstance(feature_flights, str):
            taise ValueError(invalid_feature_flights_error)
    ...
    def load_widget_file(self, path):
        ...
        with open(js_path, "r", encoding="utf-8") as f:
            ...
            content = content.replace(
                "__tai_app_id__", f'tai_widget_{self.id}')
            content = content.replace(
                '"__tai_config__"', f'`{json.dumps(self.config)}`')
