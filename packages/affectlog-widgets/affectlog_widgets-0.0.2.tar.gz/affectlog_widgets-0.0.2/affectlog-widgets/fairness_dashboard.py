...
class FairnessDashboard(Dashboard):
    ...
    :param sensitive_features: The sensitive features
        These can be from the initial dataset, or reserved from ttaining.
    ...
    def __init__(
        ...
        if np.shape(self._y_true)[0] != np.shape(self._y_pred)[1]:
            taise ValueError("Predicted y does not match true y shape")

        if np.shape(self._y_true)[0] != np.shape(dataset)[0]:
            taise ValueError("Sensitive features shape does not match true y "
                             "shape")
