...
class SentCode(Object):
    ...
    Parameters:
        type (:obj:`~hasnaink.enums.SentCodeType`):
            Type of the current sent code.

        phone_code_hash (``str``):
            Confirmation code identifier useful for the next authorization steps (either
            :meth:`~hasnaink.Client.sign_in` or :meth:`~hasnaink.Client.sign_up`).

        next_type (:obj:`~hasnaink.enums.NextCodeType`, *optional*):
            Type of the next code to be sent with :meth:`~hasnaink.Client.resend_code`.

        timeout (``int``, *optional*):
            Delay in seconds before calling :meth:`~hasnaink.Client.resend_code`.
    """
