...
# Originated from https://github.com/furiosa-ai/npu-tools/blob/2d9194c0/crates/furiosa-torch/furiosa/torch/preprocess.py
...
def preprocess(gm: GraphModule, inputs: Sequence, decomposition_table=STD_DECOMPOSITIONS):
    ...
    traced_gm = do_make_fx(gm, fake_inputs, decomposition_table)
...
def eliminate_dead_code(graph: Graph) -> None:
    for node in reversed(graph.nodes):
        if len(node.users) > 0:
            continue
        # ``torch.ops.aten.copy_.default`` is registered as impure ops. so it's never removed by `Graph.eliminate_dead_code`.
        # But we want to remove it when there's no user of it.
        if (
            node.op == 'call_function' and node.target == torch.ops.aten.copy_.default
        ) or not node.is_impure():
