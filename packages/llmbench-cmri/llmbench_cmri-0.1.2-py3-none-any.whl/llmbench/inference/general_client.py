...
class GeneralClient:
    ...
    def get_client(self):
        ...
        if client is None:
            ...
            if os.path.exists(self.model):
                if "llmbench_client" in self.model:
                    self.is_self_client = True
                client = self._get_file_client(self.model)
            else:
                client = self._get_eval_client(self.model)
        if client is None:
            raise ValueError(f"Could not find client for model: {self.model} or file_path:{self.model} or Model_Name is {self.model}")
    ...
    def _get_self_client(self, model: str):
        ...
        if model in load_client_info:
            ...
            logger.warning(f"Could not find client for model in llmbench: {model}")
