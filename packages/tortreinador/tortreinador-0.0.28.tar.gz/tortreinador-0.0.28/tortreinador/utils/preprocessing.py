...
def load_data(data: pd.DataFrame, input_parameters: list, output_parameters: list,
              ...
              if_double: bool = False, add_noise: bool = False, error_rate: list = None):
    """
    ...
    if add_noise:
        ...
        for i in range(len(input_parameters)):
            train_x[i + len(input_parameters) + 1] = train_x_noise.loc[:, i]
            val_x[i + len(input_parameters) + 1] = val_x_noise.loc[:, i]
            test_x[i + len(input_parameters) + 1] = test_x_noise.loc[:, i]
