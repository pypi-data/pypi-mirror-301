...
class TorchTrainer:
    ...
    def _get_loss_weight(self, mode):
        if mode == 't' and self.event_occurs:
            return self.lambda_factor

        return 1

    def _get_loss_aux(self, mode, x, y):
        if mode == 't' and self.event_occurs:
            pi_aux, mu_aux, sig_aux = self.model(x)
            return self.criterion(pi_aux, mu_aux, sig_aux, y)

        return 0

    def _get_idx_size(self, mode):
        if mode == 't' and self.event_occurs:
...
def config_generator(model_save_path: str, warmup_epochs: int = None, lr_milestones: list = None,
                     ...
                     warmup_error: int = None, event_rate: float = None, error: object = None, lambda_factor: float = 0.7):
    """
