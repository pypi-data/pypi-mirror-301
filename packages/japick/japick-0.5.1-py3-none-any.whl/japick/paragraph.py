...
class Paragraph:
    ...
    def as_original_pos(self, index, lazy=False) -> Pos:
        ...
        chunks = line[left_margin:].replace(INLINE_RETURN_SYMBOL, NULL_SYMBOL).split(NULL_SYMBOL)
        num_null_symbols = 0
        current = 0
        for num_null_symbols, chunk in enumerate(chunks):
            ...
            if (not lazy and pos.ch < current) or (lazy and pos.ch <= current):
                break
        pos.ch += left_margin + num_null_symbols + (self.pos.ch if pos.line == 0 else 0)
        pos.line += self.pos.line

        # Count inline breaks as line breaks.
