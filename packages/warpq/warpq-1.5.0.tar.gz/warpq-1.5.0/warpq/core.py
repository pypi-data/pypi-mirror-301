...
class warpqMetric:
    ...
    def evaluate_from_csv1(
        ...
        def process_row(row):
            ref_wave, deg_wave = row[ref_wave_col], row[deg_wave_col]
            return self.evaluate(ref_wave, deg_wave, verbose=False)

        # Use joblib to parallelize the processing
        results = Parallel(n_jobs=self.n_jobs)(
            delayed(process_row)(row) for _, row in tqdm(df.iterrows(), total=total_files, desc=f"Computing WARP-Q scores ({self.n_jobs} jobs)")
        )
