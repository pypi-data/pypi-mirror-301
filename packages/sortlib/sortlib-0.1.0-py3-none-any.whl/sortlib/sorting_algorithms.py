...
def sort_numbers(input_data: Union[str, List[int]], algorithm: str = 'merge', output_format: str = 'brackets', output_path: str = None) -> List[int]:
    if isinstance(input_data, str):
        numbers = read_numbers(input_data)
    elif isinstance(input_data, list):
        numbers = input_data
    else:
        raise ValueError("Неправильный формат входных данных. Используйте путь к файлу или список чисел.")
    ...
    if algorithm not in algorithms:
        raise ValueError(f"Неподдерживаемый алгоритм. Выберите из: {', '.join(algorithms.keys())}")
    ...
    if output_path:
        ...
        print(f"Отсортированный список сохранен в {output_path}")
    else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Сортировка чисел с выбором алгоритма и формата вывода")
    parser.add_argument("input", help="Входной файл или список чисел")
    parser.add_argument("-a", "--algorithm", choices=['merge', 'quick', 'bubble', 'insertion', 'selection', 'heap', 'shell', 'counting', 'radix', 'bucket', 'comb', 'cocktail', 'gnome', 'cycle', 'pigeonhole', 'strand', 'pancake', 'bogo', 'stooge', 'tim'],
                        default='merge', help="Алгоритм сортировки (по умолчанию: merge)")
    parser.add_argument("-f", "--format", choices=['brackets', 'curly_braces', 'parentheses', 'no_commas', 'spaces', 'vertical', 'horizontal', 'csv', 'tab_separated', 'json', 'pretty_json', 'bullet_points', 'numbered_list', 'html_list', 'xml', 'yaml', 'markdown_table', 'latex_array', 'binary', 'hexadecimal', 'scientific_notation', 'percentage'],
                        default='brackets', help="Формат вывода (по умолчанию: brackets)")
    ...
    sort_numbers(input_data, algorithm=args.algorithm, output_format=args.format, output_path=args.output)