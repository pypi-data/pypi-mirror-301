...
class PdfFileInfo:
    ...
    def __str__(self):
        return f"prefix={self.prefix}, files={list(x.name for x in self.files)}, children={self.children}"
...
def search_directory(search_dir: pathlib.Path, root_dir: pathlib.Path) -> PdfFileInfo:
    """搜索目录下的所有pdf文件"""
...
def merge_file_info(info: PdfFileInfo, root_dir: pathlib.Path, writer: pypdf.PdfWriter):
    """按照 PdfFileInfo 进行合并"""
    page_num = 0
    if info.prefix:
    ...
    for pdf_filepath in info.files:
        with open(pdf_filepath.as_posix(), "rb") as pdf_file:
            ...
            writer.add_outline_item(pdf_filepath.stem, page_num, parent=root_bookmark)
...
def main():
    ...
    pdf_dir = pathlib.Path(directory) if directory != "." else pathlib.Path.cwd()
