...
def main():
    ...
    group.add_argument("-f", dest="files", type=str, nargs="+", help=doc["files"])
    group.add_argument("-c", dest="config", type=str, help=doc["config"])
    parser.add_argument(
        "-d", dest="destination", type=str, required=True, help=doc["destination"]
    )
    parser.add_argument(
        "-n", dest="count", type=int, required=True, default=5, help=doc["count"]
    )
    ...
    if opt_config:
        ...
        if path_config.exists():
            ...
            with open(str(path_config), "r", encoding="utf-8") as f:
                content = [line.strip() for line in f.readlines()]
                targets = [
                    pathlib.Path(fn) for fn in content if pathlib.Path(fn).exists()
                ]
        else:
            print(f"config file {path_config} is not valid.")
            sys.exit(-1)
    elif opt_files:
        targets = [pathlib.Path(f) for f in opt_files if pathlib.Path(f).exists()]
    else:
