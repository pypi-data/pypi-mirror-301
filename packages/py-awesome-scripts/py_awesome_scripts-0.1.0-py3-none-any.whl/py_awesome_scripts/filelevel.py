...
# 文件级别定义
FileLevel = typing.NamedTuple(
    "FileLevel", [("code", int), ("names", typing.Tuple[str, ...])]
)
...
def add_level_mark(filepath: pathlib.Path, filelevel: int, suffix: int) -> pathlib.Path:
...
def main():
    ...
    for target in targets:
        print(
            f'start processing {"file" if target.is_file() else "folder":>6s} [{str(target)}]...'
        )
