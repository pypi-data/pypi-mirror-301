...
``<Work-environment>/ Coalispr / logs / <EXP> / run-log.txt``
...
Place a link/copy of ``coalispr.resources.numeric.py`` in ``python3/site_packages/pandas/core/indexes/`` (see :ref:`here <nummod>` in :doc:`/errors`).
