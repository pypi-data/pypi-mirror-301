<template>
  <div class="order-sidebar flex bg-white rounded-md" :class="{ 'is-sidebar-closed': isSidebarClosed }">
    <AddNoteModal :isOpen="isModalOpened" ref="noteInput" @modal-close="isModalOpened = false" @modal-submit=""
      name="note-modal" />
    <div class="order-sidebar-content w-full flex flex-col">
      ...
      <ScrollBar class="my-[0.75rem]">
        <div class="order-sidebar-content-data px-[0.75rem] flex flex-col gap-3 max-h-full"
          v-if="activeTab.name === 'Notes'">
          <div class="order-sidebar-content-data-note border-0 rounded-lg p-[0.5rem] flex flex-col"
            v-for="note in orderNotes" :key="note.id">
