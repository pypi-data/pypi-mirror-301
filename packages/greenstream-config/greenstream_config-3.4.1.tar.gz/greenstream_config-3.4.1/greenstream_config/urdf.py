...
def get_camera_urdf(
    ...
    if not has_duplicate_camera_link:
        ...
        links.append(
            Link(name=f"{camera.name}_link", inertial=None, visuals=None, collisions=None)
        )
        joints.append(
            Joint(
        ...
        if camera.ptz:
            for ptz_component in camera.ptz.ptz_components:
                ...
                if ptz_component.move_type == "pan":
                    ...
                    joints.append(
                        Joint(
                            name=f"tilt_joint_{camera.name}",
                            parent=parent_link,
    ...
    if add_optical_frame:
        ...
        if has_duplicate_camera_link:
            ...
            for joint in reversed(joints):
                if (
                    joint.child.split("_")[0] == camera.name
                    and joint.child.split("_")[2] == "frame"
                ):
    ...
    for camera in cameras:
        ...
        if camera.name in [prev_camera.name for prev_camera in cameras[: cameras.index(camera)]]:
            links, joints = get_camera_urdf(
                camera, links, joints, add_optical_frame, has_duplicate_camera_link=True
            )
        else:
            links, joints = get_camera_urdf(
                camera, links, joints, add_optical_frame, has_duplicate_camera_link=False
            )
