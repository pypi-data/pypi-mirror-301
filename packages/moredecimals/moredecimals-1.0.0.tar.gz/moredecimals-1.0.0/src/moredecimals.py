_d='x // 0'
_c='INF % x'
_b='strict semantics for mixing floats and Decimals are enabled'
_a='rounding'
_Z='capitals'
_Y='moredecimals'
_X='align'
_W='fill'
_V='inf'
_U='cannot round an infinity'
_T='cannot round a NaN'
_S='NaN'
_R='decimal_point'
_Q='grouping'
_P='minimumwidth'
...
try:from collections import namedtuple as _namedtuple;DecimalTuple=_namedtuple('DecimalTuple','sign digits exponent',module=_Y)
...
class Clamped(DecimalException):0
...
class DivisionUndefined(InvalidOperation,ZeroDivisionError):
	def handle(self,context,*args):return _NaN
class Inexact(DecimalException):0
class InvalidContext(InvalidOperation):
	def handle(self,context,*args):return _NaN
class Rounded(DecimalException):0
class Subnormal(DecimalException):0
class Overflow(Inexact,Rounded):
	def handle(self,context,sign,*args):
		...
		if sign==1:
			...
			return _dec_from_triple(sign,'9'*context.prec,context.Emax-context.prec+1)
class Underflow(Inexact,Rounded,Subnormal):0
class FloatOperation(DecimalException,TypeError):0
...
_context_attributes=frozenset(['prec','Emin','Emax',_Z,'clamp',_a,'flags','traps'])
def getcontext():
	try:return _current_context_var.get()
	except LookupError:context=Context();_current_context_var.set(context);return context
def setcontext(context):
	if context in(DefaultContext,BasicContext,ExtendedContext):context=context.copy();context.clear_flags()
	_current_context_var.set(context)
del contextvars
def localcontext(ctx=_A,**kwargs):
	if ctx is _A:ctx=getcontext()
	ctx_manager=_ContextManager(ctx)
	for(key,value)in kwargs.items():
		if key not in _context_attributes:raise TypeError(f"'{key}' is an invalid keyword argument for this function")
...
class Decimal:
	...
	def __new__(cls,value=_C,context=_A):
		...
		if isinstance(value,float):
			if context is _A:context=getcontext()
			context._raise_error(FloatOperation,_b);value=Decimal.from_float(value);self._exp=value._exp;self._sign=value._sign;self._int=value._int;self._is_special=value._is_special;return self
		raise TypeError('Cannot convert %r to Decimal'%value)
	@classmethod
	def from_float(cls,f):
		if isinstance(f,int):sign=0 if f>=0 else 1;k=0;coeff=str(abs(f))
		elif isinstance(f,float):
			if _math.isinf(f)or _math.isnan(f):return cls(repr(f))
			if _math.copysign(1.,f)==1.:sign=0
			else:sign=1
			n,d=abs(f).as_integer_ratio();k=d.bit_length()-1;coeff=str(n*5**k)
		else:raise TypeError('argument must be int or float.')
	...
	def _compare_check_nans(self,other,context):
		...
		return 0
	def __bool__(self):return self._is_special or self._int!=_C
	def _cmp(self,other):
		if self._is_special or other._is_special:
	...
	def __hash__(self):
		if self._is_special:
			...
			elif self.is_nan():return object.__hash__(self)
	...
	def as_integer_ratio(self):
		...
		if self._sign:n=-n
		return n,d
	def __repr__(self):return"Decimal('%s')"%str(self)
	...
	def to_eng_string(self,context=_A):return self.__str__(eng=_B,context=context)
	...
	def __divmod__(self,other,context=_A):
		...
		if self._isinfinity():
			...
			else:return _SignedInfinity[sign],context._raise_error(InvalidOperation,_c)
		if not other:
			if not self:ans=context._raise_error(DivisionUndefined,'divmod(0, 0)');return ans,ans
			else:return context._raise_error(DivisionByZero,_d,sign),context._raise_error(InvalidOperation,'x % 0')
	...
	def __mod__(self,other,context=_A):
		...
		ans=self._check_nans(other,context)
		if ans:return ans
		if self._isinfinity():return context._raise_error(InvalidOperation,_c)
		elif not other:
	...
	def __floordiv__(self,other,context=_A):
		...
		if not other:
			if self:return context._raise_error(DivisionByZero,_d,self._sign^other._sign)
			else:return context._raise_error(DivisionUndefined,'0 // 0')
	...
	def _fix_nan(self,context):
		payload=self._int;max_payload_len=context.prec-context.clamp
	...
	def _fix(self,context):
		...
		exp_min=len(self._int)+self._exp-context.prec;self_is_subnormal=exp_min<Etiny
	...
	def __round__(self,n=_A):
		...
		if self._is_special:
			if self.is_nan():raise ValueError(_T)
			else:raise OverflowError(_U)
	...
	def _power_exact(self,other,p):
		x=_WorkRep(self);xc,xe=x.int,x.exp
		...
		if y.sign==1:
			...
			strxc=str(xc)
			if len(strxc)>p:return
			xe=-e-xe;return _dec_from_triple(0,strxc,xe)
		if ye>=0:m,n=yc*10**ye,1
		else:
			if xe!=0 and len(str(abs(yc*xe)))<=-ye:return
			xc_bits=_nbits(xc)
			if len(str(abs(yc)*xc_bits))<=-ye:return
			m,n=yc,10**-ye
		...
		xc=xc**m;xe*=m;str_xc=str(xc)
		if len(str_xc)>p:return
	...
	def __pow__(self,other,modulo=_A,context=_A):
		...
		if exact and not other._isinteger():
			...
			if newcontext.flags[Overflow]:context._raise_error(Overflow,'above Emax',ans._sign)
	...
	def compare_total(self,other,context=_A):
		...
		return _Zero
	def compare_total_mag(self,other,context=_A):other=_convert_other(other,raiseit=_B);s=self.copy_abs();o=other.copy_abs();return s.compare_total(o)
	...
	def copy_sign(self,other,context=_A):other=_convert_other(other,raiseit=_B);return _dec_from_triple(other._sign,self._int,self._exp,self._is_special)
	...
	def is_canonical(self):return _B
	def is_finite(self):return not self._is_special
	def is_infinite(self):return self._exp==_K
	def is_nan(self):return self._exp in(_G,_J)
	def is_normal(self,context=_A):
		if self._is_special or not self:return _D
		if context is _A:context=getcontext()
		return context.Emin<=self.adjusted()
	def is_qnan(self):return self._exp==_G
	def is_signed(self):return self._sign==1
	def is_snan(self):return self._exp==_J
	def is_subnormal(self,context=_A):
		if self._is_special or not self:return _D
		if context is _A:context=getcontext()
		return self.adjusted()<context.Emin
	def is_zero(self):return not self._is_special and self._int==_C
	...
	def __format__(self,specifier,context=_A,_localeconv=_A):
		...
		rounding=context.rounding;precision=spec[_L]
		...
		if not self and spec['no_neg_0']and self._sign:adjusted_sign=0
		else:adjusted_sign=self._sign
		leftdigits=self._exp+len(self._int)
		if spec[_F]in'eE':
		...
		exp=leftdigits-dotplace;return _format_number(adjusted_sign,intpart,fracpart,exp,spec)
def _dec_from_triple(sign,coefficient,exponent,special=_D):self=object.__new__(Decimal);self._sign=sign;self._int=coefficient;self._exp=exponent;self._is_special=special;return self
...
class Context:
	...
	def __setattr__(self,name,value):
		if name=='prec':return self._set_integer_check(name,value,1,_V)
		elif name=='Emin':return self._set_integer_check(name,value,'-inf',0)
		elif name=='Emax':return self._set_integer_check(name,value,0,_V)
		elif name==_Z:return self._set_integer_check(name,value,0,1)
	...
	def _ignore_flags(self,*flags):self._ignored_flags=self._ignored_flags+list(flags);return list(flags)
	def _regard_flags(self,*flags):
		if flags and isinstance(flags[0],(tuple,list)):flags=flags[0]
		for flag in flags:self._ignored_flags.remove(flag)
	__hash__=_A
	def Etiny(self):return int(self.Emin-self.prec+1)
	...
	def create_decimal(self,num=_C):
		...
		return d._fix(self)
	def create_decimal_from_float(self,f):d=Decimal.from_float(f);return d._fix(self)
	def abs(self,a):a=_convert_other(a,raiseit=_B);return a.__abs__(context=self)
	def add(self,a,b):
	...
	def canonical(self,a):
		...
		return a.canonical()
	def compare(self,a,b):a=_convert_other(a,raiseit=_B);return a.compare(b,context=self)
	def compare_signal(self,a,b):a=_convert_other(a,raiseit=_B);return a.compare_signal(b,context=self)
	def compare_total(self,a,b):a=_convert_other(a,raiseit=_B);return a.compare_total(b)
	def compare_total_mag(self,a,b):a=_convert_other(a,raiseit=_B);return a.compare_total_mag(b)
	def copy_abs(self,a):a=_convert_other(a,raiseit=_B);return a.copy_abs()
	def copy_decimal(self,a):a=_convert_other(a,raiseit=_B);return Decimal(a)
	def copy_negate(self,a):a=_convert_other(a,raiseit=_B);return a.copy_negate()
	def copy_sign(self,a,b):a=_convert_other(a,raiseit=_B);return a.copy_sign(b)
	def divide(self,a,b):
	...
	def exp(self,a):a=_convert_other(a,raiseit=_B);return a.exp(context=self)
	def fma(self,a,b,c):a=_convert_other(a,raiseit=_B);return a.fma(b,c,context=self)
	def is_canonical(self,a):
		if not isinstance(a,Decimal):raise TypeError('is_canonical requires a Decimal as an argument.')
		return a.is_canonical()
	def is_finite(self,a):a=_convert_other(a,raiseit=_B);return a.is_finite()
	def is_infinite(self,a):a=_convert_other(a,raiseit=_B);return a.is_infinite()
	def is_nan(self,a):a=_convert_other(a,raiseit=_B);return a.is_nan()
	def is_normal(self,a):a=_convert_other(a,raiseit=_B);return a.is_normal(context=self)
	def is_qnan(self,a):a=_convert_other(a,raiseit=_B);return a.is_qnan()
	def is_signed(self,a):a=_convert_other(a,raiseit=_B);return a.is_signed()
	def is_snan(self,a):a=_convert_other(a,raiseit=_B);return a.is_snan()
	def is_subnormal(self,a):a=_convert_other(a,raiseit=_B);return a.is_subnormal(context=self)
	def is_zero(self,a):a=_convert_other(a,raiseit=_B);return a.is_zero()
	def ln(self,a):a=_convert_other(a,raiseit=_B);return a.ln(context=self)
	def log10(self,a):a=_convert_other(a,raiseit=_B);return a.log10(context=self)
	def logb(self,a):a=_convert_other(a,raiseit=_B);return a.logb(context=self)
	def logical_and(self,a,b):a=_convert_other(a,raiseit=_B);return a.logical_and(b,context=self)
	def logical_invert(self,a):a=_convert_other(a,raiseit=_B);return a.logical_invert(context=self)
	def logical_or(self,a,b):a=_convert_other(a,raiseit=_B);return a.logical_or(b,context=self)
	def logical_xor(self,a,b):a=_convert_other(a,raiseit=_B);return a.logical_xor(b,context=self)
	def max(self,a,b):a=_convert_other(a,raiseit=_B);return a.max(b,context=self)
	def max_mag(self,a,b):a=_convert_other(a,raiseit=_B);return a.max_mag(b,context=self)
	def min(self,a,b):a=_convert_other(a,raiseit=_B);return a.min(b,context=self)
	def min_mag(self,a,b):a=_convert_other(a,raiseit=_B);return a.min_mag(b,context=self)
	def minus(self,a):a=_convert_other(a,raiseit=_B);return a.__neg__(context=self)
	def multiply(self,a,b):
		a=_convert_other(a,raiseit=_B);r=a.__mul__(b,context=self)
		if r is NotImplemented:raise TypeError(_I%b)
		else:return r
	def next_minus(self,a):a=_convert_other(a,raiseit=_B);return a.next_minus(context=self)
	def next_plus(self,a):a=_convert_other(a,raiseit=_B);return a.next_plus(context=self)
	def next_toward(self,a,b):a=_convert_other(a,raiseit=_B);return a.next_toward(b,context=self)
	def normalize(self,a):a=_convert_other(a,raiseit=_B);return a.normalize(context=self)
	def number_class(self,a):a=_convert_other(a,raiseit=_B);return a.number_class(context=self)
	def plus(self,a):a=_convert_other(a,raiseit=_B);return a.__pos__(context=self)
	def power(self,a,b,modulo=_A):
		a=_convert_other(a,raiseit=_B);r=a.__pow__(b,modulo,context=self)
		if r is NotImplemented:raise TypeError(_I%b)
		else:return r
	def quantize(self,a,b):a=_convert_other(a,raiseit=_B);return a.quantize(b,context=self)
	def radix(self):return Decimal(10)
	def remainder(self,a,b):
		a=_convert_other(a,raiseit=_B);r=a.__mod__(b,context=self)
		if r is NotImplemented:raise TypeError(_I%b)
		else:return r
	def remainder_near(self,a,b):a=_convert_other(a,raiseit=_B);return a.remainder_near(b,context=self)
	def rotate(self,a,b):a=_convert_other(a,raiseit=_B);return a.rotate(b,context=self)
	def same_quantum(self,a,b):a=_convert_other(a,raiseit=_B);return a.same_quantum(b)
	def scaleb(self,a,b):a=_convert_other(a,raiseit=_B);return a.scaleb(b,context=self)
	def shift(self,a,b):a=_convert_other(a,raiseit=_B);return a.shift(b,context=self)
	def sqrt(self,a):a=_convert_other(a,raiseit=_B);return a.sqrt(context=self)
	def subtract(self,a,b):
		a=_convert_other(a,raiseit=_B);r=a.__sub__(b,context=self)
		if r is NotImplemented:raise TypeError(_I%b)
		else:return r
	def to_eng_string(self,a):a=_convert_other(a,raiseit=_B);return a.to_eng_string(context=self)
	def to_sci_string(self,a):a=_convert_other(a,raiseit=_B);return a.__str__(context=self)
	def to_integral_exact(self,a):a=_convert_other(a,raiseit=_B);return a.to_integral_exact(context=self)
	def to_integral_value(self,a):a=_convert_other(a,raiseit=_B);return a.to_integral_value(context=self)
	to_integral=to_integral_value
class _WorkRep:
...
def _sqrt_nearest(n,a):
	...
	return a
def _rshift_nearest(x,shift):b,q=1<<shift,x>>shift;return q+(2*(x&b-1)+(q&1)>b)
...
def _iexp(x,M,L=8):
	R=_nbits((x<<L)//M);T=-int(-10*len(str(M))//(3*L));y=_div_nearest(x,T);Mshift=M<<R
	...
	return M+y
def _dexp(c,e,p):
	p+=2;extra=max(0,e+len(str(c))-1);q=p+extra;shift=e+q
...
def _convert_for_comparison(self,other,equality_op=_D):
	...
	if isinstance(other,float):
		...
		else:context._raise_error(FloatOperation,_b)
...
def _parse_format_specifier(format_spec,_localeconv=_A):
	...
	format_dict=m.groupdict();fill=format_dict[_W];align=format_dict[_X];format_dict[_O]=format_dict[_O]is not _A
	if format_dict[_O]:
	...
	format_dict[_W]=fill or' ';format_dict[_X]=align or'>'
	if format_dict[_N]is _A:format_dict[_N]='-'
	format_dict[_P]=int(format_dict[_P]or _C)
	if format_dict[_L]is not _A:format_dict[_L]=int(format_dict[_L])
	if format_dict[_L]==0:
		if format_dict[_F]is _A or format_dict[_F]in'gGn':format_dict[_L]=1
	if format_dict[_F]==_G:
		format_dict[_F]='g'
		if _localeconv is _A:_localeconv=_locale.localeconv()
		if format_dict[_M]is not _A:raise ValueError("Explicit thousands separator conflicts with 'n' type in format specifier: "+format_spec)
		format_dict[_M]=_localeconv[_M];format_dict[_Q]=_localeconv[_Q];format_dict[_R]=_localeconv[_R]
	else:
		if format_dict[_M]is _A:format_dict[_M]=''
		format_dict[_Q]=[3,0];format_dict[_R]='.'
	return format_dict
def _format_align(sign,body,spec):
	minimumwidth=spec[_P];fill=spec[_W];padding=fill*(minimumwidth-len(sign)-len(body));align=spec[_X]
...
def _insert_thousands_sep(digits,spec,min_width=1):
	sep=spec[_M];grouping=spec[_Q];groups=[]
...
def _format_sign(is_negative,spec):
	...
	elif spec[_N]in' +':return spec[_N]
	else:return''
