...
class Eoq3ServerProtocol(WebSocketServerProtocol):
    ...
    def onOpen(self):
        self.factory.controller.LogInfo("WS client connected: %s"%(self.request.peer))
    ...
    def onClose(self, wasClean, code, reason):
        ...
        self.factory.controller.LogInfo("WS client disconnected: %s (clean=%s, code=%s, reason=%s)"%(client,wasClean,code,reason))
    ...
    def __InputThread(self):
        while(self.shallRun):
            try:
                ...
                print('WS input queue failed: %s'%(str(e)))
    ...
    def __AdminThread(self):
        while(self.shallRun):
            try:
                ...
                self.config.logger.Warn('Server admin queue failed: %s'%(str(e)))
    ...
    def __InputThread(self):
        while(self.shallRun):
            try:
                ...
                self.config.logger.Warn('WS input queue failed: %s'%(str(e)))
        ...
        while(self.shallRun):
            ...
            if(isNewMsg):
                ...
                try:
                    ...
                    self.config.logger.Warn('Worker thread invalid frame: %s'%(str(e)))
                #process the command
                try:
                    ...
                    msg = str(e)
                    self.config.logger.Warn('Worker thread failed: %s'%(msg))
