...
class Eoq3ClientProtocol(WebSocketClientProtocol):
    ...
    def onOpen(self):
        self.factory.client.SendAdminReplay(ADMIN_REPLIES.READY)
    ...
    def onClose(self, wasClean, code, reason):
        self.factory.client.SendAdminReplay(ADMIN_REPLIES.DISCONNECT)
    ...
    def __AdminThread(self):
        while(self.shallRun):
            try:
                ...
                if(ADMIN_REPLIES.READY == adminReply):
                    self.wsReadyEvent.set()
                elif(ADMIN_REPLIES.DISCONNECT == adminReply):
                    self.wsReadyEvent.clear()
            except queue.Empty:
                pass #wait for next frame
            except Exception as e:
                self.config.logger.Error('Domain admin queue failed: %s'%(str(e)))
        pass
    ...
    def __InputThread(self):
        while(self.shallRun):
            try:
                ...
                self.config.logger.Error('WS input queue failed: %s'%(str(e)))
