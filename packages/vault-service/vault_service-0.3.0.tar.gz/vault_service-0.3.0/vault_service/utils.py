...
def store_secret(tenant_id: str, secret_name: str, secret_data: SecretData):
    try:
        ...
        vault_controller.store_secret(tenant_id, secret_name, secret_data.model_dump())
...
def get_secret(tenant_id: str, secret_name: str):
    try:
        vault_controller = VaultController()
        if vault_controller is None:
            return Response(status="error", message="VaultController not initialized").dict()
        secret = vault_controller.get_secret(tenant_id, secret_name)
        if secret is None:
...
def update_secret(tenant_id: str, secret_name: str, secret_data: SecretData):
    try:
        vault_controller = VaultController()
        if vault_controller is None:
            return Response(status="error", message="VaultController not initialized").dict()
        resp_dict = vault_controller.update_secret(tenant_id, secret_name, secret_data.model_dump())
...
def delete_secret(tenant_id: str, secret_name: str):
    try:
        vault_controller = VaultController()
        if vault_controller is None:
            return Response(status="error", message="VaultController not initialized").dict()
        resp_dict = vault_controller.delete_secret(tenant_id, secret_name)
        if "status" in resp_dict:
            return Response(status="error", message=resp_dict["message"]).dict()
