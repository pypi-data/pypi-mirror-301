...
class MapValued( Valued, adict ):
  ...
  def __init__( self,
    ...
    adict_struct.__init__( self, _val )
  ...
  def __setitem__( self, key, val ):
    ...
    adict.__setitem__( self, key, val )
  ...
  def __str__( self ):
    ...
    return f"{type(self).__name__}({odict(self)}"
  ...
  def _encode( self ):
    ...
    if self._schema is not None:
      return self._schema.encode( dict(self), self._loc )

    return odict([
      ( k, v._encode if ( is_valued_type(v) or is_evaluated(v) ) else v )
