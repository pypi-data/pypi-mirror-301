...
class AutomaxiaApi:
    ...
    def get_controle_execucao_dash(self, tarefa_id, cliente_id, tx_chave, dt_inicio, dt_fim, tx_descricao, tx_situacao, pagina, tamanho_pagina):
        url = str(self.url)+f"/controleexecucao?tarefa_id={tarefa_id}&cliente_id={cliente_id}&tx_chave={tx_chave}&dt_inicio={dt_inicio}&dt_fim={dt_fim}&tx_descricao={tx_descricao}&tx_situacao={tx_situacao}&pagina={pagina}&tamanho_pagina={tamanho_pagina}"
        response = self.get(url)
        return response
