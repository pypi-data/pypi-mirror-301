...
def list_tasks(split="test_iid", cache_dir="./bg_wl_data", test_json_path=None):
    ...
    if not test_json_path.exists():
        raise FileNotFoundError(
            f"Test json file not found. Please make sure the file {test_json_path} exists."
        )
    ...
    print("Found", len(tasks), "tasks in the WebLINX environment.")
    return tasks
...
def preprocess_extra_props(extra_props):
    ...
    for key in extra_props:
        ...
        extra_props[key]["set_of_marks"] = 1 if width * height >= 18 else 0
...
class WeblinxTasks:
    ...
    def step(self, action, *args, **kwargs):
        ...
        logger.debug(f"ref_action:  {ref_action['intent']}, args={ref_action['args']}")
        logger.debug("-" * 80)
...
class WeblinxEnv(BrowserEnv):
    def __init__(
        ...
        # print("WeblinxEnv init, task_kwargs:", task_kwargs)
        super().__init__(
...
def register_weblinx_tasks(
    ...
    print("Registering Weblinx tasks...")
    # reset ALL_WEBLINX_TASK_IDS
    ALL_WEBLINX_TASK_IDS.clear()

    tasks = list_tasks(split=split, test_json_path=test_json_path)
    ...
    print("Weblinx tasks registered.")
