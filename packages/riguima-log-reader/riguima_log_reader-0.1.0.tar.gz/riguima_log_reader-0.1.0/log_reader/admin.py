...
class FileLogReaderAdmin(admin.ModelAdmin):
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        is_valid, file_contents = read_file_lines(file_name=filename, search=search, tenant=request.tenant)
        if not is_valid:
            self.message_user(request, file_contents, level=messages.ERROR)
        log_files = get_log_files(settings.LOG_READER_DIR_PATH, request.tenant)
