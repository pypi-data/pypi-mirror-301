...
class FuncListCheckPage:
    ...
    def open_list(self, list_pos, next_pos, fun_name="air", ocrPlus=False):
        ...
        def _open_list():
            if operate(fun_name).wait_element_appear(list_pos, ocrPlus=ocrPlus):
                ...
                log.error('无法找到功能按钮')
                raise TargetNotFoundError('找不到功能元素')
    ...
    def check_scenes(self, scenes_pos, wait_pos, fun_name="air"):
        ...
        def _check_scenes():
            if operate(fun_name).wait_element_appear(scenes_pos):
                operate(fun_name).click(scenes_pos)
            else:
                log.info("跳转场景按钮不存在")
            operate(fun_name).sleep(0.2)
...
class FuncListCheckTask(TaskCaseTemplate, FuncListCheckPage):
    ...
    def she_zhi_view(self):
        super().open_list("功能", "功能列表", ocrPlus=True)
