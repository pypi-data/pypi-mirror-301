...
from .utils import get_mlflow_experiment_link_html, get_mlflow_run_link_html
...
class TrainingRun(DeserializableModel):
    ...
    def refresh(self) -> 'TrainingRun':
        ...
        # pylint: disable-next=import-outside-toplevel,cyclic-import
        from databricks.model_training.api.foundation_model.get_runs import get_run
        return get_run(self)
    ...
    def cancel(self) -> int:
        ...
        # pylint: disable-next=import-outside-toplevel,cyclic-import
        from databricks.model_training.api.foundation_model.cancel_runs import cancel_run
        return cancel_run(self)
    ...
    def get_events(self) -> ObjectList[TrainingEvent]:
        ...
        # pylint: disable-next=import-outside-toplevel,cyclic-import
        from databricks.model_training.api.foundation_model.get_run_events import get_run_events
        return get_run_events(self)
    ...
    def _repr_html_(self) -> str:
        ...
        attr_to_label = {}
        attr_to_label['_mlflow_run_link_html'] = 'Name'
        attr_to_label['_mlflow_experiment_link_html'] = 'MLflow Experiment ID'
        keys = [
            'status',
            'details',
            'formatted_events',
        ...
        attr_to_label.update({key: ' '.join([word.title() for word in key.split('_')]) for key in keys})
    ...
    def _mlflow_experiment_link_html(self) -> str:
        ...
        """
        return get_mlflow_experiment_link_html(self.experiment_id, self.experiment_id)

    @property
    def _mlflow_run_link_html(self) -> str:
        """
        Get a hyperlinked MLflow run name. This link will open the MLflow run page in a new tab.

        Raises:
            ValueError: If the run ID or experiment ID is not set
        """
        return get_mlflow_run_link_html(self.experiment_id, self.run_id, self.name)
