...
def get_kcwebplus_cli_pid(route):
    """通过路由地址获取pid"""
    if not os.path.exists(get_kcwebplus_folder()+"/pid/"+md5(route)+"_cli_pid"):
        return False
    pid=file_get_content(get_kcwebplus_folder()+"/pid/"+md5(route)+"_cli_pid")
    return pid
def get_kcwebplus_cli_info(route):
    """通过路由地址获取进程信息"""
    pid=get_kcwebplus_cli_pid(route)
    if pid:
        try:
