...
class TrainSALT2Like:
    ...
    def __init__(self, tds, variance_model=None, calib_variance=None, color_scatter=False, dust_extinction=True):
        """
        Parameters
        ----------
        ...
        self.model = salt2.get_model(tds, dust_extinction_model=self.extinction)
    ...
    def train_salt2_model_test(self, save=False, path=None):
        ...
        Model training without any error models
        """
        first_fixed_pars = ['M0', 'M1', 'CL', 'gamma_sn', 'gamma_snake']
...
#        axes[2].plot(ii[idx], v_recal[idx], ls='', marker='.', color='r')
    ...
    def plot_control(self, path=None):
        ...
        if self.color_scatter:
            ...
            control_plot.plot_cl(self.tds, self.log, self.log[-1].ll.model.color_law, path=None)
        control_plot.plot_stacked_residuals_snake(self.tds, self.log[-1].pars, self.log[-1].v, self.log[-1].v_var, path=None)
        control_plot.plot_band_residuals_z(self.tds, self.log[-1].v, self.log[-1].v_var, path=None)
        control_plot.plot_band_residuals_phase(self.tds, self.log[-1].pars, self.log[-1].v, self.log[-1].v_var, path=None)
        control_plot.plot_band_pulls_phase(self.tds, self.log[-1].pars, self.log[-1].v, self.log[-1].v_var, path=None)
