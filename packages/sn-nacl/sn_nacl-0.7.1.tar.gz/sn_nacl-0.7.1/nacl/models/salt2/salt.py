...
#                                                    filter_basis_order)
        ...
        # filter database
        self.filter_db = self._init_filter_db(self.filter_basis,
                                              default_B="swope2::b",
                                              default_V="swope2::v")
        ...
        self.n_wl, self.n_ph = len(self.basis.bx), len(self.basis.by) # basis_knots[0],  basis_knots[1]
        self.basis_knots = self.n_wl, self.n_ph # basis_knots
        self.basis_filter_knots = len(self.filter_basis) # basis_filter_knots
    ...
    def _init_filter_db(self, filter_basis,
                        default_B="swope2::b",
                        default_V="swope2::V"):
        """
        """
        filter_db = SNFilterSet(basis=filter_basis)
        filter_db.insert(default_B, z=0.)
        filter_db.insert(default_V, z=0.)
        return filter_db
    ...
    def _init_bases(cls, tds, wl_grid=None, phase_grid=None,
        ...
        if hasattr(tds, 'basis') and tds.basis is not None:
            ...
            if wl_grid is not None:
                logging.warning(f'grid {wl_grid.min()}:{wl_grid.max()}:{len(wl_grid)} ignored')
            wl_grid = tds.basis.grid
            x_order = tds.basis.order
            logging.info(f'using the TrainingDataset basis [{wl_grid.min()},{wl_grid.max()},{len(wl_grid)},order={x_order}]')
