...
class LogLikelihood:
    ...
    def _append_to_cache(self, name, mat):
        if self._cache is None:
            return
        logging.debug(f'{name}: {mat.nnz=} {mat.shape=} {mat.nnz / np.prod(mat.shape)}')
    ...
    def __call__(self, p, deriv=False):
        ...
        logging.debug('LogLikelihood: eval model')
        ...
        if self.variance_model:
            logging.debug('LogLikelihood: eval variance model')
        ...
        logging.debug('LogLikelihood: residuals and chi2')
        res = self.y - pred
        sig = np.sqrt(var)
        w_res = res / sig
        # TODO: check: bads !
        ...
        if 'gram_matrix_mkl' in globals() and not self.force_default_spgemm:
            ...
            if len(w_J.data) == 0:
                logging.debug('LogLikelihood: w_J.data == 0 workaround')
                hess = 2. * w_J.T @ w_J
            elif 1 in w_J.shape:
                logging.debug('LogLikelihood: w_J.data workaround')
                hess = 2. * dot_product_mkl(w_J.T, w_J, reorder_output=True)
            elif False:
                logging.debug('LogLikelihood: mkl fast version')
                ...
                logging.debug(f'done. {hess.__class__} {hess.shape}')
                logging.debug('symmetrize')
        ...
        if self.variance_model is not None:
            logging.debug('variance model contribution')
            logging.debug('J_var')
