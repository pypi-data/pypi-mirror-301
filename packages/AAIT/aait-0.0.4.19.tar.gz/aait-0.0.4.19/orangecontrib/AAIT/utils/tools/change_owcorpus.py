...
def replace_owcorpus_file():
    if os.name == "nt":
        ...
        if os.path.exists(dir_path+"/lib/site-packages/orangecontrib/text/widgets"):
            if os.path.exists(owcorpus_file_path):
                os.remove(owcorpus_file_path)
                contenu = file_txt.read_text()
                file_py.write_text(contenu)
                print(f"{owcorpus_file_path} a été supprimé.")
            else:
                contenu = file_txt.read_text()
                file_py.write_text(contenu)
                print(f"{owcorpus_file_path} n'existe pas.")
