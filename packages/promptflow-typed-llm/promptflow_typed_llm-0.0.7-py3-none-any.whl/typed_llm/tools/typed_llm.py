...
async def _async_do_openai_request(
    ...
    async with semaphore:
        ...
        if completion.choices[0].message.refusal:
            raise BadRequestError(message=f"Completion refused: {completion.choices[0].message.refusal}",
...
def typed_llm(
    ...
    **kwargs) -> list[str]:
    ...
    if not system_prompt and not user_prompt and not assistant_prompt:
        raise BadRequestError(message="At least one of system_prompt, user_prompt, or assistant_prompt must be provided.",
                              response=Response(status_code=400))
    messages = []
    if system_prompt:
    ...
    if response_type not in module.__dict__:
        raise BadRequestError(f"response_type {response_type} not found in {module_path}")
