...
class DrakeLcaEnv(DrakeGymEnv):
    ...
    def step(self, action):
        ...
        mean_external_force = self.external_forces[0] * 0
        a = 1.0 / len(self.external_forces)
        for fext in self.external_forces:
            mean_external_force += a * fext
        info["external_force"] = mean_external_force
        self.external_forces = []  # reset the list.
    ...
    def construct_simulator(self):
        ...
        def get_net_external_forces(context):
            ...
            robot_tau = plant.get_generalized_contact_forces_output_port(
                model_instance=robot_model_instance
            ).Eval(plant_context)
            return robot_tau
        ...
        def monitor(context, gym_time_limit=self.parameters["gym_time_limit"]):
            ...
            self.external_forces.append(get_net_external_forces(simulator.get_context()))
