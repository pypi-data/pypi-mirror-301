...
for file in test_compressed:
    ...
    test_method = create_tests_compressed_binary(file)
    suffix = Path(file).suffix[1:]
    test_method.__name__ = (f'test_compressed_binary_{suffix}')
    setattr(TestStdOpenCompressed, test_method.__name__, test_method)
    # Create compressed text test.
    test_method = create_tests_compressed_text(file)
    suffix = Path(file).suffix[1:]
    test_method.__name__ = (f'test_compressed_text_{suffix}')
    setattr(TestStdOpenCompressed, test_method.__name__, test_method)


for file in test_uncompressed:
    # Create uncompressed binary test.
    test_method = create_tests_uncompressed_binary(file)
    suffix = Path(file).suffix[1:]
    test_method.__name__ = (f'test_uncompressed_binary_{suffix}')
    setattr(TestStdOpenUncompressed, test_method.__name__, test_method)
    # Create uncompressed text test.
    test_method = create_tests_uncompressed_text(file)
    suffix = Path(file).suffix[1:]
    test_method.__name__ = (f'test_uncompressed_text_{suffix}')
    setattr(TestStdOpenUncompressed, test_method.__name__, test_method)
