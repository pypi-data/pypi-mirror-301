...
class ImageGenerator:
    ...
    def __init__(self, model_name='k1tub/gpt2_prompts', 
        ...
        # Проверяем, что выбранный lora_type допустим, если lora_type не None
        ...
        self.text2img_pipe = StableDiffusionPipeline.from_pretrained(
            stable_diff_model, torch_dtype=torch.float16).to(device)

        # Загружаем выбранные веса LoRA, если lora_type не None
    ...
    def generate_images(self, input_prompt, num_images=1, num_inference_steps=50, 
        ...
        # Улучшаем промпт только если improve_prompt=True
        if improve_prompt:
            input_prompt = self.improve_with_gpt(input_prompt)

        # Создаём список промптов — либо один улучшенный, либо повторяем исходный
