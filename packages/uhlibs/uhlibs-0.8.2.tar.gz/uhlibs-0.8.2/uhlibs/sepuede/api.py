...
def addStep(sepuede_api, workerId, stepId):
    """
        Adds the activity step with stepId to the worker workerId
        Parameters:
            sepeuede_api: SePuede Api object
            workerId (int): SePuede ID of a worker
            stepId (int): Step ID to be added to the worker
        Returns:
            status_code (int): status code from api request
    """
    step_activity = {
        'type': 'Step',
        'stepDetailId': stepId,
...
def addDetail(sepuede_api, workerId, detailId, detailType, detailValue):
    """
    Adds a detail of type detailType to a worker
    Parameters: 
        sepeuede_api: SePuede Api object
        workerId (int): SePuede ID of a worker
        detailId (int): Id of detail to be added
        detailType (string): type of the detail, can be the following values: responseString, responseBoolean, responseDate, responseNumber, responseDecimal, responseOption, responseOptions
        detailValue (string): the value to be added
    Returns:
        status_code (int): status code from api request
    """
