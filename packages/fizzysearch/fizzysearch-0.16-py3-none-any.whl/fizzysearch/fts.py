...
def search_fts(
    ...
    def doit(q, language):
        if use_language:
            theq = f"SELECT distinct subject, object, language FROM literal_index WHERE object match ? and language = ? order by rank limit {limit}"
            params = (q, language)
        else:
            theq = f"SELECT distinct subject, object, language FROM literal_index WHERE object match ? order by rank limit {limit}"

            params = (q,)

        back = []
        for subject, object, o_language in db.execute(theq, params):
            if o_language:
                back.append((subject, f'"{object}"@{o_language}'))
            else:
                back.append((subject, f'"{object}"'))
        return back
