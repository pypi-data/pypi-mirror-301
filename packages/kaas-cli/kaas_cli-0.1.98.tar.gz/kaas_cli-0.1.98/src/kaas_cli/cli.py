...
@click.pass_context
def run(
    ctx: Context, mode: str, kontrol_version: KontrolVersion, url: str, vault: str, token: str, branch: str
) -> None:
    if kontrol_version == "v0.0.0":
        click.echo("Running Kontrol in local mode with system version")
    else:
    ...
    if mode == "remote":
        if not all([vault, token, branch]):
            click.echo(
                "You must provide all of the following options: --vault-spec VAULT_SPEC, --token PERSONAL_KEY, --branch GIT_BRANCH"
            )
            sys.exit(1)
        try:
        ...
        if not is_authenticated:
            ...
            sys.exit(1)
        if run_kontrol.run(org_name, vault_name, branch, client):
            click.echo("Kontrol ran successfully...")
            click.echo("  Results of remote runs can be found at https://kaas.runtimeverification.com/")
            return None
        else:
