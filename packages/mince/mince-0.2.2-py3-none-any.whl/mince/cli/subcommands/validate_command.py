...
def add_validate_command(
    ...
    cli_helpers.add_common_args(parser, registry_name='--registry-path')
...
def validate_command(
    ...
    if 'registry' in targets:
        ops.resolve_registry(args.registry_path)

    # validate each dashboard
    if args.dashboard is None and dashboard is None:
        # validate all dashboards
        registry = ops.resolve_registry(args.registry_path)
    ...
    *,
    ...
    if dashboard is not None:
        if isinstance(dashboard, str):
            DashboardClass = ops.get_dashboard_class(
        ...
        DashboardClass = ops.get_dashboard_class(name, registry=registry)

    if 'spec' in targets:
        spec = ops.load_dashboard_ui_spec(dashboard=DashboardClass)
        mince.types.validate_typeddict(spec, mince.types.UiSpec)
        print('    validated', name, 'spec')
    if 'data' in targets:
        data = ops.load_dashboard_data(dashboard=DashboardClass)
        mince.schema.validate_data_matches_spec(data, spec)
