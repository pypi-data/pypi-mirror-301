...
def _get_timeseries_style_kwargs(
    ...
    # get ytick format
    df_metric = dashboards.state.get_df_metric(state=state, ui_spec=ui_spec)
    ...
    df_metric = dashboards.state.get_df_metric(state, ui_spec)
...
def _create_timeseries_title(
    ...
    if df_metric in ui_spec['submetrics']:
        submetric = spec.state_to_alias(
            state['submetric'], 'submetric', ui_spec
        ).lower()
        title: str = submetric
    else:
        title = spec.state_to_alias(df_metric, 'metric', ui_spec).lower()
