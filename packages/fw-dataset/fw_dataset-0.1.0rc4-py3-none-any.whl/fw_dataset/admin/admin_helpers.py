...
from ..helpers import register_arrow_virtual_table
...
def save_schema(
    dataset_path: Path, schema_name: str, schema: dict, overwrite: bool = False
) -> None:
    """Save schema to the dataset directory.

    Args:
        dataset_path (Path): The path of the dataset.
    ...
    """
    schema_path = dataset_path / "schemas" / f"{schema_name}.json"
    schema_path.parent.mkdir(parents=True, exist_ok=True)
    if not schema_path.exists() or overwrite:
        json.dump(
            schema,
            schema_path.open("w"),
            indent=4,
...
def save_table(dataset_path: Path, table_name, table_df, partition=None) -> None:
    ...
    """
    table_path = dataset_path / "tables" / table_name

    table_path.mkdir(parents=True, exist_ok=True)
    if partition:
        table_path_file = table_path / f"{partition}.parquet"
    else:
        table_path_file = table_path / f"{table_name}.parquet"

    if not table_path_file.exists():
        table_df.to_parquet(table_path_file)
...
def get_delimiter(file_path, bytes_read=40960) -> str:
    ...
    """
    sniffer = csv.Sniffer()
    data = open(file_path, "r", encoding="utf-8").read(bytes_read)
...
def get_table_schema(file_path=None, table_df=None, schema=None):
    ...
    if file_path and file_path.exists():
        try:
            ...
            table_df = pd.read_csv(
                file_path, delimiter=delimiter, nrows=1000, comment="#"
            )
...
def validate_dataset_table(duckdb_conn, dataset_path, table_name) -> bool:
    ...
    Args:
        ...
        dataset_path (Pathlike): The path to the dataset.
    ...
    try:
        register_arrow_virtual_table(duckdb_conn, table_name, table_path)
...
def save_custom_info_table(
    dataset_path: Path, custom_info_df: pd.DataFrame
) -> pd.DataFrame:
    """
