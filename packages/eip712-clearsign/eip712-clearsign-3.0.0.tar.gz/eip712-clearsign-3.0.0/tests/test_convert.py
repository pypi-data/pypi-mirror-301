...
from eip712.common.pydantic import model_from_json_file
...
def test_convert(file: str) -> None:
    input_model = model_from_json_file(DATA_DIRECTORY / f"{file}.json", InputEIP712DAppDescriptor)
    resolved_model = EIP712InputToResolvedConverter().convert(input_model)

    resolved_expected_model = model_from_json_file(
        DATA_DIRECTORY / f"{file}.resolved.json", ResolvedEIP712DAppDescriptor
    )
