...
class ClinSigClient:
    ...
    def process_data(self, biomarker_data):
        ...
        # OncoKB
        biomarker_data = onkopus.onkopus_clients.OncoKBClient(self.genome_version).process_data(biomarker_data)

        # CIViC
        biomarker_data = onkopus.onkopus_clients.CIViCClient(self.genome_version).process_data(biomarker_data)

        # MetaKB
        biomarker_data = onkopus.onkopus_clients.MetaKBClient(self.genome_version).process_data(biomarker_data)

        # Aggregate evidence data
        biomarker_data = onkopus.onkopus_clients.AggregatorClient(self.genome_version).process_data(biomarker_data)
