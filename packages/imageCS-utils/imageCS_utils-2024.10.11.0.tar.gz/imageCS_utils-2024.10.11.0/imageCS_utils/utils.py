...
class PhiFunction:
    @staticmethod
    def _phi_function(inputs:Tensor, phi:Tensor, phi_size:int|Tuple[int, int], transpose:bool):
        """
        ...
        if transpose == True:
            ...
            return Phi @ inputs
        """
        (crop_h, crop_w) = to_2tuple(phi_size)
        (cs_out, cs_in) = phi.size()
        assert crop_h == crop_w
        assert crop_h * crop_w == cs_in
        stride = crop_w
        phi_kernel = phi.view(cs_out, 1, crop_h, crop_w)
    ...
    @staticmethod
    def phi(x:Tensor, phi:Tensor, phi_size:int|Tuple[int, int]):
        r"""$\Phi x$ function"""
    ...
    @staticmethod
    def phiT(y:Tensor, phi:Tensor, phi_size:int|Tuple[int, int]):
        r"""$\Phi^T y$ function"""
        return PhiFunction._phi_function(
            inputs=y,
            phi=phi,
    ...
    @staticmethod
    def phiTphi(x:Tensor, phi:Tensor, phi_size:int|Tuple[int, int]):
        r"""$\Phi^T \Phi x$ function"""
        phix = PhiFunction.phi(x, phi, phi_size)
        phiTphix = PhiFunction.phiT(phix, phi, phi_size)
        return phiTphix
    
    @staticmethod
    def a(inputs:Tensor, a:Tensor, a_size:int|Tuple[int, int], matmul_type:str):
        r"""
        $A x$ function
        matmal_type = "x" or "y"
        """
        assert matmul_type in ["x", "y"]
        (crop_h, crop_w) = to_2tuple(a_size)
        (dim1, dim2) = a.size()
        assert crop_h == crop_w
        assert dim1 == dim2
        dim = dim1
        ...
        if matmul_type == "x":
            stride = crop_w
            a_kernel = a.view(dim, 1, crop_h, crop_w)
        elif matmul_type == "y":
            stride = 1
        ...
        if matmul_type == "x":
            outputs = torch.pixel_shuffle(outputs, crop_w)

        return outputs

class PhiClass:
    def __init__(self, phi:Tensor, phi_size:int|Tuple[int, int]):
        self.phi_matrix = phi
        self.phi_size = phi_size
    
    def phi(self, x:Tensor):
        return PhiFunction.phi(x, self.phi_matrix, self.phi_size)
    
    def phiT(self, y:Tensor):
        return PhiFunction.phiT(y, self.phi_matrix, self.phi_size)
    
    def phiTphi(self, x:Tensor):
        return PhiFunction.phiTphi(x, self.phi_matrix, self.phi_size)
...
class ListSplit:
    """Tool for split list"""
    @staticmethod
    def via_size(lst:list, chunk_size:int):
