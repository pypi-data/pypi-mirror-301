...
async def ensure_approval(
    ...
    contract: AsyncContract = w3.eth.contract(address=src_address, abi=client_config.erc20_abi)
...
async def run(
    *,
    ...
    src_order_book_contract = utility.make_order_book_contract(src_w3, src_token)

    risk_manager: RiskManager = SimilarTokenRiskManager(
        account.address, config.max_slippage, logger
    )
    ...
    while True:
        ...
        if not risk_manager.check_order(src_token, dest_token, quote):
            ...
            yield RiskManagerRejection(
                (src_token, dest_token), desired_src_amount, quote
            )
            continue
        ...
        log.debug(
            f"Can fill {src_amount=}/{desired_src_amount=} ({100 * src_amount / desired_src_amount}%) through liquidity source {quote['liquidity_source']}"
        )
