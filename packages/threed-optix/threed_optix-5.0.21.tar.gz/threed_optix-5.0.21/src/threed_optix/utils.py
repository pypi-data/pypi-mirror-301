...
def visualize_results(results, polarization = 'None', light_source_id = 'Total', wavelength = None):
    ...
    if(wavelength is not None):
        ...
        visualize_matrix(np.sum(results[(results['spot_target_kind'] == light_source_id) &
                                        (results['polarization'] == polarization) &
                                        np.isclose(results['wl'], wavelength)]['data']))
    else:
        visualize_matrix(np.sum(results[(results['spot_target_kind'] == light_source_id) & (results['polarization'] == polarization)]['data']))
