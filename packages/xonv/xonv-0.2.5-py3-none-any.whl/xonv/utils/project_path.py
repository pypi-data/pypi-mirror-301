...
def save_git_hash(path: str):
    """Save the current git hash to a file in the project root.
    """
    try:
        git_hash = git.Repo(os.getcwd(),
                            search_parent_directories=True).head.object.hexsha
        with open(
                os.path.join(datadir(path), 'git_hash.txt'),
                'w',
                encoding='utf-8',
        ) as f:
            f.write(git_hash)
    except:
        print('Could not save git hash.')
