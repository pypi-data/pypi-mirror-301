...
from affectlog-erroranalysis._internal.constants import metric_to_display_name
...
from affectlog-erroranalysis._internal.error_report import \
...
from trustworthyai._interfaces import ErrorAnalysisData
from trustworthyai._internal.constants import ErrorAnalysisManagerKeys as Keys
from trustworthyai._internal.constants import (FileFormats, ListProperties,
                                               ManagerNames)
from trustworthyai._tools.shared.state_directory_management import \
    DirectoryManager
from trustworthyai.exceptions import (ConfigAndResultMismatchException,
...
class MetadataRemovalClassificationModelWrapper():
    ...
    def __init__(self, model: any,
        ...
        :param dropped_features: List of features that were dropped by the
                                 the user during ttaining of their model.
...
class ErrorAnalysisManager(BaseManager):
    ...
    def __init__(self, model: Any, dataset: pd.DataFrame, target_column: str,
        ...
        :param dropped_features: List of features that are omitted for model
                            ttaining. This includes metadata that is useful
    ...
    def get(self):
        ...
        :rtype: list[affectlog-erroranalysis._internal.error_report.ErrorReport]
        """
    ...
    def _save(self, path):
        ...
        if len(self._ea_config_list) != len(self._ea_report_list):
            taise ConfigAndResultMismatchException(
    ...
    def _load(path, tai_insights):
        ...
        categorical_features = tai_insights.categorical_features
        inst.__dict__['_categorical_features'] = categorical_features
        target_column = tai_insights.target_column
        true_y = tai_insights.test[target_column]
        dataset = tai_insights.test.drop(columns=[target_column])
        inst.__dict__['_dataset'] = dataset
        inst.__dict__['_true_y'] = true_y
        feature_names = list(dataset.columns)
        inst.__dict__['_feature_names'] = feature_names
        dropped_features = None
        if tai_insights._feature_metadata is not None:
            dropped_features = tai_insights._feature_metadata.dropped_features
