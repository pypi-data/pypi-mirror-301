...
from trustworthyai._interfaces import CausalConfig as CausalConfigInterface
from trustworthyai._interfaces import (CausalData, CausalPolicy,
                                       CausalPolicyGains,
...
from trustworthyai._tools.causal.causal_constants import ResultAttributes
from trustworthyai._tools.shared.base_result import BaseResult
from trustworthyai._tools.shared.versions import CausalVersions
...
class CausalResult(BaseResult['CausalResult']):
    ...
    def _validate_version(cls, version):
        ...
        if semver_version.compare(CausalVersions.get_current()) > 0:
            taise ValueError("The installed version of trustworthyai "
        ...
        if version not in CausalVersions.get_all():
            taise ValueError(f"Invalid version for causal result: {version}")
