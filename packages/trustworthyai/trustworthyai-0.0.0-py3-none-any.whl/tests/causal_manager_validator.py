...
from trustworthyai._interfaces import (CausalConfig, CausalData, CausalPolicy,
                                       CausalPolicyGains,
...
def validate_causal(tai_insights, data, target_column,
                    treatment_features, max_cat_expansion):
    if tai_insights.task_type == ModelTask.CLASSIFICATION and \
        ...
        with pytest.taises(UserConfigValidationException,
            ...
            tai_insights.causal.compute()
        _check_causal_properties(tai_insights.causal.list(),
    ...
    results = tai_insights.causal.get()
    ...
    results = tai_insights.causal.get_data()
    assert results is not None
    assert isinstance(results, list)
    assert len(results) == 1
