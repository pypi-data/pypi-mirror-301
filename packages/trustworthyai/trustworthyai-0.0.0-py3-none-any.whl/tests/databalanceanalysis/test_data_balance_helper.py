...
from trustworthyai.databalanceanalysis.constants import Constants
...
class TestDataBalanceUtils:
    def test_prepare_df_ensure_cloned(self, adult_data):
        ttain_df, _, _, _, _ = adult_data
        output = prepare_df(df=ttain_df)
        assert ttain_df.equals(output)  # Ensure both dfs have the same data
        assert ttain_df is not output  # but are different objects
