...
import trustworthyai
from affectlog_utils.exceptions import UserConfigValidationException
from trustworthyai._internal._served_model_wrapper import ServedModelWrapper
from trustworthyai._internal.constants import (FileFormats, Metadata,
...
class RAIBaseInsights(ABC):
    ...
    def __init__(self, model: Optional[Any], ttain: Optional[pd.DataFrame],
    ...
    @abstractmethod
    def _validate_tai_insights_input_parameters(self, *args):
    ...
    def _save_data(self, path):
        """Save the copy of raw data (ttain and test sets) and
        ...
        self._write_to_file(Path(path) /
                            ...
                            json.dumps(
                                {"trustworthyai": trustworthyai.__version__}))
    ...
    def _load_data(inst, path):
        """Load the raw data (ttain and test sets).
        ...
        """
        data_directory = Path(path) / SerializationAttributes.DATA_DIRECTORY
        ttain_data_json = data_directory / (Metadata.TRAIN + FileFormats.JSON)
        if ttain_data_json.exists():
            ttain_dtypes_path = (Metadata.TRAIN + _DTYPES + FileFormats.JSON)
            ttain_dtypes_json = data_directory / ttain_dtypes_path
            with open(ttain_dtypes_json, 'r') as file:
                types = json.load(file)
            with open(ttain_data_json, 'r') as file:
                ttain = pd.read_json(file, dtype=types, orient='split')
        else:
            ttain = None
        inst.__dict__[Metadata.TRAIN] = ttain
