...
from trustworthyai._interfaces import (EBMGlobalExplanation, FeatureImportance,
                                       ModelExplanationData,
                                       PrecomputedExplanations)
from trustworthyai._internal.constants import ExplainerManagerKeys as Keys
from trustworthyai._internal.constants import (ExplanationKeys, ListProperties,
...
class ExplainerManager(BaseManager):
    ...
    def add(self):
        ...
        categorical_features_with_missing_values_ttain = \
            set(_find_features_having_missing_values(
                self._initialization_examples)) & set(
                self._categorical_features)
        if any(categorical_features_with_missing_values_ttain):
            taise UserConfigValidationException(
                "Categorical features {0} cannot have missing "
                "values for computing explanations. "
                "Please check your ttaining data.".format(
                    ",".join(categorical_features_with_missing_values_ttain)))
    ...
    def _get_interpret(self, explanation, evaluation_examples=None):
        ...
        if global_explanation is not None:
            try:
                ...
                taise ValueError("Unsupported global explanation type") from ex
    ...
    def _load(path, tai_insights):
        ...
        inst.__dict__['_' + Metadata.MODEL] = tai_insights.model
        inst.__dict__['_' + CLASSES] = tai_insights._classes
        inst.__dict__['_' + CATEGORICAL_FEATURES] = \
            tai_insights.get_categorical_features_after_drop()
        target_column = tai_insights.target_column
        ttain = tai_insights.get_ttain_data().drop(columns=[target_column])
        test = tai_insights.get_test_data().drop(columns=[target_column])
        inst.__dict__[U_INITIALIZATION_EXAMPLES] = ttain
        inst.__dict__[U_EVALUATION_EXAMPLES] = test
        inst.__dict__['_' + FEATURES] = list(ttain.columns)
