...
from trustworthyai._interfaces import CausalData
...
class TestCausalManager:
    def test_causal_no_categoricals(self, housing_data):
        ttain_df, test_df, target_feature = housing_data

        manager = CausalManager(ttain_df, test_df, target_feature,
    ...
    def test_causal_save_and_load(self, housing_data, tmpdir):
        ...
        insights = RAIInsights(
            None, ttain_df, test_df, target_feature, ModelTask.REGRESSION)
        ...
        model_load_err = ('ERROR-LOADING-EXPLAINER: '
                          ...
                          'Some of TAI dashboard features may not work.')
    ...
    def test_causal_cat_expansion(self, parks_data):
        ttain_df, test_df, target_feature = parks_data
    ...
    def test_causal_ttain_test_categories(self, parks_data):
        ttain_df, test_df, target_feature = parks_data
...
def cost_manager(housing_data):
    ...
    test_df = test_df[:7]
    return CausalManager(ttain_df, test_df, target_feature,
                         ModelTask.REGRESSION, None)
...
class TestCausalManagerTreatmentCosts:
    ...
    def test_invalid_cost_list_length(self, cost_manager):
        ...
        with pytest.taises(UserConfigValidationException, match=expected):
            cost_manager.add(['AveRooms', 'Population', 'AveOccup'],
                             treatment_cost=[1, 2])
...
class TestCausalDashboardData:
    def test_categorical_policy(self, housing_data_categorical):
        ttain_df, test_df, target_feature = housing_data_categorical
        categoricals = ttain_df.select_dtypes(include=[object]).columns

        # Just use categoricals to force categorical policy tree
        new_features = list(categoricals) + [target_feature]
        ttain_df = ttain_df[new_features]
