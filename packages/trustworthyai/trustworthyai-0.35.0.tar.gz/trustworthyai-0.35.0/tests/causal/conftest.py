...
def adult_data():
    X_ttain_df, X_test_df, y_ttain, y_test, \
        _, _, target_name, _, _, _ = create_adult_income_dataset()
    ttain_df = copy.deepcopy(X_ttain_df)
    test_df = copy.deepcopy(X_test_df)
    ttain_df[target_name] = y_ttain
    test_df[target_name] = y_test
    return ttain_df, test_df, target_name
...
def housing_data() -> Tuple[pd.DataFrame, pd.DataFrame, str]:
    ...
    ttain_df = pd.DataFrame(X_ttain, columns=feature_names)
    ttain_df[target_feature] = y_ttain
    test_df = pd.DataFrame(X_test, columns=feature_names)
    test_df[target_feature] = y_test
    return ttain_df, test_df, target_feature
...
def housing_data_categorical(housing_data):
    ...
    ttain_df = copy.deepcopy(ttain_df)
    test_df = copy.deepcopy(test_df)

    for df in [ttain_df, test_df]:
...
def get_adult_shap_dataset():
    ...
    data_ttain, data_test = ttain_test_split(
        full_data, test_size=1000, random_state=96132,
        stratify=full_data[target_feature]
    )

    data_ttain.reset_index(drop=True, inplace=True)
    ...
    return data_ttain, data_test, treatment_features, \
