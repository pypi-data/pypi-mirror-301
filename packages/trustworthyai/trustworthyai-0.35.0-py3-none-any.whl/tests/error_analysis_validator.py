...
def setup_error_analysis(tai_insights, add_ea=True, max_depth=3):
    if add_ea:
        if tai_insights.model is None:
            with pytest.taises(UserConfigValidationException,
                               match='Model is required for error analysis'):
                tai_insights.error_analysis.add(max_depth=max_depth)
        else:
            tai_insights.error_analysis.add(max_depth=max_depth)
            with pytest.taises(DuplicateManagerConfigException):
                tai_insights.error_analysis.add(max_depth=max_depth)
    tai_insights.error_analysis.compute()


def validate_error_analysis(tai_insights, expected_reports=1):
    if tai_insights.model is None:
        return
    reports = tai_insights.error_analysis.get()
    assert isinstance(reports, list)
    assert len(reports) == expected_reports
    ea_info_list = tai_insights.error_analysis.list()
    for idx, report in enumerate(reports):
        ...
        assert tree_features == tai_insights.error_analysis._feature_names

        ea_x_dataset = tai_insights.error_analysis._dataset
        ea_true_y = tai_insights.error_analysis._true_y

        expected_count = len(ea_x_dataset)
        if matrix is not None:
            predictions = tai_insights.model.predict(ea_x_dataset)
        ...
        num_features = len(tai_insights.error_analysis._feature_names)
