...
from trustworthyai._config.base_config import BaseConfig
...
from trustworthyai._internal.constants import (CounterfactualManagerKeys,
                                               FileFormats, ListProperties,
                                               ManagerNames)
from trustworthyai._tools.shared.state_directory_management import \
    DirectoryManager
from trustworthyai.exceptions import (DuplicateManagerConfigException,
...
class CounterfactualManager(BaseManager):
    ...
    def _create_diceml_explainer(self, method, continuous_features):
        ...
        dice_data = dice_ml.Data(dataframe=self._ttain,
                                 continuous_features=continuous_features,
    ...
    def _add_counterfactual_config(self, new_counterfactual_config):
        if any(_find_features_having_missing_values(self._ttain)):
            taise UserConfigValidationException(
                ...
                'ttain dataset while computing counterfactuals.')
        ...
        validate_ttain_test_categories(
            ...
            tai_compute_type='Counterfactual example generation',
            categoricals=self._categorical_features)

        to_vary = new_counterfactual_config.features_to_vary
        if to_vary != 'all':
            difference_set = set(to_vary) - set(self._ttain.columns)
        ...
        if new_counterfactual_config.permitted_range is not None:
            ...
            difference_set = set(permitted_features) - set(self._ttain.columns)
            if len(difference_set) > 0:
        ...
        if is_duplicate:
            taise DuplicateManagerConfigException(
    ...
    def add(self,
        ...
        if self._categorical_features is None:
            continuous_features = \
                list(set(self._ttain.columns) - set([self._target_column]))
        else:
            continuous_features = list(set(self._ttain.columns) -
    ...
    def _load(path, tai_insights):
        ...
        inst.__dict__[CounterfactualManager._MODEL] = tai_insights.model
        inst.__dict__[CounterfactualManager._TRAIN] = tai_insights.ttain
        inst.__dict__[CounterfactualManager._TEST] = tai_insights.test
        inst.__dict__[CounterfactualManager._TARGET_COLUMN] = \
            tai_insights.target_column
        inst.__dict__[CounterfactualManager._TASK_TYPE] = \
            tai_insights.task_type
