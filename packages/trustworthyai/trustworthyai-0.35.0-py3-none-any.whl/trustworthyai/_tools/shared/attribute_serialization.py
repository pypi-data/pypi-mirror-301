...
class SerializationExtensions:
    ...
    def from_format(cls, file_format: str) -> str:
        if file_format == SerializationFormats.PICKLE:
            ...
            taise ValueError(f"Unknown format: {file_format}")
...
def _save_attribute(
    ...
    if file_format == SerializationFormats.PICKLE:
        ...
        taise ValueError(f"Invalid format {file_format}")
...
def load_attributes(
    ...
    :param fail_on_missing: Whether to taise an exception if an attribute
        was not found.
    """
    dir_path = Path(path)
    is_format_list = isinstance(file_format, list)
