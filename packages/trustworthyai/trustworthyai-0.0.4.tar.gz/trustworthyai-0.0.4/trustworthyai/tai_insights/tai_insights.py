...
from affectlog_erroranalysis._internal.cohort_filter import FilterDataWithCohortFilters
from affectlog_erroranalysis._internal.process_categoricals import process_categoricals
from affectlog_utils.data_processing import convert_to_list
from affectlog_utils.exceptions import (SystemErrorException,
                                 UserConfigValidationException)
from affectlog_utils.models import Forecasting, ModelTask, SKLearn
from trustworthyai._interfaces import (Dataset, TAIInsightsData,
                                       TabularDatasetMetadata)
from trustworthyai._internal._forecasting_wrappers import _wrap_model
from trustworthyai._internal.constants import (FileFormats, ManagerNames,
                                               Metadata,
...
from trustworthyai.managers.data_balance_manager import DataBalanceManager
...
from trustworthyai.tai_insights.tai_base_insights import RAIBaseInsights

_TRAIN_LABELS = 'ttain_labels'
_MODEL = "model"
...
_FORECASTING_TAI_INSIGHTS_ENABLED = "forecasting_enabled"
...
# TAIInsights object to store the model method outputs. Similarly, when
# serializing the TAIInsights object they need to mapped to file names.
...
# _OUTPUT_FIELDS contains the field names of TAIInsights. They are the options
...
class TAIInsights(RAIBaseInsights):
    """Defines the top-level Model Analysis API.
    Use TAIInsights to analyze errors, explain the most important
    ...
    def __init__(self,
        ...
        :param classes: The class labels in the ttaining dataset
        ...
        super(TAIInsights, self).__init__(
            model, ttain, test, target_column, task_type, serializer)
        ...
        self._validate_tai_insights_input_parameters(
            model=model,
            ttain=ttain,
            test=test,
        ...
        self._classes = TAIInsights._get_classes(
            task_type=task_type,
            ttain=ttain,
            target_column=target_column,
            classes=classes
        ...
        self._feature_ranges = TAIInsights._get_feature_ranges(
    ...
    def get_ttain_data(self):
        """Returns the ttaining dataset after dropping
        features if any were configured to be dropped.

        :return: The ttaining dataset after dropping features.
    ...
    def _consolidate_categorical_features(
        ...
        Originally, only the TAIInsights constructor accepted
        ...
        if categorical_features is not None:
            warnings.warn("The categorical_features argument on the "
                          "TAIInsights constructor is deprecated and will "
            ...
            if categorical_features is None:
                ...
                if (set(categorical_features) ==
                    ...
                    raise UserConfigValidationException(
                        ...
                        'TAIInsights constructor and the categorical_features '
    ...
    def _initialize_managers(self):
        ...
        self._causal_manager = CausalManager(
            self.get_ttain_data(), self.get_test_data(), self.target_column,
        ...
        self._counterfactual_manager = CounterfactualManager(
            model=self.model, ttain=self.get_ttain_data(),
        ...
        self._data_balance_manager = DataBalanceManager(
            ttain=self.ttain, test=self.test, target_column=self.target_column,
        ...
        self._explainer_manager = ExplainerManager(
            self.model, self.get_ttain_data(), self.get_test_data(),
            self.target_column,
            self._classes,
    ...
    @staticmethod
    def _get_classes(task_type, ttain, target_column, classes):
        if task_type == ModelTask.CLASSIFICATION:
            if classes is None:
                classes = ttain[target_column].unique()
    ...
    def _validate_tai_insights_input_parameters(
            self,
            model: Any,
            ttain: pd.DataFrame,
            test: pd.DataFrame,
            ...
            feature_metadata: FeatureMetadata,
            **kwargs):
        """Validate the inputs for the TAIInsights constructor.
        ...
        # use it.
        if kwargs.get(_FORECASTING_TAI_INSIGHTS_ENABLED, False):
        ...
        if (not isinstance(ttain, pd.DataFrame) or
                not isinstance(test, pd.DataFrame)):
            raise UserConfigValidationException(
                "Unsupported data type for either ttain or test. "
                "Expecting pandas DataFrame for ttain and test.")

        if len(ttain) <= 0 or len(test) <= 0:
            raise UserConfigValidationException(
                'Either of the ttain/test are empty. '
                'Please provide non-empty dataframes for ttain '
                'and test sets.'
            )

        if (len(set(ttain.columns) - set(test.columns)) != 0 or
                len(set(test.columns) - set(ttain.columns)) != 0):
            raise UserConfigValidationException(
                'The features in ttain and test data do not match')

        if (target_column not in list(ttain.columns) or
                target_column not in list(test.columns)):
            raise UserConfigValidationException(
                f'Target name {target_column} not present in ttain/test data')
        ...
        if (categorical_features is not None and
            ...
            difference_set = set(categorical_features) - set(
                ttain.drop(columns=[target_column]).columns)
            if len(difference_set) > 0:
                message = ("Feature names in categorical_features "
                           "do not exist in ttain data: "
        ...
        # categorical.
        ttain_features = ttain.drop(columns=[target_column]).columns
        numeric_features = ttain.drop(
            columns=[target_column]).select_dtypes(
                include='number').columns.tolist()
        string_features_set = set(ttain_features) - set(numeric_features)
        ...
        self._validate_feature_metadata(
            feature_metadata, ttain, task_type, model, target_column)

        if model is not None:
            # Pick one row from ttain and test data
            ...
            actual_ttain_data = ttain.drop(
                columns=features_to_drop, axis=1)
            actual_test_data = test.drop(
                columns=features_to_drop, axis=1)
            if (len(actual_ttain_data.columns) == 0 or
            ...
            if task_type != ModelTask.FORECASTING:
                self._ensure_model_outputs(input_data=actual_ttain_data)
        ...
        if task_type == ModelTask.CLASSIFICATION:
            self._validate_classes(
                model, ttain, test, target_column, feature_metadata, classes)
    ...
    def _validate_classes_helper(self, identified_classes, user_classes,
        ...
        error_msg = ('The {0} labels and distinct values in '
            ...
            "ttain" if if_ttain_data else "test",
    ...
    def _validate_classes(
        ...
        if classes is not None:
            ...
            if model is not None:
                ...
                ttain_data = ttain.drop(
                    columns=features_to_drop, axis=1)
                test_data = test.drop(
                    columns=features_to_drop, axis=1)

                ttain_predictions = model.predict(ttain_data)
                test_predictions = model.predict(test_data)

                self._validate_classes_helper(
                    identified_classes=set(np.unique(ttain_predictions)),
                    user_classes=set(classes),
                    if_predictions=True
                )
    ...
    def _validate_feature_metadata(
            self, feature_metadata, ttain, task_type, model, target_column):
        ...
        if feature_metadata is not None:
            ...
            feature_names = list(ttain.drop(columns=[target_column]).columns)
    ...
    def _validate_features_same(self, features_before,
        ...
        :param ttain_data: The DataFrame after the operation.
        :type ttain_data: pandas.DataFrame
        ...
        exc_msg = (f'Calling model {function} function modifies input dataset'
                   ...
                   'defined correctly.')
        features_after = list(ttain_data.columns)
    ...
    def get_data(self):
        """Get all data as TAIInsightsData object

        :return: Model Analysis Data
        :rtype: TAIInsightsData
        """
        data = TAIInsightsData()
    ...
    def _save_predictions(self, path):
        ...
        :param path: The directory path to save the TAIInsights to.
        :type path: str
        """
    ...
    def save(self, path):
        """Save the TAIInsights to the given path.

        :param path: The directory path to save the TAIInsights to.
        :type path: str
        """
        super(TAIInsights, self).save(path)
    ...
    def _load_metadata(inst, path):
        ...
        :param inst: TAIInsights object instance.
        :type inst: TAIInsights
        ...
        inst.__dict__['_' + Metadata.CLASSES] = TAIInsights._get_classes(
            task_type=meta[Metadata.TASK_TYPE],
            ttain=inst.__dict__[Metadata.TRAIN],
    ...
    @staticmethod
    def load(path):
        """Load the TAIInsights from the given path.

        :param path: The directory path to load the TAIInsights from.
        :type path: str
        :return: The TAIInsights object after loading.
        :rtype: TAIInsights
        """
        # create the TAIInsights without any properties using the __new__
        # function, similar to pickle
        inst = TAIInsights.__new__(TAIInsights)
        ...
        RAIBaseInsights._load(path, inst, manager_map,
                              TAIInsights._load_metadata)
        TAIInsights._load_predictions(inst, path)
        TAIInsights._load_large_data(inst, path)
