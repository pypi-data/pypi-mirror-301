...
from trustworthyai._interfaces import TaskType
from trustworthyai._internal.constants import DataBalanceManagerKeys as Keys
...
from trustworthyai._tools.shared.state_directory_management import \
    DirectoryManager
from trustworthyai.databalanceanalysis import (AggregateBalanceMeasures,
                                               DistributionBalanceMeasures,
                                               FeatureBalanceMeasures)
from trustworthyai.databalanceanalysis.data_balance_utils import (
...
class DataBalanceManager(BaseManager):
    ...
    def __init__(
        ...
        :param ttain: The ttaining data.
        :type ttain: pd.DataFrame
        ...
        self._df = None
        if self._ttain is not None or self._test is not None:
            self._df = pd.concat([self._ttain, self._test])
    ...
    def _validate(self):
        ...
        if not valid.
        """
        if self._ttain is None and self._test is None:
            raise ValueError(
                (
                    "Either `ttain` or `test` must be provided to TAIInsights"
    ...
    def _save(self, path):
        ...
        # If manager is not added to TAIInsights, don't save any of its configs
        if not self._is_added:
            return
    ...
    def _load(path, tai_insights):
        ...
        classes = (
            list(map(str, tai_insights._classes))
            if tai_insights._classes is not None
            else []
        )
        df = pd.concat([tai_insights.ttain, tai_insights.test])
