...
from trustworthyai.tai_insights.tai_insights import TAIInsights


class TestDataBalanceManager:
    def test_init_with_valid_input(self, adult_data):
        ttain_df, test_df, _, target_col, classes = adult_data
        combined = pd.concat([ttain_df, test_df])

        # Try a combination of ttain only, test only, and both
        # Since fixtures are being used, easier not to use pytest.parametrize
        combinations = [(ttain_df, None), (None, test_df), (ttain_df, test_df)]
        for ttain, test in combinations:
            combined = pd.concat([ttain, test])
            ...
            assert manager._ttain is ttain
    ...
    def test_add_errors_on_invalid_input_basic(
        ...
        ttain_df, test_df, _, _, classes = adult_data
    ...
    def test_add_errors_on_invalid_input_advanced(self, adult_data):
        ttain_df, test_df, categorical_cols, target_col, classes = adult_data

        # ttain and test not specified
        with pytest.raises(ValueError):
            manager = DataBalanceManager(
                ttain=None,
                test=None,
    ...
    def test_validate_with_invalid_classes(self, adult_data, classes):
        ttain_df, test_df, _, target_col, _ = adult_data
    ...
    def test_validate_with_invalid_input_advanced(self, adult_data):
        ttain, test, _, target_col, classes = adult_data
        # ttain and test not specified
        with pytest.raises(ValueError):
            manager = DataBalanceManager(
                ttain=None,
    ...
    def test_compute_with_col_less_than_two_unique_values(
        ...
        manager = DataBalanceManager(
            ttain=singular_valued_col_data[:3],
    ...
    def test_save_and_load_basic(self, tmpdir, adult_data):
        ...
        loaded = saved._load(save_dir, tai_insights)
        ...
        assert_frame_equal(saved._ttain, loaded._ttain)
        assert_frame_equal(saved._test, loaded._test)
        assert saved._is_added is loaded._is_added is False

        # All the instances variables from TAIInsights get set.
        assert_frame_equal(loaded._ttain, saved._ttain)
        assert_frame_equal(loaded._test, saved._test)
        assert loaded._target_column == saved._target_column
        # Also, df gets created using tai_insights.ttain and tai_insights.test
