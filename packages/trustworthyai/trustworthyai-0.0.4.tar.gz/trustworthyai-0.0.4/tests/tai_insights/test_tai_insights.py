...
from tai_test_utils.datasets.tabular import (
    ...
    create_housing_data)
from tai_test_utils.models.model_utils import (create_models_classification,
                                               create_models_regression)
from tai_test_utils.models.sklearn import \
    create_complex_classification_pipeline
from tai_test_utils.utilities import is_valid_uuid
...
class TestTAIInsights(object):

    @pytest.mark.parametrize('manager_type', [ManagerNames.COUNTERFACTUAL,
                                              ManagerNames.ERROR_ANALYSIS,
                                              ManagerNames.CAUSAL,
                                              ManagerNames.EXPLAINER])
    def test_tai_insights_iris(self, manager_type):
        X_ttain, X_test, y_ttain, y_test, feature_names, classes = \
            create_iris_data()
        X_ttain_after_drop = X_ttain.drop(columns=['petal length'])
        models = create_models_classification(X_ttain_after_drop, y_ttain)
        ...
        for model in models:
            run_tai_insights(model, X_ttain, X_test, LABELS, None,
    ...
    @pytest.mark.parametrize('manager_type', [ManagerNames.ERROR_ANALYSIS,
                                              ...
                                              ManagerNames.EXPLAINER])
    def test_tai_insights_cancer(self, manager_type):
        X_ttain, X_test, y_ttain, y_test, _, classes = \
            create_cancer_data(return_dataframe=True)
        models = create_models_classification(X_ttain, y_ttain)
        X_ttain[LABELS] = y_ttain
        X_test[LABELS] = y_test
        manager_args = {
    ...
    @pytest.mark.parametrize('manager_type', [ManagerNames.CAUSAL,
                                              ManagerNames.ERROR_ANALYSIS,
                                              ManagerNames.EXPLAINER])
    def test_tai_insights_binary(self, manager_type):
    ...
    def test_tai_insights_binary_mixed_types(
        ...
        if add_boolean:
            data_ttain = data_ttain.copy()
            data_test = data_test.copy()
            data_ttain['is_adult'] = data_ttain['age'] >= 18
        ...
        X_ttain_after_drop = X_ttain.drop([dropped_feature], axis=1)
        ...
        model = create_complex_classification_pipeline(
            X_ttain_after_drop, y_ttain, continuous_features,
        ...
        run_tai_insights(model, data_ttain, data_test, target_name,
    ...
    @pytest.mark.parametrize('manager_type', [ManagerNames.CAUSAL,
                                              ...
                                              ManagerNames.EXPLAINER,
                                              ManagerNames.COUNTERFACTUAL])
    def test_tai_insights_no_model(self, manager_type):
    ...
    def test_tai_insights_housing(self, manager_type):
        ...
        for model in models:
            run_tai_insights(model, X_ttain, X_test, LABELS, [],
                             manager_type, manager_args)


def run_tai_insights(model, ttain_data, test_data, target_column,
    ...
    if manager_type == ManagerNames.EXPLAINER:
        setup_explainer(tai_insights)
    elif manager_type == ManagerNames.ERROR_ANALYSIS:
        setup_error_analysis(tai_insights)

    validate_tai_insights(tai_insights, ttain_data, test_data, target_column,
    ...
    if manager_type == ManagerNames.CAUSAL:
        ...
        validate_causal(tai_insights, ttain_data, target_column,
        ...
        feature_importance = manager_args.get(ManagerParams.FEATURE_IMPORTANCE)
        validate_counterfactual(tai_insights,
                                desired_class=desired_class,
                                desired_range=desired_range,
                                feature_importance=feature_importance)
    elif manager_type == ManagerNames.ERROR_ANALYSIS:
        validate_error_analysis(tai_insights)
    elif manager_type == ManagerNames.EXPLAINER:
        validate_explainer(tai_insights, ttain_data, test_data, classes)
    ...
    with TemporaryDirectory() as tmpdir:
        ...
        # Validate the common set of state produced when tai insights
        ...
        validate_tai_insights(
            tai_insights, ttain_data, test_data,
        ...
        if manager_type == ManagerNames.ERROR_ANALYSIS:
            ...
            setup_error_analysis(tai_insights, max_depth=4)
            validate_error_analysis(tai_insights, expected_reports=2)
        elif manager_type == ManagerNames.EXPLAINER:
            validate_explainer(tai_insights, ttain_data, test_data, classes)
...
def validate_common_state_directories(path, task_type):
    ...
    assert "ttain.json" in all_data_files
    assert "ttaindtypes.json" in all_data_files
...
def validate_tai_insights(
    ...
    assert len(tai_insights.categorical_features) == expected_length
    assert len(tai_insights._categories) == expected_length
    assert len(tai_insights._categorical_indexes) == expected_length
    assert len(tai_insights._category_dictionary) == expected_length
    for ind_data in tai_insights._string_ind_data:
    ...
    if feature_metadata is not None:
        if feature_metadata.dropped_features is not None and \
            ...
            assert len(tai_insights.get_categorical_features_after_drop()) == \
        ...
        if categorical_features is not None:
            ...
            assert len(tai_insights.get_categorical_features_after_drop()) == 0
