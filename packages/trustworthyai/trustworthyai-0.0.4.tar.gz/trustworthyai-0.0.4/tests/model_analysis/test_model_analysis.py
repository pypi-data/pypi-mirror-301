...
from trustworthyai import ModelAnalysis
...
class TestModelAnalysis(object):
    ...
    def test_model_analysis_iris(self, manager_type):
        ...
        for model in models:
            run_model_analysis(model, X_ttain, X_test, LABELS, None,
    ...
    def test_model_analysis_binary_mixed_types(self, manager_type, adult_data):
        ...
        run_model_analysis(model, data_ttain, data_test, target_name,
    ...
    def test_modelanalysis_housing(self, manager_type):
        ...
        for model in models:
            run_model_analysis(model, X_ttain, X_test, LABELS, [],
                               manager_type, manager_args)


def run_model_analysis(model, ttain_data, test_data, target_column,
    ...
    if model is None:
        ...
        with pytest.warns(DeprecationWarning,
                          match=("MODULE-DEPRECATION-WARNING: "
                                 "ModelAnalysis in trustworthyai "
                                 "package is deprecated. "
                                 "Please use TAIInsights instead.")):
            model_analysis = ModelAnalysis(
                model, ttain_data, test_data,
                target_column,
                categorical_features=categorical_features,
    ...
    validate_model_analysis(model_analysis, ttain_data, test_data,
    ...
    if manager_type == ManagerNames.CAUSAL:
        ...
        validate_causal(model_analysis, ttain_data, target_column,
        ...
        validate_explainer(model_analysis, ttain_data, test_data, classes)

    with TemporaryDirectory() as tempdir:
        path = Path(tempdir) / 'tai_test_path'
        ...
        validate_model_analysis(
            model_analysis, ttain_data, test_data,
            target_column, task_type, categorical_features)
...
def validate_model_analysis(
    ...
    pd.testing.assert_frame_equal(model_analysis.ttain, ttain_data)
    ...
    if task_type == ModelTask.CLASSIFICATION:
        ...
        classes.sort()
        np.testing.assert_array_equal(model_analysis.tai_insights._classes,
                                      classes)
