...
class PandasAI(Shortcuts):
    ...
    def rephrase_query(self, question: str) -> str:
        ...
        Returns:
            str: A rephrased version of the original question.
        """

        # 定义提示模板
        template = """
        You are an AI assistant that helps rephrase user questions for better understanding.
        Please rephrase the following question to make it clearer and more structured.

        Original Question: {user_question}
        Output format: Please return only concise answers and do not contain additional information, and place the rephrased question in brackets [].
        """
        ...
        # result_question 初始值为 question
        result_question = question

        # 检查 response 是否有 content 对象
        if hasattr(response, 'content'):
            ...
            if match:
                ...
                # 赋值到 result_question
                result_question = extracted_question
                print(f"提取到的问题: {extracted_question}")
            else:
                print("没有找到方括号中的内容。")
        else:
            print("响应中没有 content 对象。")

        return result_question

    def dfagent_run(self, query: str):

        prompt = self.rephrase_query(query)
