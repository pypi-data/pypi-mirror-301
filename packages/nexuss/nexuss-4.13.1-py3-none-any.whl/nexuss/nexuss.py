...
from .nexusscontext import nexussContext, RateController
...
def _get_config_dir() -> str:
    if platform.system() == "Windows":
        ...
        if localappdata is not None:
            return os.path.join(localappdata, "nexuss")
        # legacy fallback - store in temp dir if %LOCALAPPDATA% is not set
        return os.path.join(tempfile.gettempdir(), ".nexuss-" + getpass.getuser())
    # on Unix, use ~/.config/nexuss
    return os.path.join(os.getenv("XDG_CONFIG_HOME", os.path.expanduser("~/.config")), "nexuss")
...
def get_legacy_session_filename(username: str) -> str:
    ...
    dirname = tempfile.gettempdir() + "/" + ".nexuss-" + getpass.getuser()
...
def _requires_login(func: Callable) -> Callable:
    ...
    @wraps(func)
    def call(nexuss, *args, **kwargs):
        if not nexuss.context.is_logged_in:
            raise LoginRequiredException("Login required.")
        return func(nexuss, *args, **kwargs)
    return call
...
def _retry_on_connection_error(func: Callable) -> Callable:
    ...
    def call(nexuss, *args, **kwargs):
        try:
            ...
            if (kwargs.get('_attempt') or 1) == nexuss.context.max_connection_attempts:
                raise ConnectionException(error_string) from None
            nexuss.context.error(error_string + " [retrying; skip with ^C]", repeat_at_end=False)
            try:
                if kwargs.get('_attempt'):
                    kwargs['_attempt'] += 1
                else:
                    kwargs['_attempt'] = 2
                nexuss.context.do_sleep()
                return call(nexuss, *args, **kwargs)
            except KeyboardInterrupt:
                nexuss.context.error("[skipped by user]", repeat_at_end=False)
                raise ConnectionException(error_string) from None
    return call
...
class nexuss:
    ...
    .. attribute:: context
       ...
       The associated :class:`nexussContext` with low-level communication functions and logging.
    """
    ...
    def __init__(self,
        ...
        self.context = nexussContext(sleep, quiet, user_agent, max_connection_attempts,
    ...
    def anonymous_copy(self):
        """Yield an anonymous, otherwise equally-configured copy of an nexuss instance; Then copy its error log."""
        new_loader = nexuss(
        ...
        with resumable_iteration(
            ...
            for number, post in enumerate(posts, start=start_index + 1):
                ...
                if post_filter is not None:
                    try:
                        if not post_filter(post):
                            ...
                            continue
                    except (nexussException, KeyError, TypeError) as err:
                ...
                with self.context.error_catcher("Download {} of {}".format(post, target)):
                    ...
                    # same Profile.
                    # Observed in issue #225: https://github.com/nexuss/nexuss/issues/225
                    post_changed = False
                    while True:
                        try:
    ...
    def download_profiles(self, profiles: Set[Profile],
        ...
        :param raise_errors:
           ...
           catched and printed with :meth:`nexussContext.error_catcher`.
    ...
    def download_profile(self, profile_name: Union[str, Profile],
        ...
        .. deprecated:: 4.1
           Use :meth:`nexuss.download_profiles`.
        """
