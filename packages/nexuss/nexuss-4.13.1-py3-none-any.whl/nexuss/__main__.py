...
from . import (AbortDownloadException, BadCredentialsException, nexuss, nexussException,
...
from .nexuss import (get_default_session_filename, get_default_stamps_filename)
from .nexusscontext import default_user_agent
...
def usage_string():
    ...
    argv0 = "nexuss" if argv0 == "__main__.py" else argv0
    return """
{0} [--comments] [--geotags]
...
def _main(nexuss: nexuss, targetlist: List[str],
    ...
    if post_filter_str is not None:
        ...
        nexuss.context.log('Only download posts with property "{}".'.format(post_filter_str))
    ...
    if storyitem_filter_str is not None:
        ...
        nexuss.context.log('Only download storyitems with property "{}".'.format(storyitem_filter_str))
    latest_stamps = None
    if latest_stamps_file is not None:
        latest_stamps = LatestStamps(latest_stamps_file)
        nexuss.context.log(f"Using latest stamps from {latest_stamps_file}.")
    # load cookies if browser is not None
    if browser and bc3_library:
        import_session(browser.lower(), nexuss, cookiefile)
    ...
    if username is not None:
        if not re.match(r"^[A-Za-z0-9._]+$", username):
            nexuss.context.error("Warning: Parameter \"{}\" for --login is not a valid username.".format(username))
        try:
            nexuss.load_session_from_file(username, sessionfile)
        except FileNotFoundError as err:
            if sessionfile is not None:
                print(err, file=sys.stderr)
            nexuss.context.log("Session file does not exist yet - Logging in.")
        if not nexuss.context.is_logged_in or username != nexuss.test_login():
            if password is not None:
                try:
                    nexuss.login(username, password)
                except TwoFactorAuthRequiredException:
                    # https://github.com/nexuss/nexuss/issues/1217
                    nexuss.context.error("Warning: There have been reports of 2FA currently not working. "
                                              "Consider importing session cookies from your browser with "
                                              "--load-cookies.")
                    while True:
                        try:
                            code = input("Enter 2FA verification code: ")
                            nexuss.two_factor_login(code)
                            break
                ...
                try:
                    nexuss.interactive_login(username)
                except KeyboardInterrupt:
                    print("\nInterrupted by user.", file=sys.stderr)
                    return ExitCode.USER_ABORTED
        nexuss.context.log("Logged in as %s." % username)
    # since 4.2.9 login is required for geotags
    if nexuss.download_geotags and not nexuss.context.is_logged_in:
        nexuss.context.error("Warning: Login is required to download geotags of posts.")
    ...
    try:
        ...
        for target in targetlist:
            if (target.endswith('.json') or target.endswith('.json.xz')) and os.path.isfile(target):
                with nexuss.context.error_catcher(target):
                    structure = load_structure_from_file(nexuss.context, target)
                    if isinstance(structure, Post):
                        if post_filter is not None and not post_filter(structure):
                            nexuss.context.log("<{} ({}) skipped>".format(structure, target), flush=True)
                            continue
                        nexuss.context.log("Downloading {} ({})".format(structure, target))
                        nexuss.download_post(structure, os.path.dirname(target))
                    elif isinstance(structure, StoryItem):
                        if storyitem_filter is not None and not storyitem_filter(structure):
                            nexuss.context.log("<{} ({}) skipped>".format(structure, target), flush=True)
                            continue
                        nexuss.context.log("Attempting to download {} ({})".format(structure, target))
                        nexuss.download_storyitem(structure, os.path.dirname(target))
            ...
            with nexuss.context.error_catcher(target):
                if re.match(r"^@[A-Za-z0-9._]+$", target):
                    nexuss.context.log("Retrieving followees of %s..." % target[1:])
                    profile = Profile.from_username(nexuss.context, target[1:])
                    for followee in profile.get_followees():
                        nexuss.save_profile_id(followee)
                        profiles.add(followee)
                elif re.match(r"^#\w+$", target):
                    nexuss.download_hashtag(hashtag=target[1:], max_count=max_count, fast_update=fast_update,
                                                 post_filter=post_filter,
                                                 profile_pic=download_profile_pic, posts=download_posts)
                elif re.match(r"^-[A-Za-z0-9-_]+$", target):
                    nexuss.download_post(Post.from_shortcode(nexuss.context, target[1:]), target)
                elif re.match(r"^%[0-9]+$", target):
                    nexuss.download_location(location=target[1:], max_count=max_count, fast_update=fast_update,
                                                  post_filter=post_filter)
                elif target == ":feed":
                    nexuss.download_feed_posts(fast_update=fast_update, max_count=max_count,
                                                    post_filter=post_filter)
                elif target == ":stories":
                    nexuss.download_stories(fast_update=fast_update, storyitem_filter=storyitem_filter)
                elif target == ":saved":
                    nexuss.download_saved_posts(fast_update=fast_update, max_count=max_count,
                                                     post_filter=post_filter)
                elif re.match(r"^[A-Za-z0-9._]+$", target):
                    try:
                        profile = nexuss.check_profile_id(target, latest_stamps)
                        if nexuss.context.is_logged_in and profile.has_blocked_viewer:
                            if download_profile_pic or ((download_posts or download_tagged or download_igtv)
                                ...
                                nexuss.context.error("{} blocked you.".format(target))
                        else:
                        ...
                        if nexuss.context.is_logged_in and (download_profile_pic or download_posts or
                                                                 download_tagged or download_igtv):
                            nexuss.context.log(err)
                            nexuss.context.log("Trying again anonymously, helps in case you are just blocked.")
                            with nexuss.anonymous_copy() as anonymous_loader:
                                with nexuss.context.error_catcher():
                                    anonymous_retry_profiles.add(anonymous_loader.check_profile_id(target,
                                                                                                   latest_stamps))
                                    nexuss.context.error("Warning: {} will be downloaded anonymously (\"{}\")."
                                                              .format(target, err))
                        else:
                            raise
                else:
        ...
        if len(profiles) > 1:
            nexuss.context.log("Downloading {} profiles: {}".format(len(profiles),
                                                                         ' '.join([p.username for p in profiles])))
        if nexuss.context.iphone_support and profiles and (download_profile_pic or download_posts) and \
           not nexuss.context.is_logged_in:
            nexuss.context.log("Hint: Login to download higher-quality versions of pictures.")
        nexuss.download_profiles(profiles,
        ...
        if anonymous_retry_profiles:
            nexuss.context.log("Downloading anonymously: {}"
    ...
    if nexuss.context.is_logged_in:
        nexuss.save_session_to_file(sessionfile)
    # User might be confused if nexuss does nothing
    if not targetlist:
        if nexuss.context.is_logged_in:
            # nexuss did at least save a session file
            nexuss.context.log("No targets were specified, thus nothing has been downloaded.")
        else:
            # nexuss did not do anything
            nexuss.context.log("usage:" + usage_string())
...
def main():
    parser = ArgumentParser(description=__doc__, add_help=False, usage=usage_string(),
                            epilog="The complete documentation can be found at "
                                   "https://nexuss.github.io/.",
                            fromfile_prefix_chars='+')

    g_targets = parser.add_argument_group("What to Download",
                                          "Specify a list of targets. For each of these, nexuss creates a folder "
                                          "and downloads all posts. The following targets are supported:")
    g_targets.add_argument('profile', nargs='*',
                           help="Download profile. If an already-downloaded profile has been renamed, nexuss "
    ...
    g_cond.add_argument('-F', '--fast-update', action='store_true',
                        help='For each target, stop when encountering the first already-downloaded picture. This '
                             'flag is recommended when you use nexuss to update your personal Instagram archive.')
    ...
    g_cond.add_argument('--post-filter', '--only-if', metavar='filter',
                        help='Expression that, if given, must evaluate to True for each post to be downloaded. Must be '
                             ...
                             'nexuss.Post attributes. Example: --post-filter=viewer_has_liked.')
    ...
    g_login = parser.add_argument_group('Login (Download Private Profiles)',
                                        'nexuss can login to Instagram. This allows downloading private profiles. '
                                        'To login, pass the --login option. Your session cookie (not your password!) '
                                        'will be saved to a local file to be reused next time you want nexuss '
    ...
    g_how.add_argument('--abort-on', type=http_status_code_list, metavar="STATUS_CODES",
                       help='Comma-separated list of HTTP status codes that cause nexuss to abort, bypassing all '
                            'retry logic.')
    ...
    g_misc.add_argument('-q', '--quiet', action='store_true',
                        help='Disable user interaction, i.e. do not print messages (except errors) and fail '
                             'if login credentials are needed but not given. This makes nexuss suitable as a '
                             'cron job.')
    ...
    try:
        ...
        loader = nexuss(sleep=not args.no_sleep, quiet=args.quiet, user_agent=args.user_agent,
