...
from .nexusscontext import nexussContext
...
class PostComment:
    def __init__(self, context: 'nexussContext', node: Dict[str, Any],
...
class Post:
    ...
    Created by methods :meth:`Profile.get_posts`, :meth:`nexuss.get_hashtag_posts`,
    :meth:`nexuss.get_feed_posts` and :meth:`Profile.get_saved_posts`, which return iterators of Posts::
    ...
    :param context: :attr:`nexuss.context` used for additional queries if neccessary..
    ...
    @classmethod
    def from_shortcode(cls, context: nexussContext, shortcode: str):
    ...
    @classmethod
    def from_mediaid(cls, context: nexussContext, mediaid: int):
        """Create a post object from a given mediaid"""
        return cls.from_shortcode(context, Post.mediaid_to_shortcode(mediaid))

    @classmethod
    def from_iphone_struct(cls, context: nexussContext, media: Dict[str, Any]):
    ...
    def url(self) -> str:
        ...
        if self.typename == "GraphImage" and self._context.iphone_support and self._context.is_logged_in:
            try:
                ...
                return url
            except (nexussException, KeyError, IndexError) as err:
...
class Profile:
    ...
    @classmethod
    def from_username(cls, context: nexussContext, username: str):
        """Create a Profile instance from a given username, raise exception if it does not exist.

        See also :meth:`nexuss.check_profile_id`.

        :param context: :attr:`nexuss.context`
    ...
    @classmethod
    def from_id(cls, context: nexussContext, profile_id: int):
    ...
    @classmethod
    def own_profile(cls, context: nexussContext):
    ...
    def has_viewable_story(self) -> bool:
        ...
        has at least one story which can be viewed using the associated :class:`nexussContext`,
    ...
    def profile_pic_url(self) -> str:
        ...
        if self._context.iphone_support and self._context.is_logged_in:
            ...
            except (nexussException, KeyError) as err:
...
class StoryItem:
    ...
    :param context: :class:`nexussContext` instance used for additional queries if necessary.
    ...
    def __init__(self, context: nexussContext, node: Dict[str, Any], owner_profile: Optional[Profile] = None):
        self._context = context
        self._node = node
        self._owner_profile = owner_profile
        self._iphone_struct_ = None
...
class Story:
    ...
    :class:`StoryItem` nodes. Stories are returned by :meth:`nexuss.get_stories`.

    With a logged-in :class:`nexuss` instance `L`, you may download all your visible user stories with::
...
class Highlight(Story):
    ...
    :class:`StoryItem` nodes. Highlights are returned by :meth:`nexuss.get_highlights`.

    With a logged-in :class:`nexuss` instance `L`, you may download all highlights of a :class:`Profile` instance
    USER with::
    ...
    def __init__(self, context: nexussContext, node: Dict[str, Any], owner: Optional[Profile] = None):
...
class Hashtag:
    ...
    @classmethod
    def from_name(cls, context: nexussContext, name: str):
        """
...
class TopSearchResults:
    ...
    :param context: :attr:`nexuss.context` used to send the query for the search.
    :param searchstring: String to search for with Instagram's "top search".
    """

    def __init__(self, context: nexussContext, searchstring: str):
        self._context = context
...
def get_json_structure(structure: JsonExportable) -> dict:
    """Returns nexuss JSON structure for a :class:`Post`, :class:`Profile`, :class:`StoryItem`, :class:`Hashtag`
    ...
    return {
        ...
        'nexuss': {'version': __version__, 'node_type': structure.__class__.__name__}
    }
...
def load_structure(context: nexussContext, json_structure: dict) -> JsonExportable:
    """Loads a :class:`Post`, :class:`Profile`, :class:`StoryItem`, :class:`Hashtag` or :class:`FrozenNodeIterator` from
    a json structure.

    :param context: :attr:`nexuss.context` linked to the new object, used for additional queries if neccessary.
    :param json_structure: nexuss JSON structure

    .. versionadded:: 4.8
    """
    if 'node' in json_structure and 'nexuss' in json_structure and \
            'node_type' in json_structure['nexuss']:
        node_type = json_structure['nexuss']['node_type']
        if node_type == "Post":
        ...
        # Post JSON created with nexuss v3
        return Post.from_shortcode(context, json_structure['shortcode'])
    raise InvalidArgumentException("Passed json structure is not an nexuss JSON")


def load_structure_from_file(context: nexussContext, filename: str) -> JsonExportable:
    """Loads a :class:`Post`, :class:`Profile`, :class:`StoryItem`, :class:`Hashtag` or :class:`FrozenNodeIterator` from
