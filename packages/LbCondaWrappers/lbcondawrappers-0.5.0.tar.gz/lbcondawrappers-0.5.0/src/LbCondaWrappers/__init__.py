...
def check_installroot_available(ins_root, print=False):
    try:
...
for ins_root in INSTALL_ROOT:
    check_installroot_available(ins_root)

TEXLIVE_ROOT = "/cvmfs/lhcbdev.cern.ch/tools/texlive/"
LHCB_ETC = "/cvmfs/lhcb.cern.ch/etc/grid-security"

ENVS_ROOT = [join(ins_root, "envs") for ins_root in INSTALL_ROOT]
MINICONDA_DIR = [
    get_conda_dir(ins_root)[0]
    for ins_root in INSTALL_ROOT
    if join(ins_root, "envs") in ENVS_ROOT
]
...
def lb_conda():
    ...
    for ins_root in INSTALL_ROOT:
        check_installroot_available(ins_root, print=True)
    if not ENVS_ROOT:
