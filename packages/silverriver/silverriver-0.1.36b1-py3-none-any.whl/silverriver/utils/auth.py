...
def read_silverriver_key():
    token = os.getenv("SILVERRIVER_API_KEY")
    if not token:
        try:
            token = _SILVERRIVER_CONFIG_FILE.read_text().strip()
        except FileNotFoundError:
            raise ValueError(
                "API key not found. Please set it using the auth command or via the SILVERRIVER_API_KEY environment variable.")
    return token
