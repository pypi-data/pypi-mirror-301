...
def lsqr(A, b, x=None, maxiter=200, tol=1e-8, verbose=False, damp=0.0):
    ...
    if x is None:  # no starting vector
        ...
        u = b - A.dot(x) #A.mult(x)

    beta = norm(u)
    u /= beta
    v = A.T.dot(u) #A.transMult(u)
    alfa = norm(v)
    v /= alfa
    Arnorm0 = alfa * 1.0
    Arnorm = Arnorm0 * 1.0
    ...
    for i in range(maxiter):
        ...
        u /= beta
        v = A.T.dot(u) - beta * v #A.transMult(u) - beta * v
        alfa = norm(v)
        v /= alfa
...
def cgls(A, b, x=None, maxiter=200, tol=1e-8, verbose=False, damp=0.0):
    ...
    for i in range(maxiter):
        ...
        q = A.dot(p) #A.mult(p)
        alfa = gamma / np.dot(q.T, q)
        x += p * alfa
        r -= q * alfa
        s = A.T.dot(r)#A.transMult(r)
        newgamma = np.dot(s.T, s)
        beta = newgamma / gamma
        gamma = newgamma
        p *= beta
        p += s
...
def rrls(A, b, x=None, maxiter=200, tol=1e-8, verbose=False, damp=0.0):
    ...
    for i in range(maxiter):
        ...
        lam = np.dot(p.T, r)
        r -= p * lam
        v = A.T.dot(p) - rho * v  # A.transMult(p) - rho * v
        theta = norm(v)
        v /= theta
        x += w * (lam/rho)
        w = v - w * (theta/rho)
