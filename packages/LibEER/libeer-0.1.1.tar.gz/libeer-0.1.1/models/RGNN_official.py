...
class SymSimGCNNet(torch.nn.Module):
    ...
    def train_one_round(self, args, r_idx, rr_idx, train_data, train_label, val_data, val_label, test_data, test_label):
        ...
        train_label = self.noise_label(train_label)
        ...
        regularizer = SparseL1Regularization(0.01)
        ...
        # the l1_norm_value
        # regularizer = SparseL1Regularization(0.001)
        ...
        for epoch in range(args.epochs - args.resume_epoch):
            ...
            for idx, (samples, targets) in train_bar:
                ...
                data = Data(x=samples, edge_index=edge_index, y=args.batch_size)
                
                optimizer.zero_grad()
                # perform emotion recognition
                outputs = model(data)
                # calculate the loss value
                loss = criterion(outputs, F.log_softmax(targets, dim=1)) + regularizer(self.edge_weight)
            ...
            # evaluate the model
            metric_value = self.evaluate(data_loader_val, nn.KLDivLoss(), args, model, device)
