...
class rgb:
  ...
  def to_hex(self)  -> 'hexa': return hexa(f'#{self.r:02X}{self.g:02X}{self.b:02X}{'{:02X}'.format(int(self.a * 255)) if self.a else ''}')
...
class hexa:
  ...
  def __len__(self): return 4 if self.a else 3
  def __iter__(self): return iter((f'{self.r:02X}', f'{self.g:02X}', f'{self.b:02X}') + (('{:02X}'.format(int(self.a * 255)),) if self.a else ()))
  def __getitem__(self, index): return ((f'{self.r:02X}', f'{self.g:02X}', f'{self.b:02X}') + (('{:02X}'.format(int(self.a * 255)),) if self.a else ()))[index]
  def __repr__(self): return f'hexa(#{self.r:02X}{self.g:02X}{self.b:02X}{'{:02X}'.format(int(self.a * 255)) if self.a else ''})'
  def __str__(self): return f'#{self.r:02X}{self.g:02X}{self.b:02X}{'{:02X}'.format(int(self.a * 255)) if self.a else ''}'
  def __eq__(self, other):
    if not isinstance(other, hexa): return False
    return (self.r, self.g, self.b) == (other.r, other.g, other.b) and self.a == other.a
  def list(self)      -> list: return [f'{self.r:02X}', f'{self.g:02X}', f'{self.b:02X}'] + (['{:02X}'.format(int(self.a * 255))] if self.a else [])
  def tuple(self)    -> tuple: return tuple(self.list())
  def dict(self)      -> dict: return dict(r=f'{self.r:02X}', g=f'{self.g:02X}', b=f'{self.b:02X}', a='{:02X}'.format(int(self.a * 255))) if self.a else dict(r=f'{self.r:02X}', g=f'{self.g:02X}', b=f'{self.b:02X}')
...
class Data:
  ...
  def to_str(data:list|tuple|dict, indent:int = 2, compactness:int = 0, max_width:int = 140, as_json:bool = False) -> str:
    ...
    def format_value(value:any, current_indent:int) -> str:
      ...
      else: return f'"{escape_string(str(value), '"')}"' if as_json else f"'{escape_string(str(value), "'")}'"
    ...
    def format_key(k:any) -> str: return f'"{escape_string(str(k), '"')}"' if as_json else f"'{escape_string(str(k), "'")}'" if isinstance(k, str) else str(k)
...
class Regex:
  ...
  def func_call(func_name:str = None) -> str:
    ...
    **`2`** the function's arguments\n
    ...
    ------------------------------------------------------------------------------------
    **Attention:** Requires non standard library `regex` not standard library `re`!"""
    return rf'(?<=\b)({func_name if func_name else r'[\w_]+'})\s*{Regex.brackets('(', ')', is_group=True)}'
