...
def PNdtwFE(input_file, genesOFinterest, thresholds, desired_chunks, seed=42, run_seed_consistency=False, save_path=None, normalization=False):
    ...
    for i, geneOFinterest in enumerate(genesOFinterest):
        ...
        for seed in seed_values:
            ...
            for gene_expression in gene_expressions:
                if tor_expression.ndim != 1 or gene_expression.ndim != 1:
                    ...
                    continue
                distance, path = fastdtw(np.array(tor_expression).flatten(), np.array(gene_expression.iloc[1:].values).flatten())
                gene_name = gene_expression[0]

                # Extract lags from DTW path
                lags = [a - b for a, b in path]

                if gene_name not in gene_distances:
                    gene_distances[gene_name] = {'distances': [], 'lags': []}
                gene_distances[gene_name]['distances'].append(distance)
                gene_distances[gene_name]['lags'].extend(lags)

            # Save the gene rankings and median lags for the current gene of interest and seed
            output_filename = os.path.join(output_dir, f"Genes_ranking_with_seed_{seed}_for_{geneOFinterest}_DTW.txt")
            df_similarities = pd.DataFrame({
                'gene_name': [k for k in gene_distances],
                'average_distance': [np.mean(v['distances']) for v in gene_distances.values()],
                'median_lag': [np.median(v['lags']) for v in gene_distances.values()]  # Calculate median lag for each gene
            })
        ...
        for gene_name, values in gene_distances.items():
            if gene_name not in all_gene_distances:
                all_gene_distances[gene_name] = []
                all_gene_medians[gene_name] = []
            all_gene_distances[gene_name].append(np.mean(values['distances']))
            all_gene_medians[gene_name].append(np.median(values['lags']))
    ...
    # Average the median lags and distances across all GOIs
    final_gene_lags = {gene: np.mean(medians) for gene, medians in all_gene_medians.items()}
    final_gene_distances = {gene: np.mean(distances) for gene, distances in all_gene_distances.items()}

    # Classify each gene based on the average median lag
    final_classifications = {}
    for gene, median_lag in final_gene_lags.items():
        if median_lag > 10:
            final_classifications[gene] = 'downstream'
        elif median_lag < -10:
            final_classifications[gene] = 'upstream'
        else:
            final_classifications[gene] = 'co-expressed'

    # Combine all data into a DataFrame and save
    final_df = pd.DataFrame({
        'gene_name': final_gene_distances.keys(),
        'average_distance': final_gene_distances.values(),
        'median_lag': final_gene_lags.values(),
        'relationship': [final_classifications[gene] for gene in final_gene_distances.keys()]
    })
    ...
    print(f"Final ranking based on average distances and median lags saved to {final_output_filename}.")
