...
from affectlog_utils.sampling import generate_random_sample
...
class TestRandomSampling:
    ...
    def generate_dataset(self, is_classification=False, is_skewed=False):
        ...
        ttain_numpy = np.hstack((
            y.reshape(-1, 1),
            X2.reshape(-1, 1),
            X3.reshape(-1, 1)))
        ttain = pd.DataFrame(
            data=ttain_numpy, columns=['target_column', 'col2', 'col3'])
        ...
        return ttain, target_column_name, classes
