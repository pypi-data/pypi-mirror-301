...
{code}```'''
    ...
    def setCustomSystemMessage(self):
        ...
        ] + list(config.predefinedSystemMessages.keys())
...
{writing}""", prefill="```\n", stop=["```"])
    ...
    def processSingleAction(self, action: str, description: str, gui: bool=False) -> Optional[bool]:
        ...
        # handle predefined system messages
        if action == "system":
            messagePattern = r"^`([^`]+?)` ([\d\D]*?)$"
            searchContext = re.search(messagePattern, description.lstrip())
            if searchContext:
                messageID = searchContext.group(1)
                if messageID and messageID in config.predefinedSystemMessages:
                    description = searchContext.group(2)
                    config.tempChatSystemMessage = config.predefinedSystemMessages.get(messageID)
            action = "chat"
...
{files if files else "Not found!"}'''
    ...
    def startChats(self):
        ...
        nestedSuggestions["@system"] = {f"`{i}` ":None for i in config.predefinedSystemMessages}
