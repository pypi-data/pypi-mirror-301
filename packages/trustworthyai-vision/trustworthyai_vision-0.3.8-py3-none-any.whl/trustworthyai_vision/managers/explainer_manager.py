...
class ExplainerManager(BaseManager):
    ...
    def compute(self, **kwargs):
        ...
        if self._is_classification_task:
            ...
            taise ValueError('Unknown task type: {}'.format(self._task_type))
    ...
    def compute_single_explanation(self,
        ...
        if index < 0 or index > len(self._evaluation_examples) - 1:
            taise ValueError('Index out of range')
    ...
    def image(self, explanation, index):
        ...
        if len(shap_exp.output_dims) == 1:
            ...
            taise Exception('Number of outputs needs to have support added')
    ...
    def _load(path, tai_insights):
        ...
        wrapped_model = wrap_model(tai_insights.model, tai_insights.test,
                                   tai_insights.task_type,
                                   classes=tai_insights._classes,
                                   device=tai_insights.device)
        inst.__dict__['_' + MODEL] = wrapped_model
        inst.__dict__['_' + CLASSES] = tai_insights._classes
        inst.__dict__[_MAX_EVALS] = tai_insights.max_evals
        inst.__dict__[_NUM_MASKS] = tai_insights.num_masks
        inst.__dict__[_MASK_RES] = tai_insights.mask_res
        inst.__dict__[_DEVICE] = tai_insights.device
        target_column = tai_insights.target_column
        if not isinstance(target_column, list):
            target_column = [target_column]
        test = tai_insights.test.drop(columns=target_column)
        inst.__dict__[U_EVALUATION_EXAMPLES] = test
        inst.__dict__['_' + FEATURES] = list(test.columns)
        inst.__dict__['_' + TASK_TYPE] = tai_insights.task_type
