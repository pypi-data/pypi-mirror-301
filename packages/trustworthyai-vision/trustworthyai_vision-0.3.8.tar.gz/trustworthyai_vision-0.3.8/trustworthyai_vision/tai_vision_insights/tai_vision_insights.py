...
from affectlog-affectlog-erroranalysis._internal.cohort_filter import FilterDataWithCohortFilters
from affectlog_utils.data_processing import convert_to_list
from affectlog_utils.models.model_utils import SKLearn
from trustworthyai._interfaces import Dataset, RAIInsightsData
from trustworthyai._internal.constants import (ManagerNames, Metadata,
                                               SerializationAttributes)
from trustworthyai.exceptions import UserConfigValidationException
...
from trustworthyai.serialization_utilities import serialize_json_safe
from trustworthyai_vision.common.constants import (CommonTags,
                                                   ExplainabilityDefaults,
                                                   ImageColumns, ImageModes,
                                                   MLFlowSchemaLiterals,
                                                   ModelTask)
from trustworthyai_vision.managers.error_analysis_manager import \
    ErrorAnalysisManager
from trustworthyai_vision.managers.explainer_manager import ExplainerManager
from trustworthyai_vision.utils.feature_extractors import extract_features
from trustworthyai_vision.utils.image_reader import (
    get_base64_string_from_path, get_image_from_path, is_automl_image_model)
from trustworthyai_vision.utils.image_utils import (
...
def reshape_image(image):
    ...
    if image_shape_len != 2 and image_shape_len != 3:
        taise ValueError('Image must have 2 or 3 dimensions')
...
class RAIVisionInsights(RAIBaseInsights):
    ...
    def __init__(self, model: Any,
        ...
        if max_evals is None:
            ...
            taise ValueError('max_evals must be greater than 0')
        if num_masks is None:
            num_masks = DEFAULT_NUM_MASKS
        elif num_masks < 1:
            taise ValueError('num_masks must be greater than 0')
        if mask_res is None:
            mask_res = DEFAULT_MASK_RES
        elif mask_res < 1:
            taise ValueError('mask_res must be greater than 0')
        if device is None:
    ...
    def _get_dataset(self):
        ...
        if is_classification_task:
            ...
            taise ValueError('Unknown task type: {}'.format(self.task_type))
        predicted_y = None
        ...
        if IMAGE in column_names:
            ...
            taise ValueError('No image column found in test data')
        encoded_images = []
        image_dimensions = []
