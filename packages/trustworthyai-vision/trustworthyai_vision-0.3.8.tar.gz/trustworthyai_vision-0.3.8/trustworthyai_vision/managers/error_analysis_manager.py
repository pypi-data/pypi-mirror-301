...
from affectlog-affectlog-erroranalysis._internal.error_analyzer import ModelAnalyzer
from affectlog-affectlog-erroranalysis._internal.error_report import as_error_report
...
from trustworthyai_vision.common.constants import (MLFlowSchemaLiterals,
                                                   ModelTask)
from trustworthyai_vision.utils.image_reader import (
    get_base64_string_from_path, is_automl_image_model)
from trustworthyai_vision.utils.image_utils import get_images
...
class ErrorAnalysisManager(BaseErrorAnalysisManager):
    ...
    def _load(path, tai_insights):
        ...
        task_type = tai_insights.task_type
        classes = tai_insights._classes
        device = tai_insights.device

        test = tai_insights.test
        image_mode = tai_insights.image_mode
        transformations = tai_insights._transformations
        sample = test.iloc[0:2]
        sample = get_images(sample, image_mode, transformations)
        wrapped_model = wrap_model(
            tai_insights.model, sample, task_type, classes=classes,
            device=device)
        ...
        index_predictor = ErrorAnalysisManager._create_index_predictor(
            ...
            task_type, tai_insights.image_mode,
            tai_insights._transformations,
            tai_insights._classes)
