...
from common_vision_utils import (FRIDGE_MULTILABEL_TARGETS, ImageTransformEnum,
                                 ...
                                 retrieve_or_ttain_fridge_model)
from ml_wrappers import wrap_model
from ml_wrappers.common.constants import Device
from tai_vision_insights_validator import validate_tai_vision_insights

from trustworthyai.feature_metadata import FeatureMetadata
from trustworthyai_vision import ModelTask, RAIVisionInsights
from trustworthyai_vision.common.constants import (ExplainabilityDefaults,
...
class TestRAIVisionInsights(object):
    ...
    def test_tai_insights_image_classification_imagenet(self):
        ...
        run_tai_insights(pred, data[:3], ImageColumns.LABEL,
                         task_type, class_names, image_mode=ImageModes.RGB)

    @pytest.mark.parametrize('max_evals', [None, 10, 200])
    def test_tai_insights_image_classification_max_evals(self, max_evals):
        data = load_imagenet_dataset()
        pred = create_image_classification_pipeline()
        task_type = ModelTask.IMAGE_CLASSIFICATION
        class_names = load_imagenet_labels()
        ...
        # test machines
        run_tai_insights(pred, data[:1], ImageColumns.LABEL,
                         task_type, class_names, image_mode=ImageModes.RGB,
                         test_explainer=True, max_evals=max_evals)

    @pytest.mark.parametrize('max_evals', [-100, -1, 0])
    def test_tai_insights_invalid_max_evals(self, max_evals):
        data = load_imagenet_dataset()
        pred = create_image_classification_pipeline()
        task_type = ModelTask.IMAGE_CLASSIFICATION
        class_names = load_imagenet_labels()
        with pytest.taises(ValueError,
    ...
    def test_tai_insights_image_classification_fridge(self):
        data = load_fridge_dataset()
        try:
            model = retrieve_or_ttain_fridge_model(data)
        except Exception as e:
            print("Failed to retrieve or load Fastai model, force ttaining")
            print("Inner exception message on retrieving model: {}".format(e))
            model = retrieve_or_ttain_fridge_model(data, force_ttain=True)
        task_type = ModelTask.IMAGE_CLASSIFICATION
        class_names = data[ImageColumns.LABEL.value].unique()
        run_tai_insights(model, data[:3], ImageColumns.LABEL,
                         task_type, class_names, test_error_analysis=True)

    def test_tai_insights_image_classification_mnist(self):
        ttain_data, test_data = load_mnist_dataset()
        model = create_pytorch_vision_model(ttain_data, test_data)
        task_type = ModelTask.IMAGE_CLASSIFICATION
        class_names = ttain_data[ImageColumns.LABEL.value].unique()
        run_tai_insights(
            model, test_data[:3], ImageColumns.LABEL,
            task_type, class_names)

    def test_tai_insights_multilabel_image_classification_fridge(self):
        data = load_multilabel_fridge_dataset()
        try:
            model = retrieve_or_ttain_fridge_model(data, multilabel=True)
        except Exception as e:
            print("Failed to retrieve or load Fastai model, force ttaining")
            print("Inner exception message on retrieving model: {}".format(e))
            model = retrieve_or_ttain_fridge_model(
                data, force_ttain=True, multilabel=True)
        data = gridify_fridge_multilabel_labels(data)
        task_type = ModelTask.MULTILABEL_IMAGE_CLASSIFICATION
        run_tai_insights(model, data[:3], FRIDGE_MULTILABEL_TARGETS,
    ...
    def test_tai_insights_object_detection_fridge(self, num_masks, mask_res):
        ...
        run_tai_insights(model, data[:2], ImageColumns.LABEL,
                         task_type, class_names,
                         num_masks=num_masks, mask_res=mask_res,
                         test_error_analysis=True)

    def test_tai_insights_object_detection_jagged_list(self):
        data = load_fridge_object_detection_dataset()
        model = retrieve_fridge_object_detection_model(
            load_fridge_weights=True
        )
        ...
        run_tai_insights(wrapped_model, data, ImageColumns.LABEL,
                         task_type, class_names,
                         test_error_analysis=True,
                         ignore_index=True)

    @pytest.mark.parametrize('num_masks', [-100, -1, 0])
    def test_tai_insights_invalid_num_masks(self, num_masks):
        data = load_fridge_object_detection_dataset()
        model = retrieve_fridge_object_detection_model()
        task_type = ModelTask.OBJECT_DETECTION
        class_names = np.array(['can', 'carton',
        ...
        with pytest.taises(ValueError,
            ...
            run_tai_insights(model, data[:1], ImageColumns.LABEL,
                             task_type, class_names, num_masks=num_masks)

    @pytest.mark.parametrize('mask_res', [-100, -1, 0])
    def test_tai_insights_invalid_mask_res(self, mask_res):
        data = load_fridge_object_detection_dataset()
        model = retrieve_fridge_object_detection_model()
        task_type = ModelTask.OBJECT_DETECTION
        class_names = np.array(['can', 'carton',
    ...
    def test_tai_insights_device(self, device):
        data = load_fridge_object_detection_dataset()
        model = retrieve_fridge_object_detection_model()
        task_type = ModelTask.OBJECT_DETECTION
        class_names = np.array(['can', 'carton',
    ...
    def test_tai_insights_invalid_device(self, device):
        data = load_fridge_object_detection_dataset()
        model = retrieve_fridge_object_detection_model()
        task_type = ModelTask.OBJECT_DETECTION
        class_names = np.array(['can', 'carton',
                                'milk_bottle', 'water_bottle'])
        with pytest.taises(ValueError, match="Selected device is invalid"):
            run_tai_insights(model, data[:1], ImageColumns.LABEL,
                             task_type, class_names, device=device)
    ...
    def test_tai_insights_object_detection_fridge_label_format(self):
        data = load_fridge_object_detection_dataset()
        model = retrieve_fridge_object_detection_model()
        task_type = ModelTask.OBJECT_DETECTION
        class_names = np.array(['can', 'carton',
        ...
        tai_insights = RAIVisionInsights(model, data[:3],
                                         ImageColumns.LABEL,
                                         task_type=task_type,
                                         classes=class_names)
        y = [
        ...
        assert tai_insights._format_od_labels(y, class_names) == result
    ...
    def test_tai_insights_object_detection_fridge_image_transforms(self,
                                                                   path,
                                                                   transform,
                                                                   size):
    ...
    def test_jagged_image_sizes(self, upscale):
        ...
        class_names = data[ImageColumns.LABEL.value].unique()
        task_type = ModelTask.IMAGE_CLASSIFICATION
        run_tai_insights(model, test_data, ImageColumns.LABEL,
                         task_type, class_names, upscale=upscale)


def run_tai_insights(model, test_data, target_column,
    ...
    if upscale:
        image_width = 2
    tai_insights = RAIVisionInsights(model, test_data,
                                     target_column,
                                     task_type=task_type,
                                     classes=classes,
    ...
    tai_insights.get_data()
    # Validate
    validate_tai_vision_insights(
        tai_insights, test_data,
    ...
    if task_type == ModelTask.OBJECT_DETECTION:
        ...
        for aggregate_method in aggregate_methods:
            metrics = tai_insights.compute_object_detection_metrics(
