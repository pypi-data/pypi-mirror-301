...
from tai_vision_insights_validator import run_and_validate_serialization

from trustworthyai_vision import ModelTask, RAIVisionInsights
from trustworthyai_vision.common.constants import ImageColumns
...
class TestRAIVisionInsightsSaveAndLoadScenarios(object):
    ...
    def test_tai_insights_pytorch_empty_save_load_save(self):
    ...
    def test_tai_insights_large_images_save_load_save(self):
    ...
    def test_loading_tai_insights_without_model_file(self):
        ...
        tai_insights = RAIVisionInsights(
            pred, test, label,
            task_type=task_type,
            classes=class_names,
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            model_pkl_path = Path(tmpdir) / "tai_insights" / model_name
            shutil.rmtree(model_pkl_path)
            match_msg = 'No file or directory found'
            with pytest.taises(OSError, match=match_msg):
                without_model_tai_insights = RAIVisionInsights.load(save_path)
                assert without_model_tai_insights.model is None

    @pytest.mark.parametrize('automl_format', [True, False])
    def test_tai_insights_object_detection(self, automl_format):
    ...
    def test_tai_insights_image_downloader_object_detection(self):
        ...
        get_data = ('test_tai_vision_insights_save_and_load_scenarios'
