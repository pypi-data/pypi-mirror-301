...
class BuildProjConfig:
    ...
    def get_composition_name(self):
        ...
        name, _ = os.path.splitext(self._prj_cfg['CompositionConfig']['compositionName'])
        return name

    def get_composition_ending(self):
        """Return the composition ending."""
        _, ending = os.path.splitext(self._prj_cfg['CompositionConfig']['compositionName'])
        if ending:
            return ending
        return 'yml'
    ...
    def get_composition_arxml(self):
        ...
        return self._prj_cfg['CompositionConfig']['compositionArxml']

    def get_custom_yaml_init_function_name(self):
        """Return the custom yaml init function name."""
        return self._prj_cfg['CompositionConfig'].get('customYamlInitFunctionName')

    def get_gen_ext_impl_type(self):
        """Return the generate external implementation type."""
        return self._prj_cfg.get('CompositionConfig', {}).get('generateExternalImplementationType', True)

    def get_swc_name(self):
        """Returns the software component name."""
        a2lname = f"{self.get_a2l_cfg()['name']}_SC"
        return self._prj_cfg.get('CompositionConfig', {}).get('softwareComponentName', a2lname)

    def get_swc_template(self):
        """Returns the software component template to use."""
        return self._prj_cfg['CompositionConfig'].get('softwareComponentTemplate')

    def get_swc_base(self):
        """Returns the software component base to use (ASIL classification)."""
        return self._prj_cfg.get('CompositionConfig', {}).get('softwareComponentBase', 'QM')

    def get_include_static(self):
        """Returns if the static part of the software component should be included in the composition."""
        return self._prj_cfg.get('CompositionConfig', {}).get('includeStatic', True)

    def get_include_shared(self):
        """Returns if the shared part of the software component should be included in the composition."""
        return self._prj_cfg.get('CompositionConfig', {}).get('includeShared', True)

    def get_include_diagnostics(self):
        """Returns if the diagnostic part of the software component should be included in the composition."""
        return self._prj_cfg.get('CompositionConfig', {}).get('includeDiagnostics', True)
