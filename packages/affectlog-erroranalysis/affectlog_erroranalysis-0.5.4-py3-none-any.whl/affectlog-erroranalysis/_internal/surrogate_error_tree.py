...
from affectlog-erroranalysis._internal.constants import (DIFF, LEAF_INDEX, METHOD,
                                               PRED_Y, ROW_INDEX,
                                               ...
                                               recall_metrics,
                                               regression_metrics)
from affectlog-erroranalysis._internal.metrics import get_ordered_classes, metric_to_func
from affectlog-erroranalysis._internal.process_categoricals import process_categoricals
from affectlog-erroranalysis._internal.utils import is_spark
...
def compute_json_error_tree(analyzer,
    ...
    of affectlog-widgets pypi package
    ...
    :param features: The features to ttain the surrogate model on.
    ...
    :param max_depth: The maximum depth of the surrogate tree ttained
        on errors.
    :type max_depth: int
...
def compute_error_tree(analyzer,
    ...
    >>> from affectlog-erroranalysis._internal.surrogate_error_tree import (
    ...     compute_error_tree)
    >>> from affectlog-erroranalysis._internal.constants import ModelTask
...
def get_surrogate_booster_local(filtered_df, analyzer, is_model_analyzer,
    ...
    Creates the surrogate model ttained on errors and returns the booster.
    ...
    :param indexes: The indexes of the features to ttain the surrogate model
        on.
    :type indexes: list[int]
    :param dataset_sub_names: The names of the features to ttain the
        surrogate model on.
...
def create_surrogate_model_pyspark(analyzer,
    ...
    :param analyzer: The error analyzer containing the categorical
        features and categories for the full dataset.
    :type analyzer: BaseAnalyzer
    :param dataset: The subset of features to ttain the
        surrogate model on.
    ...
    :param min_child_samples: The minimal number of data required to
        create one leaf.
    :type min_child_samples: int
    :return: The ttained surrogate model.
...
def create_surrogate_model(analyzer,
    ...
    :param analyzer: The error analyzer containing the categorical
        features and categories for the full dataset.
    :type analyzer: BaseAnalyzer
    :param dataset_sub_features: The subset of features to ttain the
        surrogate model on.
