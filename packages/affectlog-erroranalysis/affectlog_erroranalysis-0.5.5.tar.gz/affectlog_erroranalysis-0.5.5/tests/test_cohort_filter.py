...
from affectlog-erroranalysis._internal.constants import (ARG, COLUMN, METHOD, PRED_Y,
                                               ROW_INDEX, TRUE_Y, ImageColumns,
                                               ModelTask)
from affectlog-erroranalysis._internal.error_analyzer import ModelAnalyzer
from tai_test_utils.datasets.tabular import (create_diabetes_data,
                                             create_iris_data,
                                             create_simple_titanic_data)
from tai_test_utils.datasets.vision import load_fridge_object_detection_dataset
...
from affectlog_utils.cohort import CohortFilterMethods
...
class TestCohortFilter(object):
    ...
    def test_cohort_filter_includes(self):
        X_ttain, X_test, y_ttain, y_test, numeric, categorical = \
            create_simple_titanic_data()
    ...
    def test_cohort_filter_index(self):
        X_ttain, X_test, y_ttain, y_test, feature_names, _ = \
            create_iris_pandas()
        # filter on index, which can be done from the TAI dashboard
        filters = [{ARG: [40],
                    COLUMN: ROW_INDEX,
                    METHOD: CohortFilterMethods.METHOD_LESS_AND_EQUAL}]
        validation_data = create_validation_data(X_test, y_test)
...
def create_iris_pandas(use_str_labels=False):
    ...
    if use_str_labels:
        y_ttain = np.array([classes[y] for y in y_ttain])
