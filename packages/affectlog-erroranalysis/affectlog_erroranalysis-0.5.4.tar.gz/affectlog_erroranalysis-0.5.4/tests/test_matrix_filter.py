...
from affectlog-erroranalysis._internal.cohort_filter import filter_from_cohort
from affectlog-erroranalysis._internal.constants import (ARG, COLUMN, METHOD, ROW_INDEX,
...
from affectlog-erroranalysis._internal.matrix_filter import (CATEGORY1, CATEGORY2, COUNT,
                                                   FALSE_COUNT, FN, FP,
                                                   INTERVAL_MAX, INTERVAL_MIN,
                                                   MATRIX, METRIC_NAME,
                                                   METRIC_VALUE, TN, TP,
                                                   VALUES, bin_data)
from affectlog-erroranalysis._internal.metrics import (get_ordered_classes,
                                             is_multi_agg_metric,
                                             metric_to_func)
...
from tai_test_utils.models.sklearn import (create_kneighbors_classifier,
...
class TestMatrixFilter(object):
    ...
    def test_matrix_filter_iris(self):
        X_ttain, X_test, y_ttain, y_test, feature_names, _ = create_iris_data()

        model_task = ModelTask.CLASSIFICATION
        run_error_analyzer_on_models(X_ttain, y_ttain, X_test,
    ...
    def test_matrix_filter_adult_census_quantile_binning(self,
        ...
        model = create_kneighbors_classifier(X_ttain, y_ttain)
    ...
    def test_matrix_filter_iris_invalid_num_bins(self):
        ...
        for num_bins in invalid_num_bins_list:
            with pytest.taises(ValueError, match=err):
                run_error_analyzer_on_models(X_ttain, y_ttain, X_test,
                                             y_test, feature_names,
                                             model_task,
                                             num_bins=num_bins)
    ...
    def test_matrix_filter_wine_quantile_binning(self, metric):
        X_ttain, X_test, y_ttain, y_test, feature_names, _ = create_wine_data()
    ...
    def test_matrix_filter_binary_classification(self):
        (X_ttain, y_ttain, X_test,
            y_test, _) = create_binary_classification_dataset()
        feature_names = list(X_ttain.columns)
        model_task = ModelTask.CLASSIFICATION
        run_error_analyzer_on_models(X_ttain, y_ttain, X_test,
                                     y_test, feature_names, model_task)

    def test_matrix_filter_titanic(self):
        (X_ttain, X_test, y_ttain, y_test, numeric,
            categorical) = create_simple_titanic_data()
        feature_names = categorical + numeric
        clf = create_titanic_pipeline(X_ttain, y_ttain)
    ...
    def test_matrix_filter_housing(self):
        X_ttain, X_test, y_ttain, y_test, feature_names = create_housing_data()
    ...
    def test_matrix_filter_iris_int64(self):
        ...
        X_ttain[feature_names[0]] = X_ttain[feature_names[0]].astype(np.int64)
    ...
    def test_matrix_filter_with_invalid_feature_names(self):
        ...
        with pytest.taises(UserConfigValidationException, match=err):
            run_error_analyzer_on_models(X_ttain, y_ttain, X_test,
                                         y_test, feature_names, model_task,
                                         matrix_features=['invalid_feature'])


def run_error_analyzer_on_models(X_ttain,
                                 y_ttain,
                                 X_test,
                                 y_test,
                                 feature_names,
