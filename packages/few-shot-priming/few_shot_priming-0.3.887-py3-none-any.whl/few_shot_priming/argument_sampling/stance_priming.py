...
def generate_most_similar_stance_arguments(experiment, path_similarities, path_samples, different_topics, debug, min_threshold=None, max_threshold=None, drop_duplicate_perspectives=False):
    ...
    if different_topics:
        for _, test_record in tqdm(dataset["test"].iterrows()):
            ...
            if min_threshold and max_threshold:
                for key in instances_scores_map:
                    if instances_scores_map[key] >=max_threshold or instances_scores_map[key]< min_threshold:
        ...
        for _, test_record in dataset["test"].iterrows():
            ...
            if min_threshold and max_threshold:
                elements_to_drop = []
                for key in instances_scores_map:
                    if instances_scores_map[key] >=max_threshold and instances_scores_map[key]<min_threshold:
                        elements_to_drop.append(key)
                for key in elements_to_drop:
                        del instances_scores_map[key]
    ...
    if min_threshold:
        path_samples = path_samples.replace(".tsv", f"-{min_threshold}-{max_threshold}.tsv")
...
def get_thresholds_for_percentiles(path_similarities, percentile_num):
    ...
    threshold_pairs = []
    for i in range(len(thrsholds)-1):
        threshold_pairs.append((thrsholds[i], thrsholds[i+1]))

    return threshold_pairs

def get_pathes_for_thresholds(path_sample_root, path_similarities, percentile_num):
    threshold_pairs = get_thresholds_for_percentiles(path_similarities, percentile_num)
    threshold_pathes_list = []
    for threshold in threshold_pairs:
        path_samples = path_sample_root.replace(".tsv", f"-{threshold[0]}-{threshold[1]}.tsv")
