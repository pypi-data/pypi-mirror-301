...
#                                    if not exists:
    ...
    if min_threshold:
        path_samples = path_samples.replace(".tsv", f"-{min_threshold:.2}-{max_threshold:.2}.tsv")
...
def get_pathes_for_thresholds(path_sample_root, path_similarities, percentile_num):
    ...
    for threshold in threshold_pairs:
        path_samples = path_sample_root.replace(".tsv", f"-{threshold[0]:.2}-{threshold[1]:.2}.tsv")
