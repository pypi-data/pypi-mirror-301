...
class Transform(nn.Module):
    def __init__(
        self, in_channels, out_channels, nblks=0, block=None, relu=None, conv=nn.Conv2d
    ):
        ...
        if nblks > 0 and block is not None:
            ...
            for i in range(nblks - 1):
                blocks.append(
                    block(self.out_channels, step=1.0 / nblks, relu=relu, conv=conv)
                )
                blocks.append(relu)
            blocks.append(
                block(self.out_channels, step=1.0 / nblks, relu=relu, conv=conv)
...
class Block(nn.Module):
    def __init__(
        ...
        if normalizor == "batch":
            ...
            self.normalizor = nn.LayerNorm(
                tuple([transform.out_channels]) + tuple(transform.size)
            )
    ...
    def __init__(
        ...
        if not self.exceeded:
            ...
            if padding:
                ...
                self.conv_padding = 1
                self.iconv = Conv(
                    in_channels, c0, kernel_size=3, padding=self.conv_padding, groups=1
                )
                self.oconv = Conv(
                    c0,
                    out_channels,
