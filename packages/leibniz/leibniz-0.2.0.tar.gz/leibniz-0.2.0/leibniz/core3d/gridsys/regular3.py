...
class RegularGrid:
    ...
    def boundary(self) -> Tensor:
        ...
        return th.cat([top, bottom, foreward, backward, left, right], dim=1).view(
            1, 6, self.W, self.L, self.H
        )
    ...
    def data(self) -> Tensor:
        return cast(
            np.meshgrid(
                ...
                np.linspace(self.lower, self.upper, num=self.H),
                indexing="ij",
            ),
            device=self.default_device,
        ).view(1, 3, self.L, self.W, self.H)
