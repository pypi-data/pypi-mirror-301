...
pub fn whitespace_byte(b: u8) -> bool {
    matches!(b, b' ' | b'\n' | b'\r' | b'\t')
}
...
impl<R: io::BufRead> Parser<R> {
    ...
    /// # }
    /// ```
    pub fn new(reader: R, data_only: bool) -> Parser<R> {
        Parser {
            reader,
            line: 1,
            end_of_line: false,
        ...
        <T as FromStr>::Err: Into<ParseErrorKind>,
    {
        ...
        tok.parse().map_err(|e| self.error(e).into())
    }
    ...
    fn read_string_command(&mut self) -> Result<String, io::Error> {
        ...
        let s =
            from_utf8(&r).map_err(|_| io::Error::from(self.error(ParseErrorKind::InvalidUtf8)))?;
    ...
    fn parse_command(&mut self) -> Result<Command, io::Error> {
        ...
        match cmd {
            ...
            b"timescale" => {
                ...
                let quantity = num_str.parse().map_err(|e| self.error(e))?;
                let unit = unit_str.parse().map_err(|e| self.error(e))?;
    ...
    fn parse_scope(
        ...
        loop {
            match self.next() {
                ...
                Some(Ok(_)) => {
                    return Err(self.error(ParseErrorKind::UnexpectedHeaderCommand).into())
                }
    ...
    fn next(&mut self) -> Option<Result<Command, io::Error>> {
        while let Some(b) = self.read_byte_or_eof().transpose() {
            ...
            match b {
                ...
                b'$' => {
                    if self.data_only {
                        () // If we're only interested in data, ignore commands
                    } else {
                ...
                _ => return Some(Err(self.error(ParseErrorKind::UnexpectedCharacter).into())),
            }
        }
        None
    }
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            ParseErrorKind::UnexpectedCharacter => {
                write!(f, "unexpected character at start of command")
            }
...
#[cfg(test)]
mod test {
    use crate::vcd::parser::parser::{ParseError, ParseErrorKind};
    use crate::vcd::parser::{
    ...
    fn wikipedia_sample() {
        ...
        let mut b = Parser::new(&sample[..], false);
        ...
        assert_eq!(
            header.items[0],
            ScopeItem::Comment("Any comment text.".to_string())
        );
        ...
        assert_eq!(
            &header.find_var(&["logic", "data"]).unwrap().reference,
            "data"
        );
...
#0
";
        let mut parser = Parser::new(&sample[..], false);
...
";
        ...
        assert_eq!(
            &header.find_var(&["smt_clock"]).unwrap().reference,
            "smt_clock"
        );
        assert_eq!(
            &header.find_var(&["queue", "o_data"]).unwrap().reference,
            "o_data"
        );
    ...
    fn test_err_in_header() {
        ...
        let err = Parser::new(&text[..], false).parse_header().unwrap_err();
    ...
    fn test_err_in_data() {
        ...
        let err = Parser::new(&text[..], false).find_map(|f| f.err()).unwrap();
        let err: Box<ParseError> = err.into_inner().unwrap().downcast().unwrap();
        assert!(
            matches!(err.kind, ParseErrorKind::UnexpectedCharacter),
            "{:?}",
            err
        );
