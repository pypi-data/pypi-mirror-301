...
def _handle_event(context: VXContext, event: VXEvent) -> None:
    ...
    if len(handlers) == 1:
        ...
        try:
            ...
            logging.error(f"Event handler error: {event.type} -> {handler}", exc_info=True)
    else:
...
def quit_handler(signum: int, frame: Any) -> None:
    ...
    try:
        with open("data/params.json", "w") as f:
            ...
            logging.info("保存参数: %s", to_json(__context__.params))
    except Exception as err:
...
def run(args: Namespace) -> None:
    global __context__, __handlers__
    ...
    try:
        ...
        logging.error("运行时错误...%s", err)
        quit_handler(0, None)


def save_params(signum: Any, frame: Any) -> None:
    global __context__
    print("保存参数...")
    try:
        with open("data/params.json", "w") as f:
...
# signal.signal(signal.SIGINT, save_params)
# signal.signal(signal.SIGTERM, save_params)
# signal.signal(signal.CTRL_C_EVENT, save_params)
