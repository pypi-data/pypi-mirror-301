...
class IdeStorageAndZipManager extends TerminalRunner {
  ...
  _freshStore(code, extractForThis=true){
    return {
      code: code || "",
  ...
  _extractWhateverLocalStorageVersion(editorId){
    let outdatedData   = true
    let extractForThis = editorId==this.id
    let storage        = localStorage.getItem(editorId) || ""

    try{
      const obj = JSON.parse(storage)
      outdatedData = "hash code done name zip".split(' ').some( k => !(k in obj) )
      if(!outdatedData) storage=obj
    }catch(_){}

    if(outdatedData){
      storage = this._freshStore(storage, extractForThis)
    }
    return [storage, outdatedData]
  }


  getStorage(){
    let [storage, outdatedData] = this._extractWhateverLocalStorageVersion(this.id)
    this.storage = storage

    // Enforce update of internal values:
    if( outdatedData || this.storage.name!=this.pyName || this.storage.zip!=this.export ){
...
class _IdeEditorHandler extends IdeStorageAndZipManager {
  ...
  restart(){    jsLogger("[Restart]")
    ...
    this.storage = this._freshStore(exerciseCode)
