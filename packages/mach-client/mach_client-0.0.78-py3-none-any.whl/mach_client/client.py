...
class MachClient:
    ...
    async def _validate_response(self, response: ClientResponse) -> Any:
        match response.status:
            ...
            case 422:
                raise ValueError(f"Validation error - invalid request: {pformat(await response.json())}")

            case _:
                raise RuntimeError(f"Unknown status code {response.status}: {pformat(await response.json())}")
