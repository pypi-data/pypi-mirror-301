...
class GalileoObserveCallback(BaseCallbackHandler):
    ...
    def _start_new_node(self, run_id: UUID, parent_run_id: Optional[UUID]) -> tuple[str, Optional[str], Optional[str]]:
        node_id = str(run_id)
    ...
    def on_chat_model_start(
        ...
        **kwargs: Any,
        ...
        tags = kwargs.get("tags")
        metadata = kwargs.get("metadata")
        model = kwargs["invocation_params"]["model"] or kwargs["invocation_params"]["_type"]
        temperature = kwargs["invocation_params"].get("temperature")
        self.records[node_id] = TransactionRecord(
            node_id=node_id,
            chain_id=chain_id,
