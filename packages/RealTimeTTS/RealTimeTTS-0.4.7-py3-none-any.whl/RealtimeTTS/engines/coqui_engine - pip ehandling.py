...
class CoquiEngine(BaseEngine):
    ...
    def _synthesize_worker(
        ...
        try:
            ...
            logging.info("Parent process has closed the connection. Exiting worker process.")
            conn.close()

        except Exception as e:
            logging.error(f"General synthesis error: {e} occurred in synthesize worker thread of coqui engine.")
            tb_str = traceback.format_exc()
            print(f"Traceback: {tb_str}")
            print(f"Error: {e}")

            # Attempt to send the error message, but handle exceptions if the pipe is closed
            try:
                conn.send(('error', str(e)))
            except (BrokenPipeError, EOFError) as send_exc:
                logging.error(f"Failed to send error message over pipe: {send_exc}")
            conn.close()
    ...
    def send_command(self, command, data):
        ...
        try:
            ...
            logging.warning(f'Error sending command {command}: {e}')        
        # message = {'command': command, 'data': data}
        # self.parent_synthesize_pipe.send(message)            
    ...
    def set_model(self, checkpoint: str):
        ...
        # print(f"set_model")
        # self.send_command('set_model', {'checkpoint': self.model_path})
        ...
        # else:
        #     logging.error('Error updating model')
    ...
    def synthesize(self, text: str) -> bool:
        ...
        with self._synthesize_lock:
            ...
            try:
                ...
                logging.error(f'Error receiving from pipe: {e}')
                return False
    ...
    def shutdown(self):
        ...
        if self.synthesize_process.is_alive():
            try:
                ...
                logging.warning(f'Error while sending shutdown command: {e}')
            ...
            try:
                ...
                logging.warning(f'Worker process pipe was closed before shutdown acknowledgement: {e}')
        else:
            logging.info('Worker process is not alive. Skipping shutdown command.')
        ...
        if self.synthesize_process.is_alive():
            ...
            logging.info('Worker process has already terminated')

        # Now stop the output worker thread
    ...
    #         logging.warning('BrokenPipeError while sending shutdown command')
