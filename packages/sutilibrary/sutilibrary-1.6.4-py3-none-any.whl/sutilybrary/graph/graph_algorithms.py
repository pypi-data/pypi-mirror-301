...
def visualize_graph(graph: Graph, algorithm_result=None, algorithm_name=None):
    ...
    if algorithm_result and algorithm_name:
        if algorithm_name in ['bfs', 'dfs']:
            ...
            nx.draw_networkx_edges(G, pos, edgelist=algorithm_result, edge_color='r', width=2)

    plt.title(f"Graph Visualization{' - ' + algorithm_name if algorithm_name else ''}")
    plt.axis('off')
    plt.tight_layout()
    plt.show()
...
def run_algorithm(graph: Graph, algorithm: str, start: int = None) -> Any:
    ...
    visualize_graph(graph, result, algorithm)
    return result
