...
class Server:
    ...
    def _create_route(self, method: Callable):
        ...
        async def endpoint(body: ExtendedBodyModel, background_tasks: BackgroundTasks):
            ...
            # Add the API endpoint execution to background tasks
            background_tasks.add_task(self._run_endpoint, method_name, original_body, body.prediction_id)
    ...
    def run(self, host: str = "0.0.0.0", port: int = 6006):
        ...
        async def health():
            return {"success": True}
        uvicorn.run(self.fastapi_app, host=host, port=port,workers=4)
