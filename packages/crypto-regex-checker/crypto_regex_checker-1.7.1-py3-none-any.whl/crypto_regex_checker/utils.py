import json
import sys

# We need files() from Python 3.10 or higher
if sys.version_info >= (3, 10):
    import importlib.resources as ilr
else:
    import importlib_resources as ilr

from crypto_regex_checker.log import logger

str_types = (str,)


# Load all lines from browser.json file
# Returns array of objects
def load(fn="browsers.json"):
    data, ret = [], None
    try:
        if fn == "browsers.json":
            json_lines = (
                ilr.files("crypto_regex_checker.data").joinpath(fn).read_text()
            )
            for line in json_lines.splitlines():
                data.append(json.loads(line))
            ret = data
        else:
            ret = ilr.files("crypto_regex_checker.data").joinpath(fn).read_bytes()
    except Exception as exc:
        # Empty data just to be sure
        data = []
        logger.warning(
            "Unable to find local data/json file or could not parse the contents using importlib-resources. Try pkg-resource next.",
            exc_info=exc,
        )
        try:
            from pkg_resources import resource_filename

            if fn == "browsers.json":
                with open(
                    resource_filename("crypto_regex_checker", "data/"+str(fn))
                ) as file:
                    json_lines = file.read()
                    for line in json_lines.splitlines():
                        data.append(json.loads(line))
            else:
                with open(
                    resource_filename("crypto_regex_checker", "data/"+str(fn)),
                    'rb'
                ) as file:
                    data = file.read()
            ret = data
        except Exception as exc2:
            # Empty data just to be sure
            data = []
            logger.warning(
                "Could not find local data/json file or could not parse the contents using pkg-resource.",
                exc_info=exc2,
            )

    if not ret:
        raise FakeUserAgentError("Data list is empty", ret)

    if fn == "browsers.json" and not isinstance(ret, list):
        raise FakeUserAgentError("Data is not a list ", ret)
    return ret


from crypto_regex_checker import settings  # noqa # isort:skip
from crypto_regex_checker.errors import FakeUserAgentError  # noqa # isort:skip
