...
def get_helpful_meta_tags(wiki_name_or_path=None):
    ...
    try:
        wiki_name_or_path = wiki_name_or_path or hf_hub_download(WIKI_REPO_ID, filename=WIKI_FILES['helpful_meta_tags']['default']['filename'], repo_type='dataset', cache_dir=WIKI_CACHE_DIR)
        with open(wiki_name_or_path, 'r', encoding='utf-8') as f:
...
def get_helpless_meta_tags(wiki_name_or_path=None):
    ...
    try:
        wiki_name_or_path = wiki_name_or_path or hf_hub_download(WIKI_REPO_ID, filename=WIKI_FILES['helpless_meta_tags']['default']['filename'], repo_type='dataset', cache_dir=WIKI_CACHE_DIR)
        with open(wiki_name_or_path, 'r', encoding='utf-8') as f:
...
def get_character_feature2ratio(character: str, feature_type_to_frequency_threshold: Dict[Literal['physics', 'clothes', 'sex'], float] = DEFAULT_FEATURE_TYPE_TO_FREQUENCY_THRESHOLD) -> Dict[str, float]:
    character = uncomment_tag(character)
    character = fmt2danbooru(character)
    features = {}
    if 'physics' in feature_type_to_frequency_threshold:
...
def alias_tags(tags: List[str]):
    ...
    for i, tag in enumerate(tags):
        ...
        is_danbooru = ' ' not in tag
        tag = uncomment_tag(tag, tagtype)
        tag = tag_aliases.get(fmt2danbooru(tag), tag)
        if not is_danbooru:
            tag = fmt2train(tag)
        tags[i] = comment_tag(tag, tagtype) if tagtype else tag
    return tags
