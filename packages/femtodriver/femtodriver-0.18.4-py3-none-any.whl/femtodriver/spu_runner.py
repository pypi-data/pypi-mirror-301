...
class SPURunner(FemtoRunner):
    ...
    def _load_file_into_memory(self, core_id: int, memory_type: str) -> None:
        """
        ...
        memory_data = load_hexfile(
            filename=self._get_memory_file_path(
                core_id=core_id, memory_type=memory_type
            )
        )
        ...
        if memory_type in ["DM", "TM"] and core_id in core_used:
            for i in range(bank_num):
                ...
                if bank_end_index == 0:
                    return
                bank_data = memory_data[bank_size * i : bank_size * i + bank_end_index]
    ...
    def export_femto_file(
        self, model_type: str, model_name: str, model_version: int, target_spu: str
    ) -> None:
        """
