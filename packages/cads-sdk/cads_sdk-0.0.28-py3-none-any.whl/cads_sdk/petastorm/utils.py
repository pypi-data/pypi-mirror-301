...
def add_to_dataset_metadata(dataset, key, value):
    ...
    """
    if not isinstance(dataset._base_dir, str):
        raise ValueError('Expected dataset._base_dir to be a single path, not a list of paths')

    metadata_file_path = dataset._base_dir.rstrip('/') + '/_metadata'
    common_metadata_file_path = dataset._base_dir.rstrip('/') + '/_common_metadata'
    common_metadata_file_crc_path = dataset._base_dir.rstrip('/') + '/._common_metadata.crc'
    ...
    if dataset.filesystem.exists(common_metadata_file_path):
        with dataset.filesystem.open(common_metadata_file_path) as f:
        ...
        with dataset.filesystem.open(metadata_file_path) as f:
            arrow_metadata = pyarrow.parquet.read_metadata(f)
    else:
    ...
    with dataset.filesystem.open(common_metadata_file_path, 'wb') as metadata_file:
    ...
    if isinstance(dataset.filesystem, LocalFileSystem) and dataset.filesystem.exists(common_metadata_file_crc_path):
        try:
            dataset.filesystem.rm(common_metadata_file_crc_path)
