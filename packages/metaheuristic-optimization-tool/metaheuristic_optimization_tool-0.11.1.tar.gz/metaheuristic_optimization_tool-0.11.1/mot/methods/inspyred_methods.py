...
class inspyred_optimize(Method):
    ...
    def write_header(self, column_list, file_name):
        with open(file_name, 'w') as f:
            f.write(','.join(column_list))
            f.write('\n')
        
    def write_csv_line(self, column_values, file_name):
        with open(file_name, 'a') as f:
            f.write(','.join([str(q) for q in column_values]))
            f.write('\n')
    ...
    def search(self, Perturber, Fitter, algorithm='', workdir='./',
        ...
        self.write_header(column_header, self.file_name)

        # create appendix file
        column_header = ['Uniqid'] + list(input_list) + list(metric_list)
        self.write_header(column_header, self.appdx_name)
    ...
    def evaluate_(self, candidates, args):
        ...
        for cs in candidates:
            ...
            for sim in self.Fitter.simulation:
                sim(cs, uniqid)
            # read output
            metrics = [fcn(uniqid) for fcn in self.Fitter.function]
            fitness.append(emo.Pareto(metrics))

            # write appendix file
            column_values = [uniqid] + list(cs) + list(metrics)
            self.write_csv_line(column_values, self.appdx_name)
    ...
    def observe_(self, population, num_generations, num_evaluations, args):
        for indx, pop in enumerate(population):
            ...
            # write result line
            uniqid = self.get_uniqid_from_appdx(candidate, fitness)
            column_values = [num_generations, indx, uniqid] + list(candidate) + list(fitness)
            self.write_csv_line(column_values, self.file_name)
