...
def parse_team_row(tr: BeautifulSoup) -> Either[ParsingError, Pairing]:
  try:
    cols = tr.find_all('td')
    white_player = cols[1].text.strip()
    ...
    if len(cols) >= 8:
      ...
      return Right(Paired(
        ...
        result=parse_team_result(result)
      ))
    else:
      return Right(Unpaired(player=white_player, reason='Unpaired'))
  except Exception as e:
...
def parse_team_round(soup: BeautifulSoup) -> RoundPairings:
  ...
  for row in rows:
    if is_header_row(row):
      ...
      player_board = 1
      continue
    pair = parse_team_row(row)
    if pair.tag == 'left':
      continue
    pairs[f'{team_board}.{player_board}'] = pair
    player_board += 1
