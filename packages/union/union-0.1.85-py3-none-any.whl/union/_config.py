...
def _get_config_obj(
    ...
    if config_in is not None:
        if isinstance(config_in, str):
            config = Config.auto(config_in)
        else:  # ConfigWithSource object
            if isinstance(config_in.config, str):
                config = Config.auto(config_in.config)
            else:  # Config object
        ...
        if is_local_cli and (_UNION_CONFIG.is_direct_union_cli_call or default_to_union_semantics):
            msg = dedent("""\
