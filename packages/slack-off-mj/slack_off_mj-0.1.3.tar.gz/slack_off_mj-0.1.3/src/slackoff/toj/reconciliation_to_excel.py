...
class ToExcel(ToWriter):
    ...
    def cell(self, writer_rule, col, row_chain, row_data, row_num, start_row_num, sheet):
        cel_values = self.doCell(col, row_chain, row_data, row_num, start_row_num)
    ...
    def doCell(self, col, row_chain, row_data, row_num, start_row_num):
        col_keys = col.keys()
        col_values = []  # 列的所有值
        if len(row_chain) > 0:
        ...
        for col_index in col_keys:  # 一列一列的匹配
            ...
            col_values.insert(int(col_index), col_value)
        return col_values
