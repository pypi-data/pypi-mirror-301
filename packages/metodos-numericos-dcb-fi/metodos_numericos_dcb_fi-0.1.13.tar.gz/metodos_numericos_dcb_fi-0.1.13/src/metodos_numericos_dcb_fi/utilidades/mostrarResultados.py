# ------------------- Importar módulos -------------------
from metodos_numericos_dcb_fi.utilidades.configuracion import letras_latinas, letras_griegas
...
def mostrarPolinomio(a:list[float], variable:str='x', titulo:str='')->None:
    ...
    if len(variable) == 0:
        raise ValueError(f'La longitud de la variable debe ser al menos 1.\nLongitud de la variable ingresada: {len(variable)}.')
    ...
    for letra in variable:
        if not (letra in letras_latinas + letras_griegas):
            raise ValueError(f'Las variables deben ser letras latinas o griegas.\nVariable ingresada: {variable}.')
...
def agregarRenglonFactoresCuadraticos(tabla:pd.DataFrame, p:float, q:float, b:list[float], R:float, S:float, dp:float, dq:float)->pd.DataFrame:
    ...
    if len(b) != len(tabla.columns) - 6:
        raise Exception(f'EL numero de coeficientes debe ser {len(tabla.columns) - 6}.\nSe ingresaron {len(b)} coeficientes.')
