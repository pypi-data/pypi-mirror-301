...
def build_wasm(context: CertoraContext) -> None:
    ...
    if file_path.name != Util.CARGO_FILE:
        return
    if not context.rule:
        raise Util.CertoraUserInputError(f"Setting rule is mandatory when running with a {Util.CARGO_FILE} file")
...
def run_wasm_compiler(context: CertoraContext) -> None:
    ...
    try:
        ...
        cmd = f"wat2wasm {wat_file} --debug-names -o {context.wasm_file}"
        Util.run_shell_command(cmd, cwd=context.cargo_dir, env=env)

        cmd = f"wasm2wat {context.wasm_file} -o {wat_file}"
        Util.run_shell_command(cmd, cwd=context.cargo_dir, env=env)
