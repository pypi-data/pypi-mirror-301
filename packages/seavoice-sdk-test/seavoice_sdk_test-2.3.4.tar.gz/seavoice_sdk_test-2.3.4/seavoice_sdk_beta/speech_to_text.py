        ...
        while True:
            try:
                ...
                if self.retry_count > self.retry_max:
                    self.logger.error(
                        f"{self.connection_name} has too many Exception: {error.__class__.__name__} {error} reties: {self.retry_count}"
                    )
                    raise error

                self.retry_count += 1
                self.logger.warning(
                    f"{self.connection_name} will retry after {base_sleep_time} sec, encounter error: {error.__class__.__name__} {error}"
                )
