...
@click.pass_obj
def setup(env):
    """subcommands for managing workspaces for krakenit"""
...
def _service(env, command):
    """List existing workspaces for krakenit"""
    ...
    service_unit = f"""
[Unit]
Description=krakenit Supervisor Service
...
def service(env, command):
    """Install a reset service krakenit"""
...
def autocomplete(env):
    ...
    subprocess.run(
        [
            "_KRAKENIT_COMPLETE=bash_source krakenit > ~/.krakenit-complete.bash"
        ],
        shell=True,
    )
    ...
    print("source ~/.krakenit-complete.bash")
