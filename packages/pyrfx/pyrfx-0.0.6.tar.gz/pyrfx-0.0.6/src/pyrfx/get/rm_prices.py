...
from pyrfx.utils import execute_threading, get_synthetics_reader_contract, save_csv, save_json, timestamp_df
...
class RMPrices(GetData):
    ...
    def get_price_withdraw(self) -> dict[str, Any]:
        ...
        """
        return self._process_rm_price(MAX_PNL_FACTOR_FOR_WITHDRAWALS)
    ...
    def get_price_deposit(self) -> dict[str, Any]:
        ...
        :return: Dictionary of RM prices.
        """
        return self._process_rm_price(MAX_PNL_FACTOR_FOR_DEPOSITS)
    ...
    def get_price_traders(self) -> dict[str, str | float]:
        ...
        :return: Dictionary of RM prices.
        """
        return self._process_rm_price(MAX_PNL_FACTOR_FOR_TRADERS)
    ...
    def _process_rm_price(self, pnl_factor_type: bytes) -> dict[str, str | float]:
        """
    ...
    def _save_output(self) -> None:
        ...
        if self.config.save_to_json:
            file_name: str = f"{self.config.chain}_rm_prices.json"
            save_json(output_data_path=self.config.data_path, file_name=file_name, data=self.output)
            self.logger.info(f"RM Prices saved to {file_name}")

        if self.config.save_to_csv:
            file_name: str = f"{self.config.chain}_rm_prices.csv"
