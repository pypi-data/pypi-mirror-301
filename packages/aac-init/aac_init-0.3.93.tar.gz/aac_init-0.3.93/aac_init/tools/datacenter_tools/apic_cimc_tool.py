...
class APIC_CIMC_Tool:
    ...
    def ssh_init_apic(self):
        ...
        try:
            ...
            out_msg += self.ssh_connection.read_until_pattern("Enter the fabric name|Press Enter Or Input JSON string", read_timeout=300)
            # For version 6
            if "Press Enter Or Input JSON string" in out_msg:
                out_msg = self.ssh_connection.send_command_timing("\n", read_timeout=10)
                out_msg += self.ssh_connection.read_until_pattern("Enter the password for admin", read_timeout=300)
            ...
            while loop:
                if "Would you like to edit the configuration?" in out_msg:
                    ...
                    for key, value in apic_config_dict.items():
                        if key in out_msg:
                            out_msg = self.ssh_connection.send_command_timing(
                                value, last_read=5, read_timeout=15,
                                strip_prompt=False
                            )
                            matched = True
                            break
            ...
            pattern = (f"{self.hostname} login:")
            self.ssh_connection.read_until_pattern(pattern=pattern, read_timeout=180)
