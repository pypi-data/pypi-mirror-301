...
class NavIndexes:
        ...
        for date, value in self.values.items():
            if value >= scan_peak:
                ...
                if dd > mdd:
                    ...
                    mdd_recovery_date = ""
            if value == mdd_high_spot and mdd_recovery_date == "":
                mdd_recovery_date = date
    ...
    def annualized_stdev(self) -> float:
        ...
        assert annualized_stdev >= 0, "annualized_stdev should be greater than or equal to 0"
    ...
    def downside_stdev(self) -> float:
        """下行标准差"""
        values: List[float] = [value*self.annualized_days for value in self.daily_return.values() if value < self.r]
        if len(values) >= 2:
