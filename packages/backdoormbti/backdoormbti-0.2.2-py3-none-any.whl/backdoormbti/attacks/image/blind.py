...
from backdoormbti.resources.blind.backdoor_generate_poison_index import generate_poison_index_from_label_transform
from backdoormbti.resources.blind.aggregate_block.bd_attack_generate import bd_attack_label_trans_generate
from copy import deepcopy
from backdoormbti.resources.blind.aggregate_block.model_trainer_generate import generate_cls_model
from backdoormbti.resources.blind.aggregate_block.fix_random import fix_random
from backdoormbti.resources.blind.bd_dataset_v2 import dataset_wrapper_with_transform, get_labels
from backdoormbti.resources.blind.aggregate_block.save_path_generate import generate_save_folder
from backdoormbti.resources.blind.aggregate_block.dataset_and_transform_generate import get_num_classes, get_input_shape
from backdoormbti.resources.blind.aggregate_block.train_settings_generate import argparser_opt_scheduler, argparser_criterion
from backdoormbti.resources.blind.save_load_attack import save_attack_result
from backdoormbti.resources.blind.trainer_cls import all_acc, given_dataloader_test, \
    plot_loss, plot_acc_like_metric, Metric_Aggregator, test_given_dataloader_on_mix
from backdoormbti.resources.blind.bd_dataset_v2 import prepro_cls_DatasetBD_v2, dataset_wrapper_with_transform
from torch.utils.data.dataloader import DataLoader
from backdoormbti.resources.blind.aggregate_block.bd_attack_generate import general_compose
from backdoormbti.resources.blind.aggregate_block.dataset_and_transform_generate import dataset_and_transform_generate
...
class Task:
    ...
    def init_task(self):
        ...
        self.optimizer, self.scheduler = argparser_opt_scheduler(
...
def record_time(params: Params, t=None, name=None):
    if t and name and params.save_timing == name or params.save_timing is True:
        ...
        params.timing_data[name].append(
            round(1000 * (time.perf_counter() - t)))
...
def train(hlpr: Helper, epoch, model, optimizer, train_loader, attack=True):
    ...
    for i, data in tqdm(enumerate(train_loader)):
        ...
        with torch.cuda.amp.autocast(enabled=args.amp):
            loss = hlpr.attack.compute_blind_loss(
                model, criterion, batch, attack)
...
def run(hlpr, clean_test_dataloader, bd_test_dataloader, criterion, device, args):
    ...
    for epoch in range(hlpr.params.start_epoch,
        ...
        test_asr = all_acc(bd_test_epoch_predict_list,
                           bd_test_epoch_label_list)
        test_ra = all_acc(bd_test_epoch_predict_list,
...
class blendedImageAttack_on_batch(object):
    ...
    def add_trigger(self, img):
        # use the broadcast
        return (1 - self.blended_rate) * img + (self.blended_rate) * self.target_image[None, ...]
...
class Blind:
    ...
    def stage1_non_training_data_prepare(self):
        ...
        trigger_pattern_np = np.repeat(
            trigger_pattern_np[:, :, np.newaxis], args.input_channel, axis=2)
        trigger_full_size_np = np.ones(
            (args.input_height, args.input_width, args.input_channel)) * (-10)
        x_top = 3
        y_top = 23
    ...
    def prepare(self, args):
        ...
        if 'save_folder_name' not in args:
            save_path = generate_save_folder(
                ...
                all_record_folder_path='../../logs',
            )
        else:
            save_path = '../../logs/' + args.save_folder_name
            os.mkdir(save_path)
        ...
        logging.debug(
            "Only INFO or above level log will show in cmd. DEBUG level log only will show in log file.")
    ...
    def benign_prepare(self):
        ...
        clean_train_dataset_targets = get_labels(
            train_dataset_without_transform)
