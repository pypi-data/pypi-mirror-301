...
from backdoormbti.resources.blind.trainer_cls import ModelTrainerCLS
...
try:
    ...
    logging.warning(
        "efficientnet_b0,b3 fails to import, plz update your torch and torchvision")
try:
    from torchvision.models import mobilenet_v3_large
except:
    logging.warning(
        "mobilenet_v3_large fails to import, plz update your torch and torchvision")

try:
    from torchvision.models import vit_b_16, vit_b_32, vit_l_16, vit_l_32
except:
    logging.warning(
        "vit fails to import, plz update your torch and torchvision")
...
def generate_cls_model(
    ...
    if model_name == 'resnet18':
        ...
        logging.debug(
            'Make sure you want PreActResNet18, which is NOT resnet18.')
        from backdoormbti.models.preact_resnet import PreActResNet18
        if kwargs.get("pretrained", False):
            logging.warning(
                "PreActResNet18 pretrained on cifar10, NOT ImageNet!")
        ...
        if kwargs.get("pretrained", False):
            ...
            net_from_imagenet = models.vgg19_bn(pretrained=True)
            net = models.vgg19_bn(
                num_classes=num_classes, **{k: v for k, v in kwargs.items() if k != "pretrained"})
        ...
        logging.debug(
            "All vit model use the default pretrain and resize to match the input shape!")
        if model_name == 'vit_b_16':
            net = vit_b_16(
                pretrained=True,
                **{k: v for k, v in kwargs.items() if k != "pretrained"}
            )
            net.heads.head = torch.nn.Linear(
                net.heads.head.in_features, out_features=num_classes, bias=True)
        ...
        from backdoormbti.models.resnext import ResNeXt29_2x64d
        net = ResNeXt29_2x64d(num_classes=num_classes)
    elif model_name == 'senet18':
        from backdoormbti.models.senet import SENet18
        ...
        raise SystemError(
            'NO valid model match in function generate_cls_model!')
