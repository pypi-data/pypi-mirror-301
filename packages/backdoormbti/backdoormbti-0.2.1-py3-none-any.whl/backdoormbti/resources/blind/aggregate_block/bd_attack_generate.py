...
from backdoormbti.resources.blind.bd_label_transform.backdoor_label_transform import *
...
def bd_attack_img_trans_generate(args):
    ...
    if args.attack in ['badnet',]:
        ...
        train_bd_transform = general_compose([
            ...
            (blendedImageAttack(
                trans(
                    # '../data/hello_kitty.jpeg'
                    imageio.imread(args.attack_trigger_img_path)
        ...
        train_bd_transform = general_compose([
            ...
            (SSBA_attack_replace_version(
                # '../data/cifar10_SSBA/train.npy'
                replace_images=np.load(args.attack_train_replace_imgs_path)
            ), True),
            (npClipAndToUint8, False),
        ])
        test_bd_transform = general_compose([
            ...
            (np.array, False),
            (SSBA_attack_replace_version(
                # '../data/cifar10_SSBA/test.npy'
                replace_images=np.load(args.attack_test_replace_imgs_path)
            ), True),
            (npClipAndToUint8, False),
        ])

    elif args.attack in ['label_consistent']:
        add_trigger = labelConsistentAttack(
            reduced_amplitude=args.reduced_amplitude)
        ...
        if len(triggerArray.shape) == 4:
            logging.info(
                "Get lowFrequency trigger with 4 dimension, take the first one")
            ...
            raise ValueError(
                "lowFrequency trigger shape error, should be either 2 or 3 or 4")

        logging.info("Load lowFrequency trigger with shape {}".format(
            triggerArray.shape))
