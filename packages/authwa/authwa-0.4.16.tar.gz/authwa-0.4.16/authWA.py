...
class WA:
    ...
    def sql_connection(self, uid:str, pwd:str, server:str, database:str, driver=None, bd=DBs[0]):
        uid = quote_plus(uid)
        pwd = quote_plus(pwd)
        server = quote_plus(server)
        database = quote_plus(database)
        driver = quote_plus(driver)
        if bd == 'MSSQL': url = f'mssql://{uid}:{pwd}@{server}/{database}?driver={driver}&&TrustServerCertificate=yes'
...
class Parcial:
    def __init__(self, uid, pwd, server, database, driver, bd=DBs[0], hora_inicio = 0, hora_final = 23,):
        self.hora_inicio = hora_inicio
        self.hora_final = hora_final
        self.whats = WA()
        
        if uid and pwd and server and database:
            self.engine = self.whats.sql_connection(uid, pwd, server, database, driver, bd)
