        ...
        if influx.check_port():
            ...
            try:
                influx.influxwrite( DATABASE="local",
                                    ...
                                    values=commavalues,
                                    IP="127.0.0.1" )
                print(f"i...  {fg.violet}  INFLUX => DB:telematix  {fg.default}")
            except:
                print("X... {fg.violet}ERROR  WRITING  INFLUX {fg.default}")
...
def watch_mqtt(POPO, MACHINE="127.0.0.1", PORT=1883):  # ++++++++++++++++++++++++++++++++++++++++++++THREAD
    ...
    client.subscribe("#")
    print(f"i...   {bg.violet} ... MQTT subscribing  ALL  #   {bg.default}")
...
watch -n 5 'echo  "nfsrpi P1= 0.00E+0 _ P2= 0.00E+0 _ FL= 0.00E+0 _ CU= 0.00E+0 _ SP= 2.00E+0 _ PO= 1.00E+0" | nc -w 1 -u 127.0.0.1 8100'
    ...
    while True:
        ...
        print(f"{fg.yellow}{data} @{address[0]}{fg.default}" )
        ...
        if influx.check_port():
            ...
            try:
                ...
                #                    values=commavalues,
                                    IP="127.0.0.1" )
                print(f"i... {fg.yellow}Influx  DB:{measu}{fg.default} ")
