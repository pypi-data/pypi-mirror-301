...
class VoyagerDetect():
    ...
    def upload_files(self,files):
        ...
        if login_status['status']:
            try:
                response = self.req.post(self.api_url + config.VOYAGER_UPLOAD_API, data=data, files=files)
            ...
            if scan_status:
                if scan_status['success']:
                    ...
                    while True:
                        ...
                        if datetime.now() > scan_run_time + timedelta(seconds=self.time_wait_for_report):
