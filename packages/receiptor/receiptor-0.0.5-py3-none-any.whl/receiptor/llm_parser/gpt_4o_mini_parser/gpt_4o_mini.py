...
class DocumentStructureExtractor:
    ...
    def ensure_token_limit( text: str,max_tokens:int,llm_model :Optional[str] = 'gpt-4o-mini') -> str:
        ...
        if len(tokens) > max_tokens:
            ...
            return text
    @staticmethod
    def structure_document_data(raw_text: str,openai_api_key: Optional[str] = os.getenv('OPENAI_API_KEY'), org_id: Optional[str] = os.getenv('ORG_ID'),
        ...
        # limited_prompt = DocumentStructureExtractor.ensure_token_limit(prompt_to_llm,max_tokens)
        # print(f"Number of tokens: {len(tiktoken.encoding_for_model(llm_model).encode(limited_prompt))}")
        # print(prompt_to_llm)
