...
_IGNORE_TEXT = '<kdb-ignore>'
_SKIP_TEXT = '<kdb-skip>'
_WARNING_IGNORE_VALUE = 'The {} action is ignored cause by your value inputted that is <kdb-ignore> or <kdb-skip>'
...
def _compare_equals(system_value, kdb_value) -> bool:
    try:
        ...
        if system_value is not None and isinstance(system_value, str):
            return str(kdb_value) == system_value.encode('unicode_escape').decode()
