...
"""
    ...
    try:
        ...
        r_generated=True
        #pdb.set_trace()
        retcode=ftbl2optR_kvh.main(opt4py, wout=flog.write, werr=ferr.write)
        if retcode != 0:
            r_generated=False
            if dirres:
                s="=>Check "+ferr.name+"\n"
...
ord_args = []
# my install dir
dirinst=Path(influx_si.__file__).parent
dirbin=dirinst / "bin"
dirr=dirinst / "R"
...
Call influx_s.main(["-h"]) for a help message"""
    ...
    if dict_opts["install_rdep"]:
        ...
        if os.name == 'nt':
            ...
            p.stdin.write(("source('"+str(dirinst / "R" / "upd_deps.R")+"')\n").encode())
            p.stdin.flush()
            import msvcrt
            while True:
    ...
    if len(rfiles) > 1:
        ...
        if not retcode and rfiles and not dict_opts["nocalc"]:
            ...
            rarg=f"""source("{dirr / 'influx_si.R'}", echo=FALSE); e=new.env(); prep("{fp.with_suffix(".kvh")}", e); calc(e); save(e, file="/home-local/sokol/tmp/e.RData")"""
            if flog:
