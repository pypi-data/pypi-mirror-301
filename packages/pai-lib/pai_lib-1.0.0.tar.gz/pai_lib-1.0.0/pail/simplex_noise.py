...
def _init(img, seed=3):
    ...
    perm = np.zeros(img.shape[0], dtype=np.int64)
    perm_grad_index3 = np.zeros(img.shape[0], dtype=np.int64)
    source = np.arange(img.shape[0])
...
def noise3array(x, y, z, img):
        
        
        seed = np.random.randint(-10000000000, 10000000000)
        _perm, _perm_grad_index3 = _init(img, seed=seed)

        return _noise3a(x, y, z, _perm, _perm_grad_index3)

def rand_3d_octaves(img,shape, octaves=1, persistence=0.5, frequency=32):
    """
    ...
    for _ in range(octaves):
        noise += amplitude * noise3array(x / frequency, y / frequency, z / frequency, img)
        frequency /= 2
...
def get_data_transforms(image):
    data_transforms = transforms.Compose([normalize(image),\
                    ToTensor()])
