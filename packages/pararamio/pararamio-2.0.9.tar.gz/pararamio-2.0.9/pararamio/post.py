...
class Post:
    ...
    def __init__(self, chat: 'Chat', post_no: int, load_on_key_error: bool = True, **kwargs):
        self._chat = chat
        if post_no is None:
    ...
    def __getattr__(self, key: str) -> Any:
        return get_formatted_attr_or_load(
            self, key, ATTR_FORMATTERS, self.load if self._load_on_key_error else None
        )
    ...
    def _compare_validations(self, other: 'Post'):
        if not isinstance(other, Post):
