...
import actynf
from m1_IO_model import neurofeedback_training as IO_neurofeedback_training
...
def generate_a_parameter_list(A_priors,a_priors,io_std) :
    # Undordered dictionnaries are soooo not cool :(
    simulation_index_list = []    
    for kA in range(len(A_priors)):
        for ka in range(len(a_priors)):
            for kio in range(len(io_std)) :
                this_sim = {
                    "filename" : "_" + str(A_priors[kA]) + "_" + str(a_priors[ka])+"_"+str(io_std[kio])+"_.simu",
                    "A_std" : A_priors[kA],
                    "a_std" : a_priors[ka],
                    "io_std" : io_std[kio]
                }
...
if __name__ == "__main__":
    ...
    Kw = 10
    true_io_std_array = list(np.linspace(0.001,2.0,Kw))

    full_parameter_list = generate_a_parameter_list(true_fb_std_array,belief_fb_std_array,true_io_std_array)
    try :
        param_dict = full_parameter_list[list_index]
        a_std = param_dict["a_std"]
        A_std = param_dict["A_std"]
        io_std = param_dict["io_std"]
    ...
    k1a1 = 100.0
    epsilon_a = 0.01 # a0 = norm((1/101)* ones + gaussian_prior)*k1a
    k1a2 = 10.0
    ...
    net = IO_neurofeedback_training(T,Th,  # Trial duration + temporal horizon
            ...
            a_std,A_std,io_std,   # Subject belief about feedback noise / true feedback noise
