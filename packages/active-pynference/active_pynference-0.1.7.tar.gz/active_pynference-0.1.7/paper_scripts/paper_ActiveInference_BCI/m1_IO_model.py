...
def m1_IO_weights(Ns,No,
            feedback_noise_std = 0.1,intero_noise_std = 0.1,
            factor_useless_actions= 2,
            k1b = None, epsilon_b = 0.0,
            k1a1 = None,epsilon_a = 0.0,k1a2 = None,
    ...
    if (actynf.isField(k1a1)):
        # If this is the subject model
        a_fb = observation_matrix(Ns,No,feedback_noise_std,k1a1,epsilon_a)
    else :
        a_fb = observation_matrix(Ns,No,feedback_noise_std,1.0,0.0)
    
    if (actynf.isField(k1a2)):
        # If this is the subject model
        # No_intero = Ns
        a_intero = k1a2*actynf.normalize(np.ones((Ns,Ns)))
    else :
        # No_intero = Ns
        a_intero = observation_matrix(Ns,Ns,intero_noise_std,1.0,0.0)
    
    a = [a_fb,a_intero]

    kC = 2.0
    c = [np.linspace(0,kC*(No-1),No),np.zeros((Ns,))]
...
def subject_model(T,Th,
                  Ns,No,feedback_std,intero_noise_std,
                  k_useless_actions,
                  k1b, epsilon_b,
                  k1a1, epsilon_a,k1a2,
                  k1d, epsilon_d,
                  ...
                  asit=1.0,learn_a=True):
    
    a,b,c,d,e,u  = m1_IO_weights(Ns,No,
        feedback_noise_std=feedback_std,intero_noise_std = intero_noise_std,
        factor_useless_actions=k_useless_actions,
        k1b=k1b,epsilon_b=epsilon_b,
        k1a1=k1a1,epsilon_a=epsilon_a,k1a2=k1a2,
...
def neurofeedback_process(T,Ns,No,feedback_std,intero_noise_std,
                  k_useless_actions,
                  decay_probability = 0.5,
                  action_effect_probability = 0.9):
...
def neurofeedback_training(T,Th,
                           Ns_subj,Ns_proc,
                           No_subj,No_proc,
                           feedback_std_subj,feedback_std_proc,intero_std_proc,
                           k_useless_actions,
                           k1b,epsilon_b,
                           k1a1,epsilon_a,k1a2,
                           k1d,epsilon_d,
                           decay_p,action_effect_p,
                           asit=32,learn_a=True):
    
    process = neurofeedback_process(T,Ns_proc,No_proc,feedback_std_proc,intero_std_proc,
                                    k_useless_actions,
                                    # NO PRIOR WEIGHTS HERE
                                    decay_probability=decay_p,
                                    action_effect_probability=action_effect_p)
    
    model = subject_model(T,Th,Ns_subj,No_subj,feedback_std_subj,intero_std_proc,
                          k_useless_actions,
                          k1b,epsilon_b,
                          k1a1,epsilon_a,k1a2,
                          k1d,epsilon_d,
    ...
    model.inputs.o = actynf.link(process, lambda x : np.array([np.round(No_subj*(x.o[0]/No_proc)),x.o[1]]).astype(int))
