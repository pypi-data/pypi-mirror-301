...
class ProcessedData:
    def __init__(self, machine_data):
        ...
        if self._name == "jurecadc":
            ...
            node_scale_factor = 1
        elif self._name == "juwels_booster":
            node_scale_factor = 0.5
        else:
...
def aggregate_data(experiment_array):
    ...
    aggregate["juwels_booster"] = MachineData("juwels_booster")
    ...
    for experiment in experiment_array:
        ...
        elif experiment.json_data()["experiment"]["system"] == "juwels_booster":
            aggregate["juwels_booster"].add_data(data["nodes"], data["runtime"])
...
def compute_plot_range_limits(processed_data):
    ...
    range_limits["ylim"] = [0.75 * range_limits["ylim"][0], 1.25 * range_limits["ylim"][1]]
    range_limits["xticks"] = sorted(list(set(range_limits["xticks"])))
    range_limits["xticks"] = [int(p) for p in range_limits["xticks"]]


    logylims = [math.log(range_limits["ylim"][0]), math.log(range_limits["ylim"][1])]
    range_limits["yticks"] = numpy.logspace(logylims[0], logylims[1], num=6, base=math.e)
...
def generate_plot_pdf_file(plotting_data, output_dir):
    ...
    marker_array = itertools.cycle(("o"))
