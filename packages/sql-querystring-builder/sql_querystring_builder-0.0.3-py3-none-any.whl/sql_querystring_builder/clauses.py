...
class Where(Clause):
    ...
    def build(self) -> str:
        ...
        for condition in self.conditions:
            ret.append(condition.build(False, True) if isinstance(condition, Aliasable) else condition)
        return " ".join(ret)
...
class InnerJoin(Clause):
    ...
    def build(self) -> str:
        return f"INNER JOIN {self.table.build(use_name=True)} on {self.on_left.build()} = {self.on_right.build(False, True)}"
