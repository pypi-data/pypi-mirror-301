    ...
    def validate(parameter_detail_service):
        for key, value in vars(ParameterDetailConsts).items():
            ...
            if PDLookup.get_id(value) is None:
                MigrationLog.log_const_update(f"Parameter detail {value} did not exist but is required and is being added to CRUISE schema.")
                detail = parameter_detail_service.save_new_parameter_detail(CruiseParameterDetail(parameter=value, parameter_type=None, description=None))
