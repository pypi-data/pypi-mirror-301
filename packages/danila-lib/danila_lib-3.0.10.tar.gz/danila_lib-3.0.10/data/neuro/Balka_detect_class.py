    ...
    def balka_detect(self, img_path):
        ...
        if len(rects) == 2:
            ...
            if per_cent > 0.3:
                ...
                min_xmin = min(rects[1].xmin, rects[2].xmin)
                max_xmin = max(rects[1].xmin, rects[2].xmin)
                min_xmax = min(rects[1].xmax, rects[2].xmax)
                max_xmax = max(rects[1].xmax, rects[2].xmax)
                per_cent = (min_xmax - max_xmin) / float(max_xmax - min_xmin)
                if (rects[1].ymax - rects[1].ymin) < (rects[2].ymax - rects[2].ymin):
                    rects.remove(rects[1])
                else:
                    rects.remove(rects[2])
        return rects
