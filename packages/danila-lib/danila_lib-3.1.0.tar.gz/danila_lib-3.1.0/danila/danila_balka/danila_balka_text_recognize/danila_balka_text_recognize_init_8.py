...
from danila.danila_balka.danila_balka_text_recognize.danila_balka_text_recognize_model_5 import \
    Danila_balka_text_recognize_model_5
...
class Danila_balka_text_recognize_init_8:
    def __init__(self, yolov5_dir):
        danila_balka_text_recognize_tihvin_model = Danila_balka_text_recognize_model_5()
        ...
        model_tihvin = danila_balka_text_recognize_tihvin_model.get_model(yolov5_dir)
        self.prod_models = {}
        for balka_prod in Balka_Prod:
            if balka_prod == Balka_Prod.altai:
                ...
                self.prod_models[balka_prod] = Danila_balka_text_recognize_work(
                    text_recognize_model= model_tihvin,
                    prod_coefficients=danila_balka_text_recognize_tihvin_model.get_prod_coefficients(balka_prod)
                )
            else:
                self.prod_models[balka_prod] = Danila_balka_text_recognize_work(
