...
from data.result.Image_text_areas_altai import Image_text_areas_altai
from data.result.Text_area_altai import Text_area_altai


"""module for detecting text in rama"""
class Balka_text_detect_altai_class:
    ...
    def text_detect(self, img_path):
        ...
        image_text_areas = Image_text_areas_altai()
        ...
        for image_text_areas_list_el in image_text_areas_list.letters:
            ...
            if image_text_areas_list_el.letter == 'number':
                ...
                dict['class'] = 1
            elif image_text_areas_list_el.letter == 'seria':
                dict['class'] = 2
            elif image_text_areas_list_el.letter == 'sran':
                dict['class'] = 3
        ...
        for text_area_json in json_res_1:
            if text_area_json['confidence']>0.25:
                text_area = Text_area_altai(text_area_json)
    ...
    def draw_text_areas_in_opencv(self, img_text_areas_in_two_balkas, imgs):
        """draw img_text_areas on img, returns opencv img"""
        colors = [(0,0,255), (0,255,0), (0,0,0), (0,255,255), (255,255,255), (255,0,0)]
        i = 0
    ...
    def work_text_areas_for_prod(self, left_half_balka, right_half_balka):
        if len(left_half_balka.areas[Class_text_altai.year]) > 0:
            left_half_balka.areas[Class_text_altai.year] = []
        if len(right_half_balka.areas[Class_text_altai.prod]) > 0:
            right_half_balka.areas[Class_text_altai.prod] = []
