...
class Danila_rama_text_recognize_model_1(Danila_rama_text_recognize_model):
    def get_model(self, yolo_path):
        return Text_Recognize_yolo(model_path= LOCAL_RAMA_TEXT_RECOGNIZE_MODEL_ADDRESS, yolo_path=yolo_path)
    def get_prod_coefficients(self, rama_prod):
        if rama_prod == Rama_prod.Rama_Prod.altai:
            ...
            return Prod_coefficients(
                number_coefficients=Text_coefficients(
                    length=6, height=96, width=224
                ),
                prod_coefficients=Text_coefficients(
                    length=2, height=64, width=128
                ),
                year_coefficients=Text_coefficients(
                    length=2, height=64, width=96
                ),
