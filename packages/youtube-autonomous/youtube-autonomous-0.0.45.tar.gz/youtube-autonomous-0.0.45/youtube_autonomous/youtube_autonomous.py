    ...
    def validate_data(self, json_data: dict):
        ...
        for index, segment in enumerate(segments):
            ...
            if type == SegmentType.PREMADE.value:
                ...
                PremadeElementBuilder.extract_extra_params(segment, PremadeElementBuilder.premade_name_to_class(keywords).__init__, ['self', 'cls', 'args', 'kwargs', 'duration'])
            ...
            for index, enhancement in enumerate(segment.get(SegmentField.ENHANCEMENTS.value, [])):
                ...
                if enhancement_type == EnhancementType.PREMADE.value:
                    # This will raise an Exception if no mandatory parameters are given
                    PremadeElementBuilder.extract_extra_params(enhancement, PremadeElementBuilder.premade_name_to_class(keywords).__init__, ['self', 'cls', 'args', 'kwargs', 'duration'])
                elif enhancement_type == EnhancementType.EFFECT.value:
                    effect_class = EffectElementBuilder.effect_name_to_class(keywords)
