...
class Shortcode:
    ...
    def __init__(self, tag: ShortcodeTagType, type: ShortcodeType, attributes: dict, context, content: str):
        """
        ...
        self.config = Configuration.get_configuration_by_type(self.type).config_as_shortcode
    ...
    @tag.setter
    def tag(self, tag: Union[ShortcodeTagType, str]):
        self._tag = ShortcodeTagType.to_enum(tag)
    ...
    def type(self, type: Union[ShortcodeType, str]):
        self._type = ShortcodeType.to_enum(type)
    ...
    def duration(self, duration: Union[ShortcodeDuration, int, float, None]):
        ...
        self._duration = ElementValidator.validate_shortcode_duration_field(duration)
    ...
    def mode(self, mode: Union[ShortcodeMode, str]):
        ...
        ElementParameterValidator
        ShortcodeMode.to_enum(mode)

        if mode not in self.config.modes:
            raise Exception(f'The provided "{self.mode.value}" is not a valid mode for this shortcode type "{self.type.value}" and tag "{self.tag.value}".')
