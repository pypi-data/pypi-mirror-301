    ...
    def validate_data(self, json_data: dict):
        ...
        for index, segment in enumerate(segments):
            ...
            if type == SegmentType.PREMADE.value:
                ...
                PremadeElementBuilder.extract_extra_params(segment, PremadeElementBuilder.premade_name_to_class(segment.get(SegmentField.KEYWORDS.value, None)).__init__)
            ...
            for index, enhancement in enumerate(segment.get(SegmentField.ENHANCEMENTS.value, [])):
                ...
                # TODO: Validate, if premade or effect, that 'extra_data' has
                # needed fields
                if type == EnhancementType.PREMADE.value:
                    # This will raise an Exception if no mandatory parameters are given
                    PremadeElementBuilder.extract_extra_params(enhancement, PremadeElementBuilder.premade_name_to_class(enhancement.get(EnhancementField.KEYWORDS.value, None)).__init__)
                elif type == EnhancementType.EFFECT.value:
                    # TODO: This depends on the effect we are trying to apply
                    effect_class = EffectElementBuilder.effect_name_to_class(enhancement.get(EnhancementField.KEYWORDS.value, None))
                    if issubclass(effect_class, MoviepyEffect):
                        # Params are in the init, removing 'clip'
                        EffectElementBuilder.extract_extra_params(enhancement, EffectElementBuilder.effect_name_to_class(enhancement.get(EnhancementField.KEYWORDS.value, None)).__init__, params_to_ignore = ['self', 'cls', 'args', 'kwargs', 'clip'])
                    elif issubclass(effect_class, VideoEffect):
                        # Params are in the apply, removing 'video'
                        EffectElementBuilder.extract_extra_params(enhancement, EffectElementBuilder.effect_name_to_class(enhancement.get(EnhancementField.KEYWORDS.value, None)).apply, params_to_ignore = ['self', 'cls', 'args', 'kwargs', 'video'])
                # TODO: Validate for another types
