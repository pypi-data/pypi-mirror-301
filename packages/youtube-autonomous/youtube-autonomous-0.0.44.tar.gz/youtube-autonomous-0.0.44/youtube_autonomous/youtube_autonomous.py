    ...
    def validate_data(self, json_data: dict):
        ...
        for index, segment in enumerate(segments):
            ...
            if type == SegmentType.PREMADE.value:
                ...
                TextElementBuilder.extract_extra_params(segment, TextElementBuilder.text_premade_name_to_class(segment.get(SegmentField.KEYWORDS.value, None), ['self', 'cls', 'args', 'kwargs', 'output_filename']).generate)
            ...
            for index, enhancement in enumerate(segment.get(SegmentField.ENHANCEMENTS.value, [])):
                ...
                if enhancement_type == EnhancementType.PREMADE.value:
                    ...
                    if issubclass(effect_class, MoviepyEffect):
                        ...
                        EffectElementBuilder.extract_extra_params(enhancement, effect_class.__init__, params_to_ignore = ['self', 'cls', 'args', 'kwargs', 'clip'])
                    elif issubclass(effect_class, VideoEffect):
                        # Params are in the apply, removing 'video'
                        EffectElementBuilder.extract_extra_params(enhancement, effect_class.apply, params_to_ignore = ['self', 'cls', 'args', 'kwargs', 'video'])
