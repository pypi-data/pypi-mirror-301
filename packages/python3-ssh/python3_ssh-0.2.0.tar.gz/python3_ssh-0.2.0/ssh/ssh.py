    ...
    def _exec(self, cmd, promt_response,timeout=60):
        ...
        try:
            if promt_response:
                ...
                ssh_tty = self.__ssh.invoke_shell()
                command = f"nohup {cmd} > /dev/null 2>&1 &\n"
                ssh_tty.send(command)
                output=""
                while True:
                    if ssh_tty.recv_ready():
                        output += ssh_tty.recv(1024).decode()
                    else:
                        break
                std_output=output
                exit_status=ssh_tty.recv_exit_status()
                std_err=ssh_tty.recv_stderr()
                log.info(
