...
model_name  = 'GWSkyNet_v2.5.0'  # the model that is actually used.
...
def prepare_data(fits_file):
    ...
    # following https://git.ligo.org/lscsoft/ligo.skymap/-/blob/v2.1.1/ligo/skymap/distance.py?ref_type=tags#L289-298 to create a multi-order structure table
    npix = len(prob)
    nside = astropy_healpix.npix_to_nside(npix)
    ipix = numpy.arange(npix)
    skymap_moc = Table({
        'UNIQ': astropy_healpix.level_ipix_to_uniq(numpy.log2(nside), ipix),
        'PROBDENSITY': prob,
        'DISTMU': mu,
        'DISTSIGMA': sigma,
        'DISTNORM': norm
