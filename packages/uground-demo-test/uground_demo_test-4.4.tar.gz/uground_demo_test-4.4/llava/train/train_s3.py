...
class LazySupervisedDataset(Dataset):
    def __init__(self, data_path: str,
        ...
        with self.s3.open(data_path, 'rb') as f:
            parquet_file = pq.ParquetFile(f)
            self.parquet_metadata = parquet_file.metadata
        self.total_rows = sum(self.parquet_metadata.row_group(i).num_rows for i in range(self.parquet_metadata.num_row_groups))
    ...
    def _get_single_item(self, idx):
        ...
        with self.s3.open(self.data_path, 'rb') as f:
            # parquet_file = pq.ParquetFile(f)
            # num_row_groups = parquet_file.num_row_groups
            num_row_groups=self.parquet_metadata.num_row_groups
            ...
            # table = parquet_file.read_row_group(chunk_idx)
            table=pq.read_table(f, row_indices=chunk_idx)
