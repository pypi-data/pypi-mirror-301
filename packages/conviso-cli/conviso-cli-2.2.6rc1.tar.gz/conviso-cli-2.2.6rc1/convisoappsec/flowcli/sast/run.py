...
def deploy_results_to_conviso_beta(conviso_api, results_filepaths, asset_id, commit_ref=None, deploy_id=None):
                ...
                for issue in issues:
                    ...
                    try:
                        ...
                        if error.code == 'RECORD_NOT_UNIQUE':
                            ...
                            LOGGER.warn(f"⚠️ Error while sending issue to Conviso: {error}")
                            errors_occurred += 1
    ...
    LOGGER.info(f"💬 {duplicated_issues} issue(s) already on the Conviso Platform due to internal validation.")

    if errors_occurred > 0:
        LOGGER.warn(f"⚠️ {errors_occurred} error(s) occurred during processing.")
        ...
        if send_to_flow:
            if experimental:
                deploy_results_to_conviso_beta(
                    conviso_beta_api, results_filepaths, asset_id, commit_ref=end_commit, deploy_id=deploy_id
                )
