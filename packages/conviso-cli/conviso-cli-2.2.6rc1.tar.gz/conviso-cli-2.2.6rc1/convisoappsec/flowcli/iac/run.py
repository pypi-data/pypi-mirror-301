...
def run(context, flow_context, project_code, asset_id, company_id, repository_dir, send_to_flow, scanner_timeout,
    ...
    perform_command(
        flow_context, context.params['project_code'], context.params['asset_id'], context.params['repository_dir'],
...
def deploy_results_to_conviso_beta(
        conviso_api, results_filepaths, asset_id, deploy_id, commit_ref=None
):
    ...
    with results_context as reports:
        for report_path in reports:
            try:
                ...
                LOGGER.warn(f"⚠️ Error processing report file: {e}")
                continue
            ...
            for issue in sarif_data:
                ...
                try:
                    ...
                    if error.code == 'RECORD_NOT_UNIQUE':
                        ...
                        LOGGER.warn(f"⚠️ Error creating issue: {error}")
                except Exception as e:
                    # Log any other issue errors and continue
                    LOGGER.warn(f"⚠️ Unexpected error creating issue: {e}")
                    continue
...
def perform_command(
        flow_context, project_code, asset_id, repository_dir, send_to_flow, scanner_timeout, deploy_id, experimental
):
        ...
        if send_to_flow:
            ...
            if experimental:
                ...
                deploy_results_to_conviso_beta(
                    conviso_beta_api, results_filepaths, asset_id, deploy_id=deploy_id, commit_ref=end_commit
                )
            else:
