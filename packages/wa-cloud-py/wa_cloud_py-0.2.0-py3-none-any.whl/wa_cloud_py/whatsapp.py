...
class WhatsApp:
    ...
    def parse(self, request_data: dict) -> Union[UserMessage, None]:
        ...
        if len(msg_value.get("messages", [])) > 0:
            ...
            if message_type == MessageType.TEXT:
                ...
                return ImageMessage(msg_value.get("messages")[0], user=user)

            elif message_type == MessageType.VIDEO:
                return VideoMessage(msg_value.get("messages")[0], user=user)

            elif message_type == MessageType.AUDIO:
                return AudioMessage(msg_value.get("messages")[0], user=user)

            elif message_type == MessageType.DOCUMENT:
                return DocumentMessage(msg_value.get("messages")[0], user=user)
    ...
    def download_media(
        self, media_id: str, filename: str, save_path: str = "/assets"
    ) -> Tuple[bool, dict]:
        """
        ...
        Args:
            ...
            save_path (str): The path to save the downloaded media to.

        Returns:
            Tuple[bool, dict]: A tuple containing a boolean indicating whether the media was downloaded successfully and a
            dictionary containing the response data.
        """
        ...
        if "url" in res_json:
            ...
            media_url = res_json.get("url")
            media_mime_type = res_json.get("mime_type")
            media_res = requests.get(media_url, headers=self.headers)

            ext = MIME_TYPES.get(media_mime_type, "")
