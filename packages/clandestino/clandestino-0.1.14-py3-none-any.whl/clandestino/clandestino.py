...
def _load_migration_repository() -> cdti.IMigrateRepository:
    ...
    elif config("CLANDESTINO_MIGRATION_REPO", default=False) == "MINIO":
        return _cdt_minio.repository.MinIOMigrateRepository()
    elif config("CLANDESTINO_MIGRATION_REPO", default=False) == "QDRANT":
        return _cdt_qdrant.repository.QdrantMigrateRepository()
...
def _get_ref_migration(migration_type: str = None) -> str:
    ...
    elif migration_type == "MINIO":
        return _cdt_minio.ref.__file__
    elif migration_type == "QDRANT":
        return _cdt_qdrant.ref.__file__
    else:
        from .src import ref
...
def load():
    ...
    if not _loaded:
        ...
        try:
            import clandestino_minio

            global _cdt_minio
            _cdt_minio = clandestino_minio
        except ImportError as error:
            if config("CLANDESTINO_MIGRATION_REPO", default=False) == "MINIO":
                raise Exception(
                    "You define to use MINIO but not install the extra package clandestino_minio"
                )

        try:
            import clandestino_qdrant

            global _cdt_qdrant
            _cdt_qdrant = clandestino_qdrant
        except ImportError as error:
            if config("CLANDESTINO_MIGRATION_REPO", default=False) == "QDRANT":
                raise Exception(
                    "You define to use QDRANT but not install the extra package clandestino_qdrant"
                )
    _loaded = True
