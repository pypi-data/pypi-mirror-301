...
class ReuseOperation:
    ...
    def fetch_local_conf(vit_dir: Union[str, "Path"], inner_settings: dict) -> dict:
        ...
        if "redis:uri" in cfg:
            inner_settings["REDIS_CONFIG"] = {
                "uri": cfg.get("redis:uri", "uri", fallback=None)
            }
        elif "redis" in cfg:
            redis_section = cfg["redis"]
            inner_settings["REDIS_CONFIG"] = {
                "ip_ports": redis_section.get("ip_ports", "localhost:6379"),
                "db": redis_section.getint("db", None),
                "user_pass": redis_section.get("user_pass", None),
                "decode_responses": redis_section.getboolean("decode_responses", True),
            }
