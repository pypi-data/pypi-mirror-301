...
def get_network_image_bytes(url):
...
def image_path_to_bytes(path: str):
    """
    路径读取
    1. Image.open(fg_url_cv)：fg_url_cv=io.BytesIO(response.content)
    2. Image.open(path)
    """
    bg_image = Image.open(path)
    bg_byte_array = io.BytesIO()
    bg_image.save(bg_byte_array)
    background_bytes = bg_byte_array.getvalue()
