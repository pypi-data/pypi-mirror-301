...
if typing.TYPE_CHECKING:
    class LayerToggleJson(typing_extensions.TypedDict):
        area_id: int
...
class LayerToggle(BaseProperty):
    area_id: AssetId = dataclasses.field(default=default_asset_id, metadata={
        'asset_types': [], 'reflection': FieldReflection[AssetId](
            AssetId, id=0x00000000, original_name='Area ID'
        ),
    })
    layer_id: SavedStateID = dataclasses.field(default_factory=SavedStateID, metadata={
        'reflection': FieldReflection[SavedStateID](
            SavedStateID, id=0x00000001, original_name='Layer ID', from_json=SavedStateID.from_json, to_json=SavedStateID.to_json
        ),
    })
    ...
    def from_json(cls, data: json_util.JsonValue) -> typing_extensions.Self:
        json_data = typing.cast("LayerToggleJson", data)
        return cls(
