"""
Main module to run the availability checker for the "temperaturaSts" column

rules: 
    - the temperaturaSts must be != 0
"""
import pandas as pd
...
class STSTemperatureAvailabilityChecker:
    """
    ...
    def _mark_invalid_records(self) -> pd.DataFrame:
        ...
        ***
        A record is invalid if the temperaturaSts is 0
        ***
        ...
        if not validate_if_required_columns_are_in_data(self.data, required_columns):
            message = f"Columns {required_columns} not found in the data to get the availability for temperaturaSts"
            raise ValueError(message)

        # Initialize the column temperaturaStsFailures with 0
        self.data["temperaturaStsFailures"] = 0
        ...
        for vehicle in unique_vehicles:
            ...
            # Check if the temperaturaSts is 0
            invalid_temperature = vehicle_data["temperaturaSts"] == 0

            self.data.loc[vehicle_data.index, "temperaturaStsFailures"] = invalid_temperature.astype(int)

        return self.data

    def get_availability(self) -> dict:
        """
        Method to get the availability for the temperaturaSts

        Returns:
            dict: The availability data for the temperaturaSts
        """
        self.data = self._mark_invalid_records()
        common_calcs = CommonCalcsForAvailability(self.data, "temperaturaStsFailures")
        availability_data: pd.DataFrame = common_calcs.calculate_availability()
        availability_data["variable"] = "temperaturaSts"

        response = {
            "availability_data": availability_data,
            "data_for_global": self.data[["idVehiculo", "fechaHoraLecturaDato", "temperaturaStsFailures"]]
        }
