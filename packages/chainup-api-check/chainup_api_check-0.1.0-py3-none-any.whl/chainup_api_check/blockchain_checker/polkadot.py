import time
from chainup_api_check.model.blockchain_status import BlockchainStatus
from chainup_api_check.util.send_jsonrpc import send_rpc_request


class Checker:
    def __init__(self, domain, protocol, network, token):
        self.domain = domain
        self.protocol = protocol
        self.network = network
        self.token = token

        self.provider_url = self._build_provider_url()

    def _build_provider_url(self):
        if self.protocol == "avax":
            return f"https://{self.domain}/{self.protocol}/{self.network}/{self.token}/ext/bc/C/rpc"
        # elif self.protocol == "tron":

        return f"https://{self.domain}/{self.protocol}/{self.network}/{self.token}"

    def curl_command(self):
        url = self._build_provider_url()
        data = """-H 'Content-Type: application/json' -d '{"jsonrpc":"2.0","method":"system_syncState","params":[],"id":1}'"""
        return f"curl -i {url} {data}"

    def get_current_block_height(self):
        """
        获取当前块高
        """
        return send_rpc_request(url=self._build_provider_url(), method="system_syncState")

    def get_status(self):
        """
        获取当前块高、是否同步最新以及请求延迟
        """
        start_time = time.time()
        data, code = self.get_current_block_height()
        end_time = time.time()
        is_healthy = False
        current = 0
        target = 0
        if data is not None:
            current = data["currentBlock"]
            target = data["highestBlock"]
        if target != 0 and current != 0:
            is_healthy = target - current <= 100

        command = self.curl_command()
        request_delay = end_time - start_time

        status = BlockchainStatus(protocol=self.protocol, network=self.network, is_healthy=is_healthy,
                                  height=current, code=code, request_delay=request_delay, command=command)

        return status
