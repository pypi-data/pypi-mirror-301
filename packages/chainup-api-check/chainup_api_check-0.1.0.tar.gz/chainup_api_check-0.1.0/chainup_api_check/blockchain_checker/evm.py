...
class Checker:
    ...
    def _build_provider_url(self):
        if self.protocol == "avax":
            return f"https://{self.domain}/{self.protocol}/{self.network}/{self.token}/ext/bc/C/rpc"
        elif self.protocol == "filecoin":
            return f"https://{self.domain}/{self.protocol}/{self.network}/{self.token}/rpc/v1"

        return f"https://{self.domain}/{self.protocol}/{self.network}/{self.token}"

    def curl_command(self):
        url = self._build_provider_url()
        data = """-H 'Content-Type: application/json' -d '{"jsonrpc":"2.0","method":"eth_syncing","params":[],"id":1}'"""
    ...
    def get_current_block_height(self):
        ...
        获取当前块高
        """
        return send_rpc_request(url=self._build_provider_url(), method="eth_blockNumber")

    def is_syncing(self):
        """
        检查节点是否正在同步
        """
        result, code = send_rpc_request(
            url=self._build_provider_url(), method="eth_syncing")
        if self.protocol == "tron" and code == 200:
            return True
    ...
    def get_status(self):
        ...
        block_height, code = self.get_current_block_height()
        end_time = time.time()
        is_healthy = self.is_syncing()
