...
from ._internal_utils import TokenHolder, _get_azure_storage_url, _sanitize_url, _insert_proxy_auth, \
    _parse_azp_from_token, _sas_token_valid, _is_browser_available
...
class SDKClient:
    ...
    def __init__(self, logger: Logger = None, **kwargs: Union[str, int]) -> None:
        ...
        self.accessmanager_url = f'https://{self._env.domain}/accessmanager/api/v2.0/accessmanager/'
    ...
    def _get_scroll_id_and_documents(self, url: str, headers: Dict[str, str]) -> Tuple[
    ...
    def index_get_documents(self, index: str, scroll_duration: str = '10m', batch_size: int = 10000) -> Generator[
    ...
    def storage_upload_files(self, organization: str, space: str, files: List[str], local_dir: str,
        ...
        timestamp = datetime.now().strftime('%Y%m%d%H%M%S')  # timestamp to create the directory with
        _global_logger.debug(f'uploading to directory {timestamp}')
        files_log = "\n  " + "\n  ".join(files)
        ...
        for file in files:
            ...
            if not _sas_token_valid(blob_url):
                _global_logger.debug(f'renew expired sas-token')
                sas_token = self._get_azure_sas_token(organization, space, 'upload')
                blob_url = _sanitize_url(f'{storage_url}/{timestamp}/{file}?{sas_token}')
            _global_logger.info(f'uploading file {file_path}...')
            if progress_callback:
                self._upload_file_azure_chunked(blob_url, file_path, progress_callback)
            else:
                self._upload_file_azure(blob_url, file_path)
            _global_logger.info(f'uploading file {file_path}... Done')

        _global_logger.debug(f'Uploading files... Done')
        # start the ingest workflow
        _global_logger.debug(f'Comitting upload {organization}/{space}/{timestamp}...')
        url = f'{self.accessmanager_url}/commit?organization={organization}&space={space}&rootDir={timestamp}'
        ...
        response.raise_for_status()
        _global_logger.debug(f'Comitting upload {organization}/{space}/{timestamp}... Done')
    ...
    @staticmethod
    def _upload_file_azure_chunked(blob_url: str, file_path: str, progress_callback=None) -> None:
        """
        Upload files to Azure
        ...
        block_list = []  # TODO: possible memory-leak??
            ...
            if not blob_client.exists():
                with open(file_path, "rb") as data:
                    ...
                    for chunk_number in range(num_chunks):
                        _global_logger.debug(f'upload chunk {chunk_number}')
                    ...
                    # Final commit for any remaining blocks
                    if block_list:
    ...
    def _get_azure_sas_token(self, organization: str, space: str, reqtype: str = 'read') -> str:
        """
        Generates an sas-token via accessmanager (version > 1)
        ...
        """
        url = f'{self.accessmanager_url}/{reqtype}?organization={organization}&space={space}'
        url = _sanitize_url(url)
