...
def parse_langchain_llm_result(result: LLMResult):
    ...
    try:
        ...
        if usage is None:
            llm_usage = result.llm_output.get(
                "usage") if result.llm_output else None
            if llm_usage:
        ...
        if usage is None:
            # Checking the possibility of streaming
            generations = result.generations
            if generations is not None:
                for _, generation in enumerate(generations):
                    for _, gen in enumerate(generation):
                        generation_type = gen.type
                        if generation_type == "ChatGenerationChunk":
                            ...
                            if usage_data is not None:
                                ...
                                resp_metadata = gen.__dict__.get(
                                    "message").__dict__.get(
                                    "response_metadata")
                                # Checking for bedrock
                                ...
                                if usage_data is not None:
                                    usage = {
                                        ...
                                        "completion_tokens": usage_data.get("outputTokenCount", 0),
                                        "total_tokens": usage_data.get("inputTokenCount", 0) + usage_data.get("outputTokenCount", 0),
                                    }
