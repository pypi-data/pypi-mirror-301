...
class SentimentAnalyzer:
    ...
    def _load_model_and_tokenizer(self, model_path: Optional[str] = None):
        ...
        if model_path:
            ...
            model_full_path = os.path.join(parent_dir, model_path)
            if not os.path.isdir(model_full_path):
                raise FileNotFoundError(f"Model folder not found at path: {model_full_path}")
            model = BertForSequenceClassification.from_pretrained(model_full_path)
            tokenizer = BertTokenizer.from_pretrained(model_full_path)
        else:
            model = AutoModelForSequenceClassification.from_pretrained(self.default_model_name, force_download=True)
            tokenizer = AutoTokenizer.from_pretrained(self.default_model_name, force_download=True)
    ...
    def analyze(self, data: pd.DataFrame, length_limit: int = 500, enable_cuda: bool = False, model_choice: int = None,
        ...
        if model_choice == 0:
            model_path = "../finbert_hand_sentiment_model_press"
        elif model_choice == 1:
            model_path = "../finbert_hand_sentiment_model_news"
        else:
