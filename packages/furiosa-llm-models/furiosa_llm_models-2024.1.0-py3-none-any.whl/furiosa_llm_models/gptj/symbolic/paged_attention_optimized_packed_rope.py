...
class NewGPTJAttention(FastGPTJAttention):
    def forward(
        ...
        key, value = kv_page_update_do_input_select_with_past_kv_indices(
            present_key=key,
            present_value=value,
...
class GPTJModel(HfGPTJModel):
    ...
    def forward(
        ...
        bucket_size: int,
        is_prefill: bool,
        past_valid_key_indices: Optional[torch.IntTensor],
        past_valid_value_indices: Optional[torch.IntTensor],
        causal_mask: Optional[torch.BoolTensor] = None,
