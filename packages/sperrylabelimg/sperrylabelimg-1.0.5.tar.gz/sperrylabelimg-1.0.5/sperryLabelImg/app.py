...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def setup_write_dir_dock(self):
        ...
        self.add_write_dir_button = QPushButton(self.tr("Add Write Directory"))
        self.add_write_dir_button.clicked.connect(self.add_write_dir)
        write_dir_list_layout.addWidget(self.add_write_dir_button)

        write_dir_container = QWidget()
        write_dir_container.setLayout(write_dir_list_layout)

        self.write_dir_dock = QDockWidget(self.tr("Write Directories"), self)
    ...
    def add_write_dir(self):
        dir_path = QFileDialog.getExistingDirectory(self, self.tr("Select Write Directory"))
        if dir_path:
    ...
    def scan_all_images(self):
        images = []
        for read_dir in self.read_dirs + self.write_dirs:
            images.extend(self.scan_for_images(read_dir))
        return images

    def scan_all_for_file(self, name: str) -> str:
        for directory in self.read_dirs + self.write_dirs:
    ...
    def show_bounding_box_from_annotation_file(self, file_path):
        ...
        if not xml_path and not txt_path and not json_path:
            message = "Can't find annotation file in either the read or write directories"
            message += f"\nread directories: {self.read_dirs}"
            message += f"\nwrite directories: {self.write_dirs}"
            raise ValueError(message)

        if os.path.exists(xml_path):
            pass
            # find some default file path to use instead
    ...
    def open_dir_dialog(self, _value=False, dirpath=None, silent=False):
        ...
        if target_dir in self.write_dirs:
            self.write_dirs.remove(target_dir)
        elif target_dir in self.read_dirs:
            pass
        else:
            self.read_dirs.append(target_dir)
